...
list="hrrr"    #
for item in $list ; do
  cp $fixdir/${RUN}_select_stnlist_${item}.dat select_stnlist_${item}.dat
  cp $fixdir/${RUN}_non_viable_stnlocation_list_${item}.dat non_viable_stnlocation_list_${item}.dat
done
