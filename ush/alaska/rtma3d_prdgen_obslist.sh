...
cp $COMIN/${RUN}ak.t${cyc}z.diag_conv_ges.gz diag_conv_ges.gz
cp $COMIN/${RUN}ak.t${cyc}z.diag_conv_02.gz  diag_conv_02.gz
cp $COMIN/${RUN}ak.t${cyc}z.diag_conv_anl.gz diag_conv_anl.gz
...
cp $COMIN/postprd.t${cyc}z/${RUN}.t${cyc}z.wrfsubhprs.grib2 anlfile_1.grb2
...
&faa_grid_and_anlfile_info
    ngrids=1
    cgridset(1)='akhrrr'
    anlfileset(1)='anlfile_1.grb2'
    stnlistset(1)='select_stnlist_akhrrr.dat'
    non_viableset(1)='non_viable_stnlocation_list_akhrrr.dat'
    outfileset(1)='stn_analysis_values_akhrrr.dat'
    outsideset(1)='stn_outside_of_domain_akhrrr.dat'
/
&faa_timerange
    iyyyy0=$iyyyy0
    imm0=$imm0
