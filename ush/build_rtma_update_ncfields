...
dirname_source="rtma_update_ncfields.fd"
exefile_name_update_ncfields="rtma3d_updatevars_ncfields"
#=========================================================================#
...
echo " make -f makefile_${target}  >& ./log.make.updatevars_ncfields "
make -f makefile_${target} >& ./log.make.updatevars_ncfields

if [ $? -eq 0 ] ; then
  echo " code was built successfully."
  echo " cp -p ${SOURCE_DIR}/rtma3d_updatevars_ncfields   ${EXEC}/GSI/${exefile_name_update_ncfields} "
  cp -p ${SOURCE_DIR}/rtma3d_updatevars_ncfields   ${EXEC}/GSI/${exefile_name_update_ncfields}
  ls -l ${EXEC}/GSI/${exefile_name_update_ncfields}
else
  echo " ================ WARNING =============== " 
  echo " Compilation of process_updatevars_ncfields was failed."
  echo " Check up with the log file under ${SOURCE_DIR}"
  echo "   ----> log.make.process_updatevars_ncfields  : "
  echo " ================ WARNING =============== " 
fi
