...
dirname_source="rtma_updatevars.fd"
exefile_name_updatevars_wrf="rtma3d_updatevars_wrf"
exefile_name_updatevars_ndown="rtma3d_updatevars_ndown"
#=========================================================================#
...
#    module load module load PNetCDF/1.8.1
    export PNETCDF=/gpfs/dell1/usrx/local/prod/packages/ips/19.0.5/impi/19.0.5/PNetCDF/1.8.1
...
./clean -aa
cp config_dell configure.wrf
#echo "./compile -j 8 em_real >& ./log.make.updatevars_wrf"
./compile -j 8 em_real >& ./log.make.updatevars_wrf

if [ $? -eq 0 ] ; then
  echo " code was built successfully."
  echo " cp -p ${MAIN_DIR}/wrf.exe   ${EXEC}/GSI/${exefile_name_updatevars_wrf} "
  cp -p ${MAIN_DIR}/wrf.exe   ${EXEC}/GSI/${exefile_name_updatevars_wrf}
  ls -l ${EXEC}/GSI/${exefile_name_updatevars_wrf}
  echo " cp -p ${MAIN_DIR}/ndown.exe   ${EXEC}/GSI/${exefile_name_updatevars_ndown} "
  cp -p ${MAIN_DIR}/ndown.exe   ${EXEC}/GSI/${exefile_name_updatevars_ndown}
  ls -l ${EXEC}/GSI/${exefile_name_updatevars_ndown}
else
  echo " ================ WARNING =============== " 
