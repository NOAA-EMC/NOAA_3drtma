...
#  else:
  ...
  for ax in axes:
    print ll
    map = Basemap(ax=ax,projection='stere',lat_0=35.4,lon_0=-97.6,\
                  llcrnrlat=21,urcrnrlat=49,llcrnrlon=-120.5,urcrnrlon=-64.5,\
                  resolution='l')
    x,y=map(lons,lats)
...
#  plt.tight_layout()
  plt.savefig('comparetype_conus_f'+fhr+'.png', bbox_inches='tight',dpi=150)
  plt.close()
