...
#  else:
  ...
  for ax in axes:
    print ll
    map = Basemap(ax=ax,projection='stere',lat_0=32.0,lon_0=-98.0,\
                  llcrnrlat=25.0,urcrnrlat=37.0,llcrnrlon=-108.0,urcrnrlon=-88.0,\
                  resolution='l')
    x,y=map(lons,lats)
...
#  plt.tight_layout()
  plt.savefig('comparetype_SC_f'+fhr+'.png', bbox_inches='tight',dpi=150)
  plt.close()
