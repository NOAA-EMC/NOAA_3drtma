...
#  else:
  ...
  for ax in axes:
    print ll
    map = Basemap(ax=ax,projection='stere',lat_0=41.0,lon_0=-74.6,\
                  llcrnrlat=40.0,urcrnrlat=43.0,llcrnrlon=-75.75,urcrnrlon=-69.5,\
                  resolution='l')
    x,y=map(lons,lats)
...
#  plt.tight_layout()
  plt.savefig('comparetype_BN_f'+fhr+'.png', bbox_inches='tight',dpi=150)
  plt.close()
