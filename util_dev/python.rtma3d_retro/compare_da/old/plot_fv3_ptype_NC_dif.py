...
#  else:
  ...
  for ax in axes:
    print ll
    map = Basemap(ax=ax,projection='stere',lat_0=44.5,lon_0=-102.0,\
                  llcrnrlat=39.0,urcrnrlat=49.0,llcrnrlon=-111.0,urcrnrlon=-93.5,\
                  resolution='l')
    x,y=map(lons,lats)
...
#  plt.tight_layout()
  plt.savefig('comparetype_NC_f'+fhr+'.png', bbox_inches='tight',dpi=150)
  plt.close()
