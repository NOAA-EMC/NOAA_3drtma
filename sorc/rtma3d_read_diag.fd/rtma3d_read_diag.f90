...
!
  ...
  logical        :: l_tdry          ! if tdry is available in obsdiag for qob only
  real(r_single) :: rtdry           ! dry-bulb air temp (q obs only)
...
!     write(*,*) var, nchar,nreal,ii,mype
     read(17, ERR=999,iostat=ios,end=110) var, nchar,nreal,ii,mype,ioff0
     write(*,*) var, nchar,nreal,ii,mype,ioff0
     l_tdry = .false.                                 ! reset l_tdry to false (as default for all obs)
     if ( var(1:3) == '  q' .and. ioff0 >= 22) l_tdry = .true.
     if (ii > 0) then
