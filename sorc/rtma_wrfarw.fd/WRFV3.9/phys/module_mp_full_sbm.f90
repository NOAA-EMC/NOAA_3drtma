     ...
     & T_NEW(I,K,J),DT,rhocgs(I,K,J),COL,ICEMAX,NKR)
        ...
        IF (T_OLD(I,K,J).GT.213)THEN
         ...
         DO IKL=1,NCOND
          ...
          IF (DEL2NR.EQ.0)call wrf_error_fatal3("<stdin>",841,&
     ...
     & kts,kte,nkr)
      ...
      DO KR=1,NKR
       ...
       IF (IFALL.EQ.1)THEN
        ...
        END DO
        IF(TFALL.GE.1.E10) call wrf_error_fatal3("<stdin>",1804,&
       ...
       DOUBLE PRECISION CHECK,TPN
          CHECK=0.D0
           DO KR=1,NKR
             ...
             IF (CHECK.LT.0) call wrf_error_fatal3("<stdin>",4958,&
 ...
 ELSE IF(RI.NE.0.AND.RW*1.E5.LT.RI) THEN
     ...
     & (DEL1N_2P-RI*DEL2N_2P/PI)*DT+RI*DEL2INT_2P/PI
        ELSE
          ...
          IF (RW.LE.0) call wrf_error_fatal3("<stdin>",5641,&
"fatal error in module_mp_full_sbm (RW.LE.0), model stop")
          IF (PW.LE.0) call wrf_error_fatal3("<stdin>",5643,&
"fatal error in module_mp_full_sbm (PW.LE.0), model stop")
          IF (RI.LE.0) call wrf_error_fatal3("<stdin>",5645,&
"fatal error in module_mp_full_sbm (RI.LE.0), model stop")
          IF (PI.LE.0) call wrf_error_fatal3("<stdin>",5647,&
"fatal error in module_mp_full_sbm (PI.LE.0), model stop")
   ...
   ALFAPX=.5*(ALFA_DP+X_DP)
          IF (ALFAPX.LE.0) call wrf_error_fatal3("<stdin>",5652,&
...
10 continue
       do it = 1,ndiv
         ...
         if (ndiv.gt.10000) call wrf_error_fatal3("<stdin>",6995,&
...
es_zero = 611.21D0
if(pres_SI > 200000.D0.or.pres_SI < 10000.D0) call wrf_error_fatal3("<stdin>",8431,&
...
RHO_CRIT = 910.D0
if(rhoa_SI > 2.D0) call wrf_error_fatal3("<stdin>",8460,&
...
if(FLIQFR_I(IK,I) > 0.D0) then
  if(IEVAP_ADJUST == 1 ) then
    if(FLIQFR_I(IK,I) > 1.D0) call wrf_error_fatal3("<stdin>",8554,&
"fatal error in module_mp_full_sbm (FLIQFR_I > 1), model stop 114")
   ...
   XI_MELT_SI(IK,I)=XX_MELT
    ...
    QQV=QQV+Q_EVAP
    ee = QQV*pres_SI/(EPS + QQV)
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",8587,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",8589,&
"fatal error in module_mp_full_sbm , model stop 138")
  endif
else
  if(ISUBLIME_ADJUST == 1 ) then
    if(TS > 273.16) call wrf_error_fatal3("<stdin>",8598,&
"fatal error in module_mp_full_sbm , model stop 13655")
   ...
   (XI_SI(IK,I),rhoa_SI,Q_SUBL,FF2_SI(IK,I))
    ...
    QQV=QQV+Q_SUBL
    ee = QQV*pres_SI/(EPS + QQV)
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",8616,&
"fatal error in module_mp_full_sbm (QQV < 0), model stop ")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",8618,&
...
IF(FLIQFR_I(IK,I) > FLIQFRAC_LIM) then
  if(XL_SI(IK).NE.XI_SI(IK,I)) call wrf_error_fatal3("<stdin>",8628,&
...
if(FLIQFR_S(IK) > 0.D0) then
  if(IEVAP_ADJUST == 1) then
    if(FLIQFR_S(IK) > 1.D0) call wrf_error_fatal3("<stdin>",8717,&
"fatal error in module_mp_full_sbm , model stop 905")
   ...
   (XS_SI(IK),rhoa_SI,Q_EVAP,FLIQFR_S(IK),FF3_SI(IK))
    ...
    QQV=QQV+Q_EVAP
    ee = QQV*pres_SI/(EPS + QQV)
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",8742,&
"fatal error in module_mp_full_sbm , model stop 915")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",8744,&
"fatal error in module_mp_full_sbm , model stop 916")
  ...
  if(ISUBLIME_ADJUST == 1) then
    ...
    QQV=QQV+Q_SUBL
    ee = QQV*pres_SI/(EPS + QQV)
    if(QQV < 0.) call wrf_error_fatal3("<stdin>",8768,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.) call wrf_error_fatal3("<stdin>",8770,&
"fatal error in module_mp_full_sbm , model stop 138")
  endif
endif
if(FLIQFR_S(IK) < 0.D0) then
...
IF(FLIQFR_S(IK) > FLIQFRAC_LIM) then
  if(XL_SI(IK).NE.XS_SI(IK)) call wrf_error_fatal3("<stdin>",8780,&
...
if(FLIQFR_G(IK) > 0.D0) then
  if(IEVAP_ADJUST == 1) then
    if(FLIQFR_G(IK) > 1.D0) call wrf_error_fatal3("<stdin>",8906,&
"fatal error in module_mp_full_sbm , model stop 901")
  ...
  if(ISUBLIME_ADJUST == 1)then
    ...
    QQV = QQV + Q_SUBL
    ee = QQV*pres_SI/(EPS + QQV)
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",8946,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",8948,&
"fatal error in module_mp_full_sbm , model stop 138")
  endif
endif
IF(FLIQFR_G(IK) > FLIQFRAC_LIM) then
  if(XL_SI(IK).NE.XG_SI(IK)) call wrf_error_fatal3("<stdin>",8955,&
...
if(FLIQFR_H(IK) > 0.D0) then
  if(IEVAP_ADJUST == 1) then
    if( FLIQFR_H(IK) > 1.D0) call wrf_error_fatal3("<stdin>",9101,&
"fatal error in module_mp_full_sbm , model stop 906")
   ...
   (XH_SI(IK),rhoa_SI,Q_EVAP,FLIQFR_H(IK),FF5_SI(IK))
    ...
    QQV = QQV + Q_EVAP
    ee = QQV*pres_SI/(EPS + QQV)
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",9126,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",9128,&
"fatal error in module_mp_full_sbm , model stop 138")
  endif
else
  if(ISUBLIME_ADJUST == 1) then
    ...
    QQV = QQV + Q_SUBL
    ee = QQV*pres_SI/(EPS + QQV)
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",9152,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",9154,&
"fatal error in module_mp_full_sbm , model stop 138")
  endif
endif
if(FLIQFR_H(IK) < 0.D0) then
...
IF(FLIQFR_H(IK) > FLIQFRAC_LIM) then
  if(XL_SI(IK).NE.XH_SI(IK)) call wrf_error_fatal3("<stdin>",9164,&
...
if(FFX < 0.D0) call wrf_error_fatal3("<stdin>",9249,&
"fatal error in module_mp_full_sbm , model stop 136")
if(total_mass_liq < 0.D0) call wrf_error_fatal3("<stdin>",9251,&
"fatal error in module_mp_full_sbm , model stop 137")
if(total_mass_ice < 0.D0) call wrf_error_fatal3("<stdin>",9253,&
"fatal error in module_mp_full_sbm , model stop 140")
if(total_mass < 0.D0) call wrf_error_fatal3("<stdin>",9255,&
...
FFX = total_mass/(XX*XX*3.D0*COL/rhoax)
if(FFX < 0.D0) call wrf_error_fatal3("<stdin>",9285,&
"fatal error in module_mp_full_sbm , model stop 140")
if(total_mass < 0.D0) call wrf_error_fatal3("<stdin>",9287,&
"fatal error in module_mp_full_sbm , model stop 141")
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",9499,&
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",9518,&
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",9536,&
...
pc_hi = xxa(ix)
ENDDO
if(ix -1 < 1) call wrf_error_fatal3("<stdin>",9595,&
"fatal error in module_mp_full_sbm , model stop 42567")
if(ix > ix_max) call wrf_error_fatal3("<stdin>",9597,&
...
if(chi_fra > 0.3D0 .and. pc < 75.D0) call wrf_error_fatal3("<stdin>",9606,&
"fatal error in module_mp_full_sbm , model stop 1478")
if(chi_fra > 0.6D0 .and. pc < 90.D0) call wrf_error_fatal3("<stdin>",9608,&
...
if(IKX > NKR.or.IKX < 1) call wrf_error_fatal3("<stdin>",9702,&
...
if( epsil_i < 0.D0) call wrf_error_fatal3("<stdin>",9800,&
"fatal error in module_mp_full_sbm , model stop 9086")
epsil_i = epsil_i**0.5D0
if(epsil_i > 0.D0) then
  PLANAR_CAP_ZERO = a_ix*epsil_i/DASIN(epsil_i)
  if((1.D0+epsil_i)/(1.D0-epsil_i).le.0.D0) call wrf_error_fatal3("<stdin>",9808,&
...
ee = QQV*pres_SI/(EPS + QQV)
es_zero = 611.21D0
if(pres_SI > 200000.D0.or.pres_SI < 10000.D0) call wrf_error_fatal3("<stdin>",10285,&
...
FNPR = etaa/(rhoa_SI*ALPHA_H)
RHO_CRIT = 910.D0
if(rhoa_SI > 2.D0) call wrf_error_fatal3("<stdin>",10313,&
...
"fatal error in module_mp_full_sbm , model stop 9999")
    ...
    if(fm_w < fm_w_soaked) call wrf_error_fatal3("<stdin>",10660,&
"fatal error in module_mp_full_sbm , model stop 9065")
    ...
    if(INEW < INK) then
      ...
      if(fmass_X(INK) < 0.D0) call wrf_error_fatal3("<stdin>",10676,&
"fatal error in module_mp_full_sbm , model stop 8020")
      if(res_mass_shed < 0.D0) call wrf_error_fatal3("<stdin>",10678,&
...
"fatal error in module_mp_full_sbm , model stop 8003")
      ...
      if(FLIQFR_X(INEW+1) < 0.D0.or.FLIQFR_X(INEW+1) > 1.D0) call wrf_error_fatal3("<stdin>",10746,&
"fatal error in module_mp_full_sbm , model stop 8004")
      if(FLIQFR_X(INK) < 0.D0.or.FLIQFR_X(INK) > 1.D0) call wrf_error_fatal3("<stdin>",10748,&
