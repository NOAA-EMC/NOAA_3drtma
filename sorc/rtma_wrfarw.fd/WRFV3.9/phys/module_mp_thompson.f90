      ...
      LOGICAL, PARAMETER :: print250flag=.false.
      LOGICAL, PARAMETER :: print150flag=.false.
      LOGICAL, PARAMETER :: print100flag=.false.
      LOGICAL, PARAMETER :: print0flag=.false.
      LOGICAL, PARAMETER :: print200flag=.false.
      is_aerosol_aware = .FALSE.
      micro_init = .FALSE.
      has_CCN = .FALSE.
      has_IN = .FALSE.
      if(print250flag) then
      ...
      if (max_test .lt. eps) then
         ...
         do i = its, min(ide-1,ite)
         do k = kts, kte
            if (nwfa(i,k,j) .LT. 0.) then
              if(print0flag) then
                 write(mp_debug,*) 'Found negative QNWFA...replacing with non-negative values. Check met_em files.'
                 CALL wrf_debug(0, mp_debug)
              endif
              nwfa(i,k,j) = max( nwfa(i,max(k-1,kts),j) , nwfa(i,min(k+1,kte),j) )
              nwfa(i,k,j) = max( 0., nwfa(i,k,j) )
            endif
         enddo
         enddo
         enddo
      ...
      if (max_test .lt. eps) then
         ...
         do i = its, min(ide-1,ite)
         do k = kts, kte
            if (nifa(i,k,j) .LT. 0.) then
              if(print0flag) then
                 write(mp_debug,*) 'Found negative QNIFA...replacing with non-negative values. Check met_em files.'
                 CALL wrf_debug(0, mp_debug)
              endif
              nifa(i,k,j) = max( nifa(i,max(k-1,kts),j) , nifa(i,min(k+1,kte),j) )
              nifa(i,k,j) = max( 0., nifa(i,k,j) )
            endif
         enddo
         enddo
         enddo
      ...
      REAL, DIMENSION(ims:ime, jms:jme), OPTIONAL, INTENT(INOUT):: &
                          SNOWNC, SNOWNCV, GRAUPELNC, GRAUPELNCV, frain
      REAL, DIMENSION(ims:ime, kms:kme, jms:jme), INTENT(INOUT):: &
                          refl_10cm
      ...
      REAL, DIMENSION(1:50):: re_qc1d, re_qi1d, re_qs1d
      ...
      LOGICAL, PARAMETER :: print0flag=.false.
      LOGICAL, PARAMETER :: print150flag=.false.
      first_time_step = (itimestep .eq. 1)
      i_start = its
      j_start = jts
      i_end = MIN(ite, ide-1)
      j_end = MIN(jte, jde-1)
      if (rand_perturb_on .ne. 0) &
      min_rand = wrf_dm_max_real (MINVAL(rand_pert(i_start:i_end,1,j_start:j_end)) )
      dt = dt_in
      qc_max = 0.
      qr_max = 0.
      qs_max = 0.
      ...
      i_loop: do i = i_start, i_end
         ...
         pcp_gr(i,j) = pptgraul
         pcp_ic(i,j) = pptice
         IF ( PRESENT(frain) ) THEN
           frain(i,j) = 0.
           if(t1d(1) .le. 273) then
         ...
         if (is_aerosol_aware) then
            nwfa1d(1) = nwfa1d(1) + nwfa2d(i,j)*dt_in
            do k = 1, 50
               nc(i,k,j) = nc1d(k)
               nwfa(i,k,j) = nwfa1d(k)
               nifa(i,k,j) = nifa1d(k)
         ...
         do k = 1, 50
            ...
            if (qc1d(k) .gt. qc_max) then
             ...
             kmax_qc = k
             qc_max = qc1d(k)
            elseif (qc1d(k) .lt. 0.0 .and. print150flag) then
            ...
            if (qr1d(k) .gt. qr_max) then
             ...
             kmax_qr = k
             qr_max = qr1d(k)
            elseif (qr1d(k) .lt. 0.0 .and. print150flag) then
            ...
            if (nr1d(k) .gt. nr_max) then
             ...
             kmax_nr = k
             nr_max = nr1d(k)
            elseif (nr1d(k) .lt. 0.0 .and. print150flag) then
            ...
            if (qs1d(k) .gt. qs_max) then
             ...
             kmax_qs = k
             qs_max = qs1d(k)
            elseif (qs1d(k) .lt. 0.0 .and. print150flag) then
            ...
            if (qi1d(k) .gt. qi_max) then
             ...
             kmax_qi = k
             qi_max = qi1d(k)
            elseif (qi1d(k) .lt. 0.0 .and. print150flag) then
            ...
            if (qg1d(k) .gt. qg_max) then
             ...
             kmax_qg = k
             qg_max = qg1d(k)
            elseif (qg1d(k) .lt. 0.0 .and. print150flag) then
            ...
            if (ni1d(k) .gt. ni_max) then
             ...
             kmax_ni = k
             ni_max = ni1d(k)
            elseif (ni1d(k) .lt. 0.0 .and. print150flag) then
            ...
            if (qv1d(k) .lt. 0.0) then
             ...
             if (k.lt. 50 -2 .and. k.gt. 1 +1) then
                if(print150flag) then
      ...
      REAL, DIMENSION(1:50), INTENT(IN):: p1d, w1d, dzq
      REAL, INTENT(INOUT):: pptrain, pptsnow, pptgraul, pptice
      REAL, INTENT(IN):: dt
      REAL, INTENT(IN):: rand1, rand2, rand3
      REAL, DIMENSION(1:50):: tten, qvten, qcten, qiten, &
           qrten, qsten, qgten, niten, nrten, ncten, nwfaten, nifaten
      DOUBLE PRECISION, DIMENSION(1:50):: prw_vcd
      DOUBLE PRECISION, DIMENSION(1:50):: pnc_wcd, pnc_wau, pnc_rcw, &
           pnc_scw, pnc_gcw
      DOUBLE PRECISION, DIMENSION(1:50):: pna_rca, pna_sca, pna_gca, &
           pnd_rcd, pnd_scd, pnd_gcd
      DOUBLE PRECISION, DIMENSION(1:50):: prr_wau, prr_rcw, prr_rcs, &
           prr_rcg, prr_sml, prr_gml, &
           prr_rci, prv_rev, &
           pnr_wau, pnr_rcs, pnr_rcg, &
           pnr_rci, pnr_sml, pnr_gml, &
           pnr_rev, pnr_rcr, pnr_rfz
      DOUBLE PRECISION, DIMENSION(1:50):: pri_inu, pni_inu, pri_ihm, &
           pni_ihm, pri_wfz, pni_wfz, &
           pri_rfz, pni_rfz, pri_ide, &
           pni_ide, pri_rci, pni_rci, &
           pni_sci, pni_iau, pri_iha, pni_iha
      DOUBLE PRECISION, DIMENSION(1:50):: prs_iau, prs_sci, prs_rcs, &
           prs_scw, prs_sde, prs_ihm, &
           prs_ide
      DOUBLE PRECISION, DIMENSION(1:50):: prg_scw, prg_rfz, prg_gde, &
           prg_gcw, prg_rci, prg_rcs, &
           prg_rcg, prg_ihm
      DOUBLE PRECISION, PARAMETER:: zeroD0 = 0.0d0
      REAL, DIMENSION(1:50):: temp, pres, qv
      REAL, DIMENSION(1:50):: rc, ri, rr, rs, rg, ni, nr, nc, nwfa, nifa
      REAL, DIMENSION(1:50):: rho, rhof, rhof2
      REAL, DIMENSION(1:50):: qvs, qvsi, delQvs
      REAL, DIMENSION(1:50):: satw, sati, ssatw, ssati
      REAL, DIMENSION(1:50):: diffu, visco, vsc2, &
           tcond, lvap, ocp, lvt2
      DOUBLE PRECISION, DIMENSION(1:50):: ilamr, ilamg, N0_r, N0_g
      REAL, DIMENSION(1:50):: mvd_r, mvd_c
      REAL, DIMENSION(1:50):: smob, smo2, smo1, smo0, &
           smoc, smod, smoe, smof
      REAL, DIMENSION(1:50):: sed_r, sed_s, sed_g, sed_i, sed_n,sed_c
      ...
      REAL, DIMENSION(1:50 +1):: vtik, vtnik, vtrk, vtnrk, vtsk, vtgk, &
           vtck, vtnck
      REAL, DIMENSION(1:50):: vts_boost
      ...
      LOGICAL, DIMENSION(1:50):: L_qc, L_qi, L_qr, L_qs, L_qg
 ...
 151 continue
     ...
     & vts*((vtrk(k)-vts*vts_boost(k))/(temp(k)-T_0)))
      ...
      do n = 1, nstep
         ...
         enddo
         if (rr(1).gt.R1*10.) &
         pptrain = pptrain + sed_r(1)*DT*onstep(1)
      enddo
      do k = 50, 1, -1
         sed_c(k) = vtck(k)*rc(k)
      ...
      do n = 1, nstep
         ...
         enddo
         if (ri(1).gt.R1*10.) &
         pptice = pptice + sed_i(1)*DT*onstep(2)
      enddo
      ...
      do n = 1, nstep
         ...
         enddo
         if (rs(1).gt.R1*10.) &
         pptsnow = pptsnow + sed_s(1)*DT*onstep(3)
      enddo
      ...
      do n = 1, nstep
         ...
         enddo
         if (rg(1).gt.R1*10.) &
         pptgraul = pptgraul + sed_g(1)*DT*onstep(4)
      enddo
      if (.not. iiwarm) then
      do k = 1, 50
...
"Error writing freezeH2O.dat")
      ...
      LOGICAL, PARAMETER :: print_flag=.false.
      iunit_mp_th1 = -1
      IF ( wrf_dm_on_monitor() ) THEN
        DO i = 20,99
