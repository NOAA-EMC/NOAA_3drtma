MODULE module_sf_lake
 ...
 USE module_model_constants, ONLY : rcp
    ...
    SUBROUTINE Lake( t_phy ,p8w ,dz8w ,qvcurr ,&
                     ...
                     xlat_urb2d ,xlat ,xlong ,&
                     z_lake3d ,dz_lake3d ,lakedepth2d ,&
    ...
    INTEGER :: C,i,j,k
      ...
      real(r8) :: ram1(1)
      real(r8) :: z0mg(1)
      logical, dimension( ims:ime, jms:jme ) :: salty
      dtime = dtbl
        DO J = jts,jte
        DO I = its,ite
           SFCTMP = t_phy(i,1,j)
        ...
        if (lakemask(i,j).eq.1) THEN
           if ((xlong(i,j).gt.-117.7 .and. xlong(i,j).lt.-111.5) .and. &
              .not. (xlong(i,j).gt.-112.104 .and. xlong(i,j).lt.-112.100))then
             if(xlat(i,j).gt.39.5 .and. xlat(i,j).lt.41.22) then
                salty(i,j) = .true.
             elseif(( xlat(i,j).ge.41.22 .and. xlat(i,j).lt.42.) .and. .not. &
                    (xlat(i,j).gt.41.352 .and. xlat(i,j).lt.41.354)) then
                salty(i,j) = .true.
             endif
           endif
           if (xlong(i,j).gt.-119.3.and. xlong(i,j).lt.-118.8) then
             if(xlat(i,j).gt.37.9 .and. xlat(i,j).lt.38.2) then
                salty(i,j) = .true.
             endif
           endif
           do c = 1,column
            ...
            do k = 1,nlevlake
              ...
              if(salty(i,j) .and. t_lake(c,k) > tfrz .and. lake_icefrac3d(i,k,j) > 0.) then
               lake_icefrac(c,k) = 0.
              else
            ...
            enddo
              if(lake_icefrac(c,1) == 0. .and. h2osno(c) > 0.) then
               snowdp(c) = 0.
               h2osno(c) = 0.
               snl(c) = 0.
               t_grnd(c) = t_lake(c,1)
        ...
        h2osoi_ice(c,k) = h2osoi_ice3d(i,k,j)
          ...
          enddo
           ...
           if ((xlong(i,j).gt.-117.7 .and. xlong(i,j).lt.-111.5) .and. &
             ...
             elseif(( xlat(i,j).ge.41.22 .and. xlat(i,j).lt.42.) .and. .not. &
                    (xlat(i,j).gt.41.352 .and. xlat(i,j).lt.41.354)) then
                t_grnd(c) = max(274.,t_grnd(c))
                do k = 1,nlevlake
                  t_lake(c,k) = max(274.,t_lake(c,k))
                  lake_icefrac(c,k) = 0.
                enddo
             endif
           endif
           ...
           do c = 1,column
            ...
            if(h2osno2d(i,j) == 0.) snowdp2d(i,j) = 0.
     snl2d(i,j) = snl(c)
    ...
    CALL ShalLakeTemperature(t_grnd,h2osno,sabg,dz,dz_lake,z,zi, &
                                 ...
                                 h2osoi_ice,savedtke1, &
                                 frac_iceold,qflx_snomelt,imelt,ii,jj)
...
!dir$ concurrent
    do fp = 1, num_shlakep
       ...
       if (t_grnd(c) >= tfrz) then
        ...
        tc=forc_t(g)-273.15
        visc=1.326e-5*(1. + 6.542e-3*tc + 8.301e-6*tc*tc &
                        - 4.84e-9*tc*tc*tc)
        Ren = MAX(ustar(p)*z0mg(p)/visc, 0.1)
        z0hg(p) = (5.5e-5)*(Ren**(-0.60))
        z0hg(p) = MIN(z0hg(p),1.0e-4)
        z0hg(p) = MAX(z0hg(p),2.0e-9)
        z0qg(p) = z0hg(p)
       if (t_grnd(c) > tfrz) then
          htvp(c) = hvap
       else
       ...
       do fc = 1, num_shlakec
          ...
          if (j >= jtop(c)) then
             ...
             if(abs(t_soisno(c,j)-288) > 150) then
                WRITE( message,* ) 'WARNING: Extreme t_soisno at c, level',c, j, ii,jj,t_soisno(c,j)
...
subroutine LakeDebug( str )
 ...
 SUBROUTINE lakeini(id, IVGTYP, ISLTYP, HT, T_soibot2d, SNOW,SNOWH, &
                    ...
                    cycling, &
                    lakemask, lakeflag, &
                    lake_depth_flag, use_lakedepth, julday, xlong, xlat, &
                    tkdry3d, tksatu3d, lake, &
  ...
  INTEGER, DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: IVGTYP, &
                                                              ISLTYP
  REAL, DIMENSION( ims:ime, jms:jme ), INTENT(IN) :: HT, t_soibot2d
  ...
  integer :: numb_lak
  character*256 :: message
  INTEGER, DIMENSION( ims:ime, jms:jme ) :: lakeinit
  REAL , DIMENSION( ims:ime, jms:jme ) :: lakedepth
  REAL :: rhosn
  logical :: debug
  real :: wght1, wght2, TclimGSL
  integer :: month, iday
  integer :: day1, day2, mon, num1, num2, juld
  REAL :: SaltLk_T(1:25)
  real :: julm(1:13)
  data (SaltLk_T(i), i=1,25)/0.5, 0.,-0.5, 3., 4., 7., 8., 12., 13., 16., 19., 21., &
                           23.5, 25.,26.,24.,23.,20.5,18.,15., 11.5, 8., 4., 1., 0.5/
  data (julm(i),i=1,13)/0,31,59,90,120,151,181,212,243,273,304,334,365/
  debug = .false.
  CALL nl_get_start_month(id,month)
  CALL nl_get_start_day(id,iday)
    if(debug) print *,'day,month',iday,month
  mon = month
  ...
  if(debug) print *,'num1,num2,day1,day2,wght1,wght2',num1,num2,day1,day2,wght1,wght2
  IF ( RESTART ) RETURN
  DO j = jts,jte
  DO i = its,ite
    if(lake_depth(i,j) == 10.) then
     if( lakemask(i,j)==0) then
      ...
      lakedepth(i,j) = lake_depth(i,j)
    endif
     lakeinit(i,j) = defval
  if((.not. cycling .and. lakedepth(i,j) > 0.) .or. (lakeflag > 0 .and. lakemask(i,j)==1) &
      .or. (lakeflag == 0 .and. ht(i,j)>=lake_min_elev .and. &
           lakedepth(i,j) > 0. ) ) then
   if(t_lake3d(i,1,j) < 150. .or. t_lake3d(i,1,j) > 350.) then
     lakeinit(i,j) = 0
     lake_depth_flag = 1
   else
     lakeinit(i,j) = 1
  ...
  if(debug) then
    if (i==1002.and.j==1006) then
       print *,'lakeinit(i,j) = ', i,j,lakeinit(i,j)
       print *,'lake_min_elev,t_soisno3d(i,1,j),t_lake3d(i,1,j) ',i,j,lake_min_elev,t_soisno3d(i,1,j),t_lake3d(i,1,j)
       print *,'cycling, lakeflag,lakemask(i,j),lake_depth_flag,lakedepth_default,ht(i,j),lake_depth(i,j)', &
                cycling, lakeflag,lakemask(i,j),lake_depth_flag,lakedepth_default,ht(i,j),lake_depth(i,j)
       print *,'use_lakedepth=',use_lakedepth
    endif
  endif
  ENDDO
  ENDDO
  ...
  DO i = its,ite
   if(lakeinit(i,j) == 0) then
    ...
    enddo
        snowdp2d(i,j) = snowh(i,j)
        h2osno2d(i,j) = snow(i,j)
   elseif(lakeinit(i,j) == 1) then
        xice(i,j) = lake_icefrac3d(i,1,j)
   endif
  ENDDO
  ENDDO
   numb_lak = 0
       do i=its,ite
         ...
         IF (lakeflag.eq.0) THEN
            if(ht(i,j)>=lake_min_elev .and. lakedepth(i,j) > 0. ) then
            ...
            if(ivgtyp(i,j)==iswater.and.ht(i,j)>=lake_min_elev.and.lakedepth(i,j) >0.) then
                lake(i,j) = .true.
                lakemask(i,j) = 1
                numb_lak = numb_lak + 1
            else
     ...
     if ( lake(i,j) ) then
       if(lakeinit(i,j) /= 0) then
           if ((xlong(i,j).gt.-117.7 .and. xlong(i,j).lt.-111.5) .and. &
              .not. (xlong(i,j).gt.-112.104 .and. xlong(i,j).lt.-112.100))then
             if(xlat(i,j).gt.39.5 .and. xlat(i,j).lt.41.22) then
                TclimGSL = 273.15 + wght1*saltlk_t(num1) &
                                  + wght2*saltlk_t(num2)
                if(debug) print *,'TclimGSL,TSK,t_lake3d',i,j,TclimGSL,TSK(i,j),t_lake3d(i,:,j),t_soisno3d(i,:,j)
                tsk(i,j) = min(TclimGSL+3.,(max(tsk(i,j),TclimGSL-3.0)))
                t_grnd2d(i,j) = tsk(i,j)
                do k = 1,nlevlake
                  t_lake3d(i,k,j) = min(TclimGSL+3.,(max(t_lake3d(i,k,j),TclimGSL-3.0)))
                enddo
                  t_soisno3d(i,1,j) = min(TclimGSL+3.,(max(t_soisno3d(i,1,j),TclimGSL-3.0)))
                if(debug) print *,'After TclimGSL,TSK,t_lake3d',i,j,TclimGSL,TSK(i,j),t_lake3d(i,:,j),t_soisno3d(i,:,j)
             elseif(( xlat(i,j).ge.41.22 .and. xlat(i,j).lt.42.) .and. .not. &
                    (xlat(i,j).gt.41.352 .and. xlat(i,j).lt.41.354)) then
                TclimGSL = 273.15 + wght1*saltlk_t(num1) &
                                  + wght2*saltlk_t(num2)
                ...
                t_grnd2d(i,j) = tsk(i,j)
                do k = 1,nlevlake
                  t_lake3d(i,k,j) = min(TclimGSL+3.0,(max(t_lake3d(i,k,j),TclimGSL-3.0)))
                enddo
                  t_soisno3d(i,1,j) = min(TclimGSL+3.,(max(t_soisno3d(i,1,j),TclimGSL-3.0)))
                if(debug) print *,'After TclimGSL,TSK,t_lake3d',i,j,TclimGSL,TSK(i,j),t_lake3d(i,:,j),t_soisno3d(i,:,j)
             endif
...
'STOP: You need lake-depth information. Rerun WPS or set use_lakedepth = 0')
        ...
        if ( lake_depth_flag.eq.1 ) then
          if (lakedepth(i,j) > 0.0) then
            lakedepth2d(i,j) = lakedepth(i,j)
          else
  ...
  zlak(10)= 0.95_r8
   ...
   if (debug) print *,'zsoi, dzsoi, zisoi =',zsoi, dzsoi, zisoi
  DO j = jts,jte
  DO i = its,ite
     if ( lake(i,j) ) then
         isl = ISLTYP(i,j)
         ...
         if(i==275.and.j==707 .or. i==435.and.j==314) then
           print *,'z_lake3d(i,:,j)i,dz_lake3d(i,:,j)',i,j,z_lake3d(i,:,j),dz_lake3d(i,:,j)
          end if
        ...
        t_soisno3d(i,nlevsoil,j) = T_soibot2d(i,j)
      if(lakeinit(i,j) == 0) then
        t_grnd2d(i,j) = tsk(i,j)
        t_lake3d(i,1,j) = tsk(i,j)
        if(snowdp2d(i,j) > 0.) then
          do k = snl2d(i,j)+1, 0
            t_soisno3d(i,k,j) =tsk(i,j)
          enddo
        endif
        do k = 2, nlevlake
          if(z_lake3d(i,k,j).le.depth_c) then
           t_lake3d(i,k,j)=t_lake3d(i,1,j)+(277.0-t_lake3d(i,1,j))/depth_c*z_lake3d(i,k,j)
          else
           t_lake3d(i,k,j) = 277.0
          end if
        enddo
          t_soisno3d(i,1,j) = t_lake3d(i,nlevlake,j)
          t_soisno3d(i,nlevsoil,j) = T_soibot2d(i,j)
          do k = 2, nlevsoil-1
           t_soisno3d(i,k,j)=t_soisno3d(i,1,j)+(T_soibot2d(i,j)-t_soisno3d(i,1,j))*dz3d(i,k,j)
          enddo
          if(debug) then
            print *,'i,j,t_lake3d(i,:,j)',i,j,t_lake3d(i,:,j)
            print *,'t_soibot2d(i,j),snl2d(i,j)', T_soibot2d(i,j),snl2d(i,j)
            print *,'i,j,t_soisno3d(i,:,j)',i,j,T_soibot2d(i,j),t_soisno3d(i,:,j)
          endif
        endif
        if (snl2d(i,j) < 0) then
           do k = snl2d(i,j)+1, 0
             if ( lakeinit(i,j)==0 .or. t_soisno3d(i,k,j) > 300 .or. &
                  t_soisno3d(i,k,j) < 200) t_soisno3d(i,k,j) = tsk(i,j)
           enddo
        end if
          if(debug) then
            print *,'i,j,T_soisno3d(i,:,j)',i,j,T_soibot2d(i,j),t_soisno3d(i,:,j)
      ...
      if(lakeinit(i,j) == 0) then
        ...
        if(h2osno2d(i,j).gt.0. .and. snowdp2d(i,j).gt.0.) then
          rhosn = h2osno2d(i,j)/snowdp2d(i,j)
        else
          rhosn = bdsno
        endif
        if(debug) print *,'rhosn=',rhosn,i,j
        do k = -nlevsnow+1, 0
           if (k > snl2d(i,j)) then
