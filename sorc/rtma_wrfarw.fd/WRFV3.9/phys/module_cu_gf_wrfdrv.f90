...
CONTAINS
   SUBROUTINE GFDRV(spp_conv,pattern_spp_conv,field_conv, &
              ...
              ,bl_mynn_edmf &
              ,chem,chem_opt,gf_convmix,gf_wetrmv &
              ,num_chem )
   IMPLICIT NONE
   ...
   integer, intent (in ) :: ichoice,num_chem
   INTEGER, INTENT(IN ) :: ishallow_g3,bl_mynn_edmf
   ...
   INTEGER, INTENT(IN) :: chem_opt, gf_convmix, gf_wetrmv
   ...
   REAL, DIMENSION( its:ite ) :: cap_suppress_j
     ...
     real, dimension (its:ite) :: &
        hfxi,qfxi,dxi
     real, dimension (its:ite,kts:kte,max(1,num_chem-1)) :: &
        chem_loc,outc,outcs,outcm
     real, dimension (max(1,num_chem-1)) :: alpha_sc
     ...
     real, dimension (its:ite) :: &
        edt,edtm,mconv,edtd
   INTEGER :: i,j,k,ICLDCK,ipr,jpr,n, numc
   REAL :: tcrit,dp,dq,chem_temp, maxchem
   INTEGER :: itf,jtf,ktf,iss,jss,nbegin,nend
   REAL :: chammax,rkbcon,rktop
   ...
   do i=its,ite
     k22_shallow(i,j)=0
     kbcon_shallow(i,j)=0
     IF(bl_mynn_edmf==0)ktop_shallow(i,j)=0
     xmb_shallow(i,j)=0
   enddo
   enddo
   endif
...
ENDIF
       if(ishallow_g3 == 1 )then
          ...
          CALL neg_check('shallow',ipr,dt,q2d,outqs,outts,outus,outvs, &
                         outqcs,outcs,prets,its,ite,kts,kte,itf,ktf,numc,ktops)
        endif
alpha_sc(:)=0.0
IF (numc>0) THEN
 DO nv=1,max(1,num_chem-1)
   DO k=kts,ktf
      DO i = its,itf
         chem_loc(i,k,nv)= chem(i,k,j,nv+1)
      enddo
   enddo
 ENDDO
ENDIF
...
ENDIF
   if(imid_gf == 1)then
      ...
      CALL neg_check('mid',ipr,dt,qcheck,outqm,outtm,outum,outvm, &
                     outqcm,outcm,pretm,its,ite,kts,kte,itf,ktf,numc,ktopm)
    endif
      if(do_capsuppress == 1 ) then
        DO I= its,itf
            cap_suppress_j(i)=cap_suppress_loc(i,j)
        ...
        IF(PRESENT(k22_shallow)) THEN
         ...
         ENDIF
          ...
          IF (numc>0) THEN
            DO nv=2,num_chem
              DO i=ibegc,iendc
                maxchem=maxval(chem(i,kts:ktf,j,nv))
                DO k=kts,ktf
                  chem_temp= max( 1.e-16,chem(i,k,j,nv) + outc(i,k,nv-1)*dt )
                  chem(i,k,j,nv)= min( maxchem,chem_temp )
                ENDDO
              ENDDO
            ENDDO
           ENDIF
