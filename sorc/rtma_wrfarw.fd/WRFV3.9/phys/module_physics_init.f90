...
CONTAINS
   ...
   REAL, DIMENSION( ims:ime , jms:jme ) , INTENT(IN) :: XLAT, XLONG,xlong_u,xlat_v
  ...
  REAL, DIMENSION( ims:ime , kms:kme , jms:jme ) , INTENT(INOUT) :: &
             RUBLTEN,RVBLTEN,RTHBLTEN,RQVBLTEN,RQCBLTEN,RQIBLTEN,EXCH_H
  REAL, DIMENSION( ims:ime , kms:kme , jms:jme ) , INTENT(INOUT), &
             OPTIONAL :: QKE,TKE_PBL
  REAL, DIMENSION( ims:ime , kms:kme , jms:jme ) , INTENT(OUT), OPTIONAL :: &
   ...
   REAL, OPTIONAL, DIMENSION(ims:ime,jms:jme), INTENT(IN) :: rivermask
   REAL, OPTIONAL, DIMENSION(ims:ime,jms:jme), INTENT(IN) :: nonriverxy
 ...
 ENDIF
  ...
  ENDIF
   ...
   CALL bl_init(id,STEPBL,BLDT,DT,RUBLTEN,RVBLTEN,RTHBLTEN, &
                ...
                num_soil_layers,TKE_PBL,mfshconv,julday, &
                XLAT,XLONG, &
                massflux_EDKF, entr_EDKF, detr_EDKF, &
                thl_up, thv_up, rt_up, &
                rv_up, rc_up, u_up, v_up, &
        ...
        DO ls=1,luseas
          ...
          DO LC=1,LUCATS
            IF(found_lu)THEN
              ...
              IF(LC.NE.LI)CALL wrf_error_fatal3("<stdin>",1354,&
...
'Need CAM or RRTMG radiation for bucket_J option')
   ...
   REAL, DIMENSION( ims:ime , kms:kme , jms:jme ) , INTENT(INOUT) :: &
                                                          ...
                                                          RQCBLTEN, &
                                                          RQIBLTEN
   REAL, DIMENSION( ims:ime , kms:kme , jms:jme ) , INTENT(INOUT), OPTIONAL :: TKE_PBL
   ...
   sfclay_select: SELECT CASE(config_flags%sf_sfclay_physics)
      ...
      CASE (GFSSFCSCHEME)
           CALL myjsfcinit(LOWLYR,UST, &
                         ...
                         its, ite, jts, jte, kts, kte )
           isfc = 2
        CASE (MYNNSFCSCHEME, MYNNSFCSCHEMEV34, MYNNSFCSCHEMEV36)
...
"SF CLM CURRENTLY ifdef'ed out, set -DWRF_USE_CLM in configure.wrf in ARCH_LOCAL" )
      CASE DEFAULT
   ...
   ENDIF
     IF ( LakeModel == 1 ) THEN
             call lakeini(id, IVGTYP, ISLTYP, HT, TMN, SNOW, SNOWH, &
      ...
      CASE (BOULACSCHEME)
        ...
        CASE (CAMUWPBLSCHEME)
           ...
           CALL camuwpblinit(RUBLTEN,RVBLTEN,RTHBLTEN,RQVBLTEN, &
                ...
                ims, ime, jms, jme, kms, kme, &
                its, ite, jts, jte, kts, kte )
        CASE (MYNNPBLSCHEME2, MYNNPBLSCHEME3, MYNNPBLSCHEMEV34, MYNNPBLSCHEMEV36)
           ...
           SELECT CASE(config_flags%bl_pbl_physics)
             ...
             CASE(MYNNPBLSCHEMEV34)
                mynn_closure_level=2
             CASE(MYNNPBLSCHEMEV36)
                mynn_closure_level=2
             CASE DEFAULT
           END SELECT
...
'arguments not present for calling TEMF scheme' )
   ...
   USE module_cu_gf_wrfdrv, ONLY : GFINIT
   USE module_cu_sas
