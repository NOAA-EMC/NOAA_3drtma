...
CONTAINS
  ...
  SUBROUTINE mym_level2 (kts,kte,&
       ...
       & dtl, dqw, dtv, gm, gh, sm, sh )
    INTEGER, INTENT(IN) :: kts,kte
    REAL, DIMENSION(1:50), INTENT(in) :: dz
    REAL, DIMENSION(1:50), INTENT(in) :: u,v,thl,qw,ql,vt,vq
    REAL, DIMENSION(1:50), INTENT(out) :: &
         &dtl,dqw,dtv,gm,gh,sm,sh
    INTEGER :: k
  ...
  SUBROUTINE mym_length ( &
    ...
    INTEGER, INTENT(IN) :: bl_mynn_mixlength,bl_mynn_edmf
    REAL, DIMENSION(1:50), INTENT(in) :: dz
    REAL, DIMENSION(1:50 +1), INTENT(in) :: zw
    REAL, INTENT(in) :: rmo,flt,flq,Psig_bl
    REAL, DIMENSION(1:50), INTENT(IN) :: qke,vt,vq,cldfra_bl1D,&
                                          edmf_w1,edmf_a1,edmf_qc1
    REAL, DIMENSION(1:50), INTENT(out) :: qkw, el
    REAL, DIMENSION(1:50), INTENT(in) :: dtv
    REAL :: elt,vsc
    REAL, DIMENSION(1:50), INTENT(IN) :: theta
    REAL, DIMENSION(1:50) :: qtke,elBLmin,elBLavg,thetaw
    ...
    SELECT CASE(bl_mynn_mixlength)
      CASE (0)
        ...
        alp4 = 100.
        alp5 = 0.4
        zi2 = MIN(10000.,zw(50 -2))
        h1=MAX(0.3*zi2,mindz)
        h1=MIN(h1,maxdz)
        h2=h1/2.0
        qkw(1) = SQRT(MAX(qke(1),1.0e-10))
        DO k = 1 +1,50
           afk = dz(k)/( dz(k)+dz(k-1) )
           abk = 1.0 -afk
        ...
        END DO
        elt = alp1*elt/vsc
        vflx = ( vt(1)+1.0 )*flt +( vq(1)+tv0 )*flq
      ...
      CASE (1)
        ...
        el(1) = 0.0
        zwk1 = zw(1 +1)
        CALL boulac_length(1,50,zw,dz,qtke,thetaw,elBLmin,elBLavg)
        DO k = 1 +1,50
           zwk = zw(k)
           IF ( dtv(k) .GT. 0.0 ) THEN
              bv = SQRT( gtr*dtv(k) )
  ...
  SUBROUTINE mym_turbulence ( &
    ...
    REAL, DIMENSION(1:50), INTENT(in) :: u,v,thl,qw,&
         &ql,vt,vq,qke,tsq,qsq,cov,cldfra_bl1D,edmf_w1,edmf_a1,edmf_qc1,&
         &TKEprodTD
    REAL, DIMENSION(1:50), INTENT(out) :: dfm,dfh,dfq,&
         &pdk,pdt,pdq,pdc,tcd,qcd,el
    REAL, DIMENSION(1:50), INTENT(inout) :: &
         qWT1D,qSHEAR1D,qBUOY1D,qDISS1D
    REAL :: q3sq_old,dlsq1,qWTP_old,qWTP_new
    REAL :: dudz,dvdz,dTdz,&
            upwp,vpwp,Tpwp
    INTEGER, INTENT(in) :: bl_mynn_tkebudget
    REAL, DIMENSION(1:50) :: qkw,dtl,dqw,dtv,gm,gh,sm,sh
    INTEGER :: k
   ...
   IF ( bl_mynn_tkebudget == 1) THEN
      qWT1D(1)=0.
      qSHEAR1D(1)=qSHEAR1D(1 +1)
      qBUOY1D(1)=qBUOY1D(1 +1)
      qDISS1D(1)=qDISS1D(1 +1)
   ENDIF
    if (spp_pbl==1) then
       DO k = 1,50
  ...
  SUBROUTINE mym_predict (kts,kte,&
    ...
    REAL, INTENT(IN) :: delt
    REAL, DIMENSION(1:50), INTENT(IN) :: dz, dfq,el
    REAL, DIMENSION(1:50), INTENT(INOUT) :: pdk, pdt, pdq, pdc
    REAL, INTENT(IN) :: flt, flq, ust, pmz, phh
    REAL, DIMENSION(1:50), INTENT(INOUT) :: qke,tsq, qsq, cov
    REAL, DIMENSION(1:50 +1), INTENT(INOUT) :: s_awqke,s_aw
    INTEGER :: k
    REAL, DIMENSION(1:50) :: qkw, bp, rp, df3q
    REAL :: vkz,pdk1,phm,pdt1,pdq1,pdc1,b1l,b2l,onoff
    REAL, DIMENSION(1:50) :: dtz
    REAL, DIMENSION(1:50) :: a,b,c,d,x
    ...
    END DO
    DO k=1,50 -1
       a(k-1 +1)=-dtz(k)*df3q(k) + 0.5*dtz(k)*s_aw(k)*onoff
       b(k-1 +1)=1. + dtz(k)*(df3q(k)+df3q(k+1)) &
                     + 0.5*dtz(k)*(s_aw(k)-s_aw(k+1))*onoff + bp(k)*delt
       c(k-1 +1)=-dtz(k)*df3q(k+1) - 0.5*dtz(k)*s_aw(k+1)*onoff
       d(k-1 +1)=rp(k)*delt + qke(k) + dtz(k)*(s_awqke(k)-s_awqke(k+1))*onoff
    ENDDO
    a(50)=-1.
    b(50)=1.
    c(50)=0.
    d(50)=0.
    CALL tridiag2(50,a,b,c,d,x)
    DO k=1,50
       qke(k)=max(x(k), 1.e-4)
    ENDDO
    IF ( levflag .EQ. 3 ) THEN
       ...
       DO k=1,50 -1
          a(k-1 +1)=-dtz(k)*dfq(k)
          b(k-1 +1)=1.+dtz(k)*(dfq(k)+dfq(k+1))+bp(k)*delt
          c(k-1 +1)=-dtz(k)*dfq(k+1)
          d(k-1 +1)=rp(k)*delt + tsq(k)
       ENDDO
       a(50)=-1.
       b(50)=1.
       c(50)=0.
  ...
  SUBROUTINE mym_condensation (kts,kte, &
    ...
    REAL, DIMENSION(1:50), INTENT(IN) :: dz
    REAL, DIMENSION(1:50 +1), INTENT(IN) :: zw
    REAL, DIMENSION(1:50), INTENT(IN) :: p,exner, thl, qw, &
         &tsq, qsq, cov, th
    REAL, DIMENSION(1:50), INTENT(INOUT) :: vt,vq,sgm
    REAL, DIMENSION(1:50) :: qmq,alp,a,bet,b,ql,q1,cld,RH
    REAL, DIMENSION(1:50), INTENT(OUT) :: qc_bl1D,cldfra_bl1D
    ...
    REAL, DIMENSION(1:50), INTENT(IN) :: Sh,el
 ...
 86 continue
    ...
    SELECT CASE(bl_mynn_cloudpdf)
      ...
      CASE (1, -1)
        DO k = 1,50 -1
           ...
           dth = 0.5*(thl(k+1)+thl(k)) - 0.5*(thl(k)+thl(MAX(k-1,1)))
           dqw = 0.5*(qw(k+1) + qw(k)) - 0.5*(qw(k) + qw(MAX(k-1,1)))
      ...
      CASE (2, -2)
        DO k = 1,50 -1
           ...
           dtl = 0.5*(thl(k+1)*(p(k+1)/p1000mb)**rcp + tlk) &
               & - 0.5*(tlk + thl(MAX(k-1,1))*(p(MAX(k-1,1))/p1000mb)**rcp)
           dqw = 0.5*(qw(k+1) + qw(k)) - 0.5*(qw(k) + qw(MAX(k-1,1)))
           if (k .eq. 1) then
             dzk = 0.5*dz(k)
           else
  ...
  SUBROUTINE mynn_tendencies(kts,kte, &
    ...
    LOGICAL, INTENT(IN) :: FLAG_QI,FLAG_QNI,FLAG_QC,FLAG_QNC,&
                           FLAG_QNWFA,FLAG_QNIFA
    REAL, DIMENSION(1:50 +1), INTENT(in) :: s_aw,s_awthl,s_awqt,&
         &s_awqnc,s_awqni,s_awqv,s_awqc,s_awu,s_awv,s_awqnwfa,s_awqnifa
    REAL, DIMENSION(1:50), INTENT(in) :: u,v,th,tk,qv,qc,qi,qni,qnc,&
         &rho,p,exner,dfq,dz,tsq,qsq,cov,tcd,qcd,cldfra_bl1d,diss_heat
    REAL, DIMENSION(1:50), INTENT(inout) :: thl,sqw,sqv,sqc,sqi,&
         &qnwfa,qnifa,dfm,dfh
    REAL, DIMENSION(1:50), INTENT(inout) :: du,dv,dth,dqv,dqc,dqi,&
         &dqni,dqnc,dqnwfa,dqnifa
    REAL, INTENT(IN) :: delt,ust,flt,flq,flqv,flqc,wspd,uoce,voce,qcg
    REAL, DIMENSION(1:50) :: dtz,vt,vq,dfhc,dfmc
    REAL, DIMENSION(1:50) :: sqv2,sqc2,sqi2,sqw2,qni2,qnc2, &
                                qnwfa2,qnifa2
    REAL, DIMENSION(1:50) :: zfac,plumeKh
    REAL, DIMENSION(1:50) :: a,b,c,d,x
    REAL, DIMENSION(1:50 +1) :: rhoz, &
          & khdz, kmdz
    ...
    km=dfm(1)*dz(1)
    rhoz(1)=rho(1)
    khdz(1)=rhoz(1)*kh/dz(1)
    kmdz(1)=rhoz(1)*km/dz(1)
    DO k=1 +1,50
       dtz(k)=delt/dz(k)
    ...
    kh=dfh(50)*dz(50)
    km=dfm(50)*dz(50)
    khdz(50 +1)=rhoz(50 +1)*kh/dz(50)
    kmdz(50 +1)=rhoz(50 +1)*km/dz(50)
    k=1
    a(1)=0.
    ...
    d(k)=thl(k) + dtz(k)*flt + tcd(k)*delt &
        & -dtz(k)*s_awthl(1 +1) + diss_heat(k)*delt*dheat_opt
    DO k=1 +1,50 -1
...
ENDIF
   ...
   logical :: cloudflg
    ...
    IF (bl_mynn_edmf > 0) THEN
      IF (bl_mynn_edmf > 1) THEN
         edmf_a(its:ite,1:50,jts:jte)=0.
         edmf_w(its:ite,1:50,jts:jte)=0.
         edmf_qt(its:ite,1:50,jts:jte)=0.
         edmf_thl(its:ite,1:50,jts:jte)=0.
         edmf_ent(its:ite,1:50,jts:jte)=0.
         edmf_qc(its:ite,1:50,jts:jte)=0.
      ENDIF
    ...
    IF (initflag > 0) THEN
       IF ( (restart .or. cycling)) THEN
          IF (MAXVAL(QKE(its:ite,1,jts:jte)) < 0.0002) THEN
             INITIALIZE_QKE = .TRUE.
          ELSE
       ...
       ELSE
          INITIALIZE_QKE = .TRUE.
       ENDIF
       Sh3D(its:ite,1:50,jts:jte)=0.
       el_pbl(its:ite,1:50,jts:jte)=0.
       tsq(its:ite,1:50,jts:jte)=0.
       qsq(its:ite,1:50,jts:jte)=0.
       cov(its:ite,1:50,jts:jte)=0.
       dqc1(1:50)=0.0
       dqi1(1:50)=0.0
       dqni1(1:50)=0.0
       dqnc1(1:50)=0.0
       ...
       dqnifa1(1:50)=0.0
       qc_bl1D(1:50)=0.0
       cldfra_bl1D(1:50)=0.0
       qc_bl1D_old(1:50)=0.0
       cldfra_bl1D_old(1:50)=0.0
       edmf_a1(1:50)=0.0
       edmf_w1(1:50)=0.0
       edmf_qc1(1:50)=0.0
       sgm(1:50)=0.0
    ...
    DO j=JTS,JTF
       DO i=ITS,ITF
          ...
          ENDIF
          sqcg= 0.0
          cpm=cp*(1.+0.84*qv(i,1,j))
          exnerg=(ps(i,j)/p1000mb)**rcp
          flt = hfx(i,j)/( rho(i,1,j)*cpm ) &
            & +xlvcp*vdfg(i,j)*(sqc(1)/exner(i,1,j)- sqcg/exnerg)
          flq = qfx(i,j)/ rho(i,1,j) &
            & -vdfg(i,j)*(sqc(1) - sqcg )
          flqv = qfx(i,j)/rho(i,1,j)
          flqc = -vdfg(i,j)*(sqc(1) - sqcg )
          th_sfc = ts(i,j)/ex1(1)
          zet = 0.5*dz(i,1,j)*rmol(i,j)
          if ( zet >= 0.0 ) then
          ...
          govrth = g/th1(1)
          sflux = hfx(i,j)/rho(i,1,j)/cpm + &
                  qfx(i,j)/rho(i,1,j)*ep_1*th1(1)
          bfx0 = max(sflux,0.)
          ...
          CALL mynn_tendencies(1,50, &
               ...
               &bl_mynn_edmf_mom, &
               &bl_mynn_mixscalars )
          CALL retrieve_exchange_coeffs(1,50,&
  ...
  SUBROUTINE GET_PBLH(KTS,KTE,zi,thetav1D,qke1D,zw1D,dz1D,landsea,kzi)
    ...
    DO k=1 +1,50 -1
       ...
       ENDIF
       IF (k .EQ. 50 -1) zi = zw1D(1 +1)
       IF (zi .NE. 0.0) exit
    ENDDO
    ktke = 1
    maxqke = MAX(Qke1D(1),0.)
    TKEeps = maxqke/40.
    ...
    DO k=1 +1,50 -1
       ...
       IF (qtke .LE. TKEeps) THEN
           ...
           PBLH_TKE = MAX(PBLH_TKE,zw1D(1 +1))
           kzi2 = MAX(k-1,1) + NINT((PBLH_TKE-zw1D(k-1))/dz1D(k-1))
       ENDIF
       IF (k .EQ. 50 -1) PBLH_TKE = zw1D(1 +1)
  ...
  IF(nup2 > 0) THEN
    ...
    ENDIF
    IF (s_aw(1 +1) /= 0.) THEN
       dzi(1) = 0.5*(DZ(1)+DZ(1 +1))
       flx1 = MAX(s_aw(1 +1)*(TH(1)-TH(1 +1))/dzi(1),1.0e-5)
    ELSE
       flx1 = 0.0
    ENDIF
    adjustment=1.0
    IF (flx1 > fluxportion*flt/dz(1) .AND. flx1>0.0) THEN
       adjustment= fluxportion*flt/dz(1)/flx1
