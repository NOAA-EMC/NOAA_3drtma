...
CONTAINS
SUBROUTINE find_w_extrema ( w, iup, jup, kup, idn, jdn, kdn, &
   ...
   INTEGER, INTENT(OUT) :: iup, jup, kup, idn, jdn, kdn
   ...
   j_start = jts
   j_end = MIN(jte,jde-1)
   iup = ims; jup = jms; kup = kms
   idn = ims; jdn = jms; kdn = kms
   wmax = w(iup,kup,jup)
   wmin = w(idn,kdn,jdn)
   DO j = j_start, j_end
   DO k = kts , kte
   DO i = i_start, i_end
      IF ( w(i,k,j) > wmax ) THEN
         iup = i
         jup = j
         kup = k
         wmax = w(i,k,j)
      END IF
      IF ( w(i,k,j) < wmin ) THEN
         idn = i
         jdn = j
         kdn = k
         wmin = w(i,k,j)
   ...
   DO j = j_start, j_end
     ...
     DO k = kts, ktf
       DO i = i_start, i_end
         ...
         IF( k .eq. ktf ) kp1 = ktf
         IF( k .eq. kts ) km1 = kts
         wiL = rom(i,k, j) * rdzw(k) / (c1(k)*mut(i,j)+c2(k))
         wiR = rom(i,k+1,j) * rdzw(k) / (c1(k)*mut(i,j)+c2(k))
         at(i,k) = - dt_rk*max(wiL,0.0)
         ct(i,k) = dt_rk*min(wiR,0.0)
         btmp(i) = dt_rk*(max(wiR,0.0) - min(wiL,0.0))
         bt(i,k) = 1.0 + btmp(i)
         rt(i,k) = dt_rk*tendency(i,k,j) &
                 - (c1(k)*mut(i,j)+c2(k))*(at(i,k)*s_old(i,km1,j) &
                           + btmp(i)*s_old(i,k,j) &
                           + ct(i,k)*s_old(i,kp1,j))
       ENDDO
     ENDDO
   ...
   DO j = j_start, j_end
     ...
     DO k = kts, ktf
       DO i = i_start, i_end
         ...
         IF( k .eq. ktf ) kp1 = ktf
         IF( k .eq. kts ) km1 = kts
         wiL = 0.5*(rom(i-1,k, j)+rom(i,k, j)) * rdzw(k) * msfuy(i,j) / (c1(k)*muu(i,j)+c2(k))
         wiR = 0.5*(rom(i-1,k+1,j)+rom(i,k+1,j)) * rdzw(k) * msfuy(i,j) / (c1(k)*muu(i,j)+c2(k))
         at(i,k) = - dt_rk*max(wiL,0.0)
         ct(i,k) = dt_rk*min(wiR,0.0)
         btmp(i) = dt_rk*(max(wiR,0.0) - min(wiL,0.0))
         bt(i,k) = 1.0 + btmp(i)
         rt(i,k) = dt_rk*tendency(i,k,j)*msfuy(i,j) &
                 - (c1(k)*muu(i,j)+c2(k)) * (at(i,k)*u_old(i,km1,j) &
                              +btmp(i)*u_old(i,k,j) &
                              +ct(i,k)*u_old(i,kp1,j))
       ENDDO
     ENDDO
     CALL tridiag2D(at, bt, ct, rt, bb, ims, ime, i_start, i_end, kts, kte, kts, ktf)
     DO k = kts, ktf
   ...
   DO j = j_start, j_end
     ...
     DO k = kts, ktf
       DO i = i_start, i_end
         ...
         IF( k .eq. ktf ) kp1 = ktf
         IF( k .eq. kts ) km1 = kts
         wiL = 0.5*(rom(i,k, j-1)+rom(i,k, j)) * rdzw(k) * msfvy(i,j) / (c1(k)*muv(i,j)+c2(k))
         wiR = 0.5*(rom(i,k+1,j-1)+rom(i,k+1,j)) * rdzw(k) * msfvy(i,j) / (c1(k)*muv(i,j)+c2(k))
         at(i,k) = - dt_rk*max(wiL,0.0)
         ct(i,k) = dt_rk*min(wiR,0.0)
         btmp(i) = dt_rk*(max(wiR,0.0) - min(wiL,0.0))
         bt(i,k) = 1.0 + btmp(i)
         rt(i,k) = dt_rk*tendency(i,k,j) * msfvx(i,j) &
                 - (c1(k)*muv(i,j)+c2(k)) * (at(i,k)*v_old(i,km1,j) &
                              +btmp(i)*v_old(i,k,j) &
                              +ct(i,k)*v_old(i,kp1,j))
       ENDDO
     ENDDO
     CALL tridiag2D(at, bt, ct, rt, bb, ims, ime, i_start, i_end, kts, kte, kts, ktf)
     DO k = kts, ktf
...
SUBROUTINE advect_w_implicit( w, w_old, tendency, &
   ...
   DO j = j_start, j_end
     ...
     DO k = kts+1, ktf
       DO i = i_start, i_end
         wiL = 0.5*(rom(i,k-1,j)+rom(i,k,j)) * rdzu(k) * msfty(i,j) / (c1(k)*mut(i,j)+c2(k))
         wiR = 0.5*(rom(i,k+1,j)+rom(i,k,j)) * rdzu(k) * msfty(i,j) / (c1(k)*mut(i,j)+c2(k))
         at(i,k) = - dt_rk*max(wiL,0.0)
         ct(i,k) = dt_rk*min(wiR,0.0)
         btmp(i) = dt_rk*(max(wiR,0.0) - min(wiL,0.0))
         bt(i,k) = 1.0 + btmp(i)
         rt(i,k) = dt_rk*tendency(i,k,j) * msfty(i,j) &
                 - (c1(k)*mut(i,j)+c2(k)) * (at(i,k)*w_old(i,k-1,j) &
                              +btmp(i)*w_old(i,k,j) &
                              +ct(i,k)*w_old(i,k+1,j))
        IF( k == kts+1 ) THEN
         dw = msfty(i,j)*.5*rdy*( &
                           (ht(i,j+1)-ht(i,j )) &
...
SUBROUTINE advect_scalar_pd ( field, field_old, tendency, &
   ...
   REAL :: q_im3, q_im2, q_im1, q_i, q_ip1, q_ip2, ua, vel, cr
    ...
    INTEGER :: kshift
    REAL :: crl, qtmp
    REAL(KIND=8), PARAMETER :: deps = 1.0d-15
      ...
      DO j = j_start, j_end
        ...
        DO k=kts+3,ktf-2
          DO i = i_start, i_end
            ...
            vel = rom(i,k,j)*sign(1,time_step)
            cr = vel*dt/dz/mu
            kshift = min(max(nint(floor(abs(cr)) * sign(1.0,cr)), -3), 3)
            IF( cr .gt. 1.0 ) THEN
              qtmp = SUM(field_old(i,k-kshift:k-1,j))
              fqzl(i,k,j) = vel * qtmp / max(deps,abs(cr))
            ELSEIF( cr .lt. -1.0 ) THEN
              qtmp = SUM(field_old(i,k:k-kshift-1,j))
              fqzl(i,k,j) = vel * qtmp / max(deps,abs(cr))
            ELSE
              fqzl(i,k,j) = max(vel,0.0)*field_old(i,k-1,j) + min(vel,0.0)*field_old(i,k,j)
            ENDIF
        ...
        DO i = i_start, i_end
          ...
          vel = rom(i,k,j)*sign(1,time_step)
          cr = vel*dt/dz/mu
          kshift = min(max(nint(floor(abs(cr)) * sign(1.0,cr)), -3), 2)
          IF( cr .gt. 1.0 ) THEN
            qtmp = SUM(field_old(i,k-kshift:k-1,j))
            fqzl(i,k,j) = vel * qtmp / max(deps,abs(cr))
          ELSEIF( cr .lt. -1.0 ) THEN
          ...
          vel = rom(i,k,j)*sign(1,time_step)
          cr = vel*dt/dz/mu
          kshift = min(max(nint(floor(abs(cr)) * sign(1.0,cr)), -2), 3)
          IF( cr .gt. 1.0 ) THEN
            qtmp = SUM(field_old(i,k-kshift:k-1,j))
            fqzl(i,k,j) = vel * qtmp / max(deps,abs(cr))
          ELSEIF( cr .lt. -1.0 ) THEN
      ...
      DO j = j_start, j_end
         ...
         DO k=kts+2,ktf-1
           DO i = i_start, i_end
             ...
             vel = rom(i,k,j)*sign(1,time_step)
             cr = vel*dt/dz/mu
            kshift = min(max(nint(floor(abs(cr)) * sign(1.0,cr)), -2), 2)
            IF( cr .gt. 1.0 ) THEN
              qtmp = SUM(field_old(i,k-kshift:k-1,j))
              fqzl(i,k,j) = vel * qtmp / max(deps,abs(cr))
            ELSEIF( cr .lt. -1.0 ) THEN
            ...
            ELSE
              fqzl(i,k,j) = max(vel,0.0)*field_old(i,k-1,j) + min(vel,0.0)*field_old(i,k,j)
            ENDIF
            fqz(i,k,j) = vel*flux4(field(i,k-2,j), field(i,k-1,j), &
                                   field(i,k ,j), field(i,k+1,j), -vel )
            fqz(i,k,j) = fqz(i,k,j) - fqzl(i,k,j)
          ENDDO
        ENDDO
...
SUBROUTINE advect_scalar_wenopd ( field, field_old, tendency, &
   ...
   REAL, PARAMETER :: eps=1.e-20
    ...
    real, parameter :: f50 = 37./60., f51 = 2./15., f52 = 1./60.
    real :: qim2, qim1, qi, qip1, qip2
    REAL(KIND=8) :: beta0, beta1, beta2, f0, f1, f2, wi0, wi1, wi2, sumwk
    REAL(KIND=8), parameter :: gi0 = 1.d0/10.d0, gi1 = 6.d0/10.d0, gi2 = 3.d0/10.d0, eps1=1.0d-28
    integer, parameter :: pw = 2
    INTEGER :: kshift
    REAL :: crl, qtmp
    REAL(KIND=8), PARAMETER :: deps = 1.0d-15
...
SUBROUTINE advect_scalar_mono ( field, field_old, tendency, &
  ...
  horizontal_order_test : IF( horz_order == 5 ) THEN
   ...
   ENDDO j_loop_y_flux_5
      ...
      ENDDO
   ELSE
     WRITE ( wrf_err_message , * ) 'MODULE_ADVECT: ADVECT_SCALAR_MONO, HORIZONTAL ORDER UNKNOWN:  ',horz_order
