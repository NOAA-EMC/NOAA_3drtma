...
IF(okay_to_alloc.AND.in_use_for_config(id,'emis_dust'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((model_config_rec%kfuture)-(1)+1))*(((em33)-(sm33)+1)*num_emis_dust)) * 4
  ALLOCATE(grid%emis_dust(sm31:em31,1:model_config_rec%kfuture,sm33:em33,num_emis_dust),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",138,&
    'frame/module_domain.f: Failed to allocate grid%emis_dust(sm31:em31,1:model_config_rec%kfuture,sm33:em33,num_emis_dust). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%emis_dust=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'emis_dust'
  grid%tail_statevars%DataName = 'EMIS_DUST'
  grid%tail_statevars%Description = 'Dust Emissions'
  ...
  grid%tail_statevars%rfield_4d => grid%emis_dust
  grid%tail_statevars%num_table => emis_dust_num_table
  grid%tail_statevars%index_table => emis_dust_index_table
  grid%tail_statevars%boundary_table => emis_dust_boundary_table
  grid%tail_statevars%dname_table => emis_dust_dname_table
  grid%tail_statevars%desc_table => emis_dust_desc_table
  grid%tail_statevars%units_table => emis_dust_units_table
  ...
  grid%tail_statevars%ed2 = config_flags%kfuture
  ...
  grid%tail_statevars%em2 = config_flags%kfuture
  ...
  grid%tail_statevars%ep2 = config_flags%kfuture
  ...
  grid%tail_statevars%dimname2 = 'klevs_for_dust'
  grid%tail_statevars%dimname3 = 'south_north'
  ENDIF
ELSE
  ALLOCATE(grid%emis_dust(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",195,&
    'frame/module_domain.f: Failed to allocate grid%emis_dust(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'emis_seas'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((model_config_rec%kfuture)-(1)+1))*(((em33)-(sm33)+1)*num_emis_seas)) * 4
  ALLOCATE(grid%emis_seas(sm31:em31,1:model_config_rec%kfuture,sm33:em33,num_emis_seas),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",204,&
    'frame/module_domain.f: Failed to allocate grid%emis_seas(sm31:em31,1:model_config_rec%kfuture,sm33:em33,num_emis_seas). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%emis_seas=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'emis_seas'
  grid%tail_statevars%DataName = 'EMIS_SEAS'
  grid%tail_statevars%Description = 'Sea-Salt Emissions'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%emis_seas
  grid%tail_statevars%num_table => emis_seas_num_table
  grid%tail_statevars%index_table => emis_seas_index_table
  grid%tail_statevars%boundary_table => emis_seas_boundary_table
  grid%tail_statevars%dname_table => emis_seas_dname_table
  grid%tail_statevars%desc_table => emis_seas_desc_table
  grid%tail_statevars%units_table => emis_seas_units_table
  grid%tail_statevars%streams_table => emis_seas_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%emis_seas(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",261,&
    'frame/module_domain.f: Failed to allocate grid%emis_seas(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'emis_seas2'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((model_config_rec%kfuture)-(1)+1))*(((em33)-(sm33)+1)*num_emis_seas2)) * 4
  ALLOCATE(grid%emis_seas2(sm31:em31,1:model_config_rec%kfuture,sm33:em33,num_emis_seas2),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",270,&
    'frame/module_domain.f: Failed to allocate grid%emis_seas2(sm31:em31,1:model_config_rec%kfuture,sm33:em33,num_emis_seas2). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%emis_seas2=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'emis_seas2'
  grid%tail_statevars%DataName = 'EMIS_SEAS2'
  grid%tail_statevars%Description = 'Sea-Salt Emissions'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%emis_seas2
  grid%tail_statevars%num_table => emis_seas2_num_table
  grid%tail_statevars%index_table => emis_seas2_index_table
  grid%tail_statevars%boundary_table => emis_seas2_boundary_table
  grid%tail_statevars%dname_table => emis_seas2_dname_table
  grid%tail_statevars%desc_table => emis_seas2_desc_table
  grid%tail_statevars%units_table => emis_seas2_units_table
  grid%tail_statevars%streams_table => emis_seas2_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%emis_seas2(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",327,&
    'frame/module_domain.f: Failed to allocate grid%emis_seas2(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'emis_vol'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_emis_vol)) * 4
  ALLOCATE(grid%emis_vol(sm31:em31,sm32:em32,sm33:em33,num_emis_vol),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",336,&
    'frame/module_domain.f: Failed to allocate grid%emis_vol(sm31:em31,sm32:em32,sm33:em33,num_emis_vol). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%emis_vol=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'emis_vol'
  grid%tail_statevars%DataName = 'EMIS_VOL'
  grid%tail_statevars%Description = 'Volcanic Emissions'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%emis_vol
  grid%tail_statevars%num_table => emis_vol_num_table
  grid%tail_statevars%index_table => emis_vol_index_table
  grid%tail_statevars%boundary_table => emis_vol_boundary_table
  grid%tail_statevars%dname_table => emis_vol_dname_table
  grid%tail_statevars%desc_table => emis_vol_desc_table
  grid%tail_statevars%units_table => emis_vol_units_table
  grid%tail_statevars%streams_table => emis_vol_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097280
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%emis_vol(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",393,&
    'frame/module_domain.f: Failed to allocate grid%emis_vol(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'ebu'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_ebu)) * 4
  ALLOCATE(grid%ebu(sm31:em31,sm32:em32,sm33:em33,num_ebu),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",402,&
    'frame/module_domain.f: Failed to allocate grid%ebu(sm31:em31,sm32:em32,sm33:em33,num_ebu). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%ebu=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'ebu'
  grid%tail_statevars%DataName = 'EBU'
  grid%tail_statevars%Description = 'Biomass burnung Emissions'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%ebu
  grid%tail_statevars%num_table => ebu_num_table
  grid%tail_statevars%index_table => ebu_index_table
  grid%tail_statevars%boundary_table => ebu_boundary_table
  grid%tail_statevars%dname_table => ebu_dname_table
  grid%tail_statevars%desc_table => ebu_desc_table
  grid%tail_statevars%units_table => ebu_units_table
  grid%tail_statevars%streams_table => ebu_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%ebu(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",459,&
    'frame/module_domain.f: Failed to allocate grid%ebu(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'ebu_in'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((model_config_rec%kfire)-(1)+1))*(((em33)-(sm33)+1)*num_ebu_in)) * 4
  ALLOCATE(grid%ebu_in(sm31:em31,1:model_config_rec%kfire,sm33:em33,num_ebu_in),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",468,&
    'frame/module_domain.f: Failed to allocate grid%ebu_in(sm31:em31,1:model_config_rec%kfire,sm33:em33,num_ebu_in). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%ebu_in=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'ebu_in'
  grid%tail_statevars%DataName = 'EBU_IN'
  grid%tail_statevars%Description = 'Biomass burnung input '
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%ebu_in
  grid%tail_statevars%num_table => ebu_in_num_table
  grid%tail_statevars%index_table => ebu_in_index_table
  grid%tail_statevars%boundary_table => ebu_in_boundary_table
  grid%tail_statevars%dname_table => ebu_in_dname_table
  grid%tail_statevars%desc_table => ebu_in_desc_table
  grid%tail_statevars%units_table => ebu_in_units_table
  grid%tail_statevars%streams_table => ebu_in_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  grid%tail_statevars%sd2 = 1
  grid%tail_statevars%ed2 = config_flags%kfire
  grid%tail_statevars%sd3 = jds
  grid%tail_statevars%ed3 = (jde-1)
  grid%tail_statevars%sm1 = ims
  grid%tail_statevars%em1 = ime
  grid%tail_statevars%sm2 = 1
  grid%tail_statevars%em2 = config_flags%kfire
  grid%tail_statevars%sm3 = jms
  grid%tail_statevars%em3 = jme
  grid%tail_statevars%sp1 = ips
  grid%tail_statevars%ep1 = MIN( (ide-1), ipe )
  grid%tail_statevars%sp2 = 1
  grid%tail_statevars%ep2 = config_flags%kfire
  grid%tail_statevars%sp3 = jps
  grid%tail_statevars%ep3 = MIN( (jde-1), jpe )
  grid%tail_statevars%subgrid_x = .FALSE.
  grid%tail_statevars%subgrid_y = .FALSE.
  grid%tail_statevars%dimname1 = 'west_east'
  grid%tail_statevars%dimname2 = 'klevs_for_fire'
  grid%tail_statevars%dimname3 = 'south_north'
  ENDIF
ELSE
  ALLOCATE(grid%ebu_in(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",525,&
    'frame/module_domain.f: Failed to allocate grid%ebu_in(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'mean_fct_agtf').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%mean_fct_agtf(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",534,&
    'frame/module_domain.f: Failed to allocate grid%mean_fct_agtf(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%mean_fct_agtf=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'mean_fct_agtf'
  grid%tail_statevars%DataName = 'MEAN_FCT_AGTF'
  grid%tail_statevars%Description = 'mean fraction of tropical forest'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%rfield_2d => grid%mean_fct_agtf
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%mean_fct_agtf(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",584,&
    'frame/module_domain.f: Failed to allocate grid%mean_fct_agtf(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'mean_fct_agef').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%mean_fct_agef(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",593,&
    'frame/module_domain.f: Failed to allocate grid%mean_fct_agef(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%mean_fct_agef=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'mean_fct_agef'
  grid%tail_statevars%DataName = 'MEAN_FCT_AGEF'
  grid%tail_statevars%Description = 'mean fraction of extra tropical forest'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%mean_fct_agef
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%mean_fct_agef(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",643,&
    'frame/module_domain.f: Failed to allocate grid%mean_fct_agef(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'mean_fct_agsv').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%mean_fct_agsv(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",652,&
    'frame/module_domain.f: Failed to allocate grid%mean_fct_agsv(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%mean_fct_agsv=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'mean_fct_agsv'
  grid%tail_statevars%DataName = 'MEAN_FCT_AGSV'
  grid%tail_statevars%Description = 'mean fraction of savanna'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%mean_fct_agsv
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%mean_fct_agsv(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",702,&
    'frame/module_domain.f: Failed to allocate grid%mean_fct_agsv(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'mean_fct_aggr').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%mean_fct_aggr(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",711,&
    'frame/module_domain.f: Failed to allocate grid%mean_fct_aggr(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%mean_fct_aggr=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'mean_fct_aggr'
  grid%tail_statevars%DataName = 'MEAN_FCT_AGGR'
  grid%tail_statevars%Description = 'mean fraction of grassland'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%mean_fct_aggr
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%mean_fct_aggr(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",761,&
    'frame/module_domain.f: Failed to allocate grid%mean_fct_aggr(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'firesize_agtf').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%firesize_agtf(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",770,&
    'frame/module_domain.f: Failed to allocate grid%firesize_agtf(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%firesize_agtf=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'firesize_agtf'
  grid%tail_statevars%DataName = 'FIRESIZE_AGTF'
  grid%tail_statevars%Description = 'mean firesize for tropical forest'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%firesize_agtf
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%firesize_agtf(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",820,&
    'frame/module_domain.f: Failed to allocate grid%firesize_agtf(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'firesize_agef').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%firesize_agef(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",829,&
    'frame/module_domain.f: Failed to allocate grid%firesize_agef(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%firesize_agef=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'firesize_agef'
  grid%tail_statevars%DataName = 'FIRESIZE_AGEF'
  grid%tail_statevars%Description = 'mean firesize for extratropical forest'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%firesize_agef
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%firesize_agef(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",879,&
    'frame/module_domain.f: Failed to allocate grid%firesize_agef(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'firesize_agsv').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%firesize_agsv(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",888,&
    'frame/module_domain.f: Failed to allocate grid%firesize_agsv(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%firesize_agsv=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'firesize_agsv'
  grid%tail_statevars%DataName = 'FIRESIZE_AGSV'
  grid%tail_statevars%Description = 'mean firesize for savanna'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%firesize_agsv
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%firesize_agsv(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",938,&
    'frame/module_domain.f: Failed to allocate grid%firesize_agsv(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'firesize_aggr').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%firesize_aggr(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",947,&
    'frame/module_domain.f: Failed to allocate grid%firesize_aggr(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%firesize_aggr=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'firesize_aggr'
  grid%tail_statevars%DataName = 'FIRESIZE_AGGR'
  grid%tail_statevars%Description = 'mean firesize for grassland'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%firesize_aggr
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%firesize_aggr(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",997,&
    'frame/module_domain.f: Failed to allocate grid%firesize_aggr(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'mean_frp').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%mean_frp(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1006,&
    'frame/module_domain.f: Failed to allocate grid%mean_frp(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%mean_frp=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'mean_frp'
  grid%tail_statevars%DataName = 'MEAN_FRP'
  grid%tail_statevars%Description = 'mean fire radiative power'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%mean_frp
  ...
  ENDIF
ELSE
  ALLOCATE(grid%mean_frp(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1056,&
    'frame/module_domain.f: Failed to allocate grid%mean_frp(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'std_frp').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%std_frp(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1065,&
    'frame/module_domain.f: Failed to allocate grid%std_frp(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%std_frp=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'std_frp'
  grid%tail_statevars%DataName = 'STD_FRP'
  grid%tail_statevars%Description = 'standard deviation of fire radiative power'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%std_frp
  grid%tail_statevars%streams(1) = 33554433
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%std_frp(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1115,&
    'frame/module_domain.f: Failed to allocate grid%std_frp(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'mean_fsize').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%mean_fsize(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1124,&
    'frame/module_domain.f: Failed to allocate grid%mean_fsize(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%mean_fsize=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'mean_fsize'
  grid%tail_statevars%DataName = 'MEAN_FSIZE'
  grid%tail_statevars%Description = 'mean fire size'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%mean_fsize
  grid%tail_statevars%streams(1) = 33554433
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%mean_fsize(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1174,&
    'frame/module_domain.f: Failed to allocate grid%mean_fsize(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'std_fsize').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%std_fsize(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1183,&
    'frame/module_domain.f: Failed to allocate grid%std_fsize(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%std_fsize=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'std_fsize'
  grid%tail_statevars%DataName = 'STD_FSIZE'
  grid%tail_statevars%Description = 'standard deviation of fire size'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%std_fsize
  grid%tail_statevars%streams(1) = 33554433
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%std_fsize(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1233,&
    'frame/module_domain.f: Failed to allocate grid%std_fsize(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'ebb_smoke').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%ebb_smoke(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1242,&
    'frame/module_domain.f: Failed to allocate grid%ebb_smoke(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%ebb_smoke=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'ebb_smoke'
  grid%tail_statevars%DataName = 'EBB_SMOKE'
  grid%tail_statevars%Description = '2D BB emissions of primary PM2.5'
  grid%tail_statevars%Units = 'ug/m2/s'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  grid%tail_statevars%Stagger = ''
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%ebb_smoke
  grid%tail_statevars%streams(1) = 33554433
  grid%tail_statevars%streams(2) = 2
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%ebb_smoke(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1292,&
    'frame/module_domain.f: Failed to allocate grid%ebb_smoke(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'flam_frac').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%flam_frac(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1301,&
    'frame/module_domain.f: Failed to allocate grid%flam_frac(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%flam_frac=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'flam_frac'
  grid%tail_statevars%DataName = 'FLAM_FRAC'
  grid%tail_statevars%Description = 'fraction of flammable ebb_smoke'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%flam_frac
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%flam_frac(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1351,&
    'frame/module_domain.f: Failed to allocate grid%flam_frac(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'peak_hr').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%peak_hr(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1360,&
    'frame/module_domain.f: Failed to allocate grid%peak_hr(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%peak_hr=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'peak_hr'
  grid%tail_statevars%DataName = 'PEAK_HR'
  grid%tail_statevars%Description = 'peak hour of fire'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%peak_hr
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%peak_hr(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1410,&
    'frame/module_domain.f: Failed to allocate grid%peak_hr(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'lu_fire1').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%lu_fire1(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1419,&
    'frame/module_domain.f: Failed to allocate grid%lu_fire1(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%lu_fire1=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'lu_fire1'
  grid%tail_statevars%DataName = 'LU_FIRE1'
  grid%tail_statevars%Description = 'LU fraction array 1 - used for smoke'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%lu_fire1
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%lu_fire1(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1469,&
    'frame/module_domain.f: Failed to allocate grid%lu_fire1(1,1).  ')
  endif
ENDIF
  IF (.NOT.grid%is_intermediate) THEN
   ALLOCATE( grid%tail_statevars%next )
   ...
   NULLIFY( grid%tail_statevars%next )
   grid%tail_statevars%ProcOrient = '  '
   grid%tail_statevars%VarName = 'aircraftemissframes'
   grid%tail_statevars%DataName = 'AIRCRAFTEMISSFRAMES'
   ...
   grid%tail_statevars%scalar_array = .FALSE.
   grid%tail_statevars%ifield_0d => grid%aircraftemissframes
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  ENDIF
IF ( setinitval .EQ. 3 ) grid%aircraftemissframes=0
IF(okay_to_alloc.AND.in_use_for_config(id,'emis_aircraft'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((model_config_rec%kemit_aircraft)-(1)+1))*(((em33)-(sm33)+1)*num_emis_aircraft)) * 4
  ALLOCATE(grid%emis_aircraft(sm31:em31,1:model_config_rec%kemit_aircraft,sm33:em33,num_emis_aircraft),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1497,&
    'frame/module_domain.f: Failed to allocate grid%emis_aircraft(sm31:em31,1:model_config_rec%kemit_aircraft,sm33:em33,num_emis_aircraft). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%emis_aircraft=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'emis_aircraft'
  grid%tail_statevars%DataName = 'EMIS_AIRCRAFT'
  grid%tail_statevars%Description = 'Aircraft Emissions'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%emis_aircraft
  grid%tail_statevars%num_table => emis_aircraft_num_table
  grid%tail_statevars%index_table => emis_aircraft_index_table
  grid%tail_statevars%boundary_table => emis_aircraft_boundary_table
  grid%tail_statevars%dname_table => emis_aircraft_dname_table
  grid%tail_statevars%desc_table => emis_aircraft_desc_table
  grid%tail_statevars%units_table => emis_aircraft_units_table
  grid%tail_statevars%streams_table => emis_aircraft_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 256
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  grid%tail_statevars%sd2 = 1
  grid%tail_statevars%ed2 = config_flags%kemit_aircraft
  grid%tail_statevars%sd3 = jds
  grid%tail_statevars%ed3 = (jde-1)
  grid%tail_statevars%sm1 = ims
  grid%tail_statevars%em1 = ime
  grid%tail_statevars%sm2 = 1
  grid%tail_statevars%em2 = config_flags%kemit_aircraft
  grid%tail_statevars%sm3 = jms
  grid%tail_statevars%em3 = jme
  grid%tail_statevars%sp1 = ips
  grid%tail_statevars%ep1 = MIN( (ide-1), ipe )
  grid%tail_statevars%sp2 = 1
  grid%tail_statevars%ep2 = config_flags%kemit_aircraft
  grid%tail_statevars%sp3 = jps
  grid%tail_statevars%ep3 = MIN( (jde-1), jpe )
  grid%tail_statevars%subgrid_x = .FALSE.
  grid%tail_statevars%subgrid_y = .FALSE.
  grid%tail_statevars%dimname1 = 'west_east'
  grid%tail_statevars%dimname2 = 'ac_emissions_zdim'
  grid%tail_statevars%dimname3 = 'south_north'
  ENDIF
ELSE
  ALLOCATE(grid%emis_aircraft(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1554,&
    'frame/module_domain.f: Failed to allocate grid%emis_aircraft(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'ext_coef'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_ext_coef)) * 4
  ALLOCATE(grid%ext_coef(sm31:em31,sm32:em32,sm33:em33,num_ext_coef),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1563,&
    'frame/module_domain.f: Failed to allocate grid%ext_coef(sm31:em31,sm32:em32,sm33:em33,num_ext_coef). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%ext_coef=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'ext_coef'
  grid%tail_statevars%DataName = 'EXT_COEF'
  grid%tail_statevars%Description = 'Extinction coefficients'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%ext_coef
  grid%tail_statevars%num_table => ext_coef_num_table
  grid%tail_statevars%index_table => ext_coef_index_table
  grid%tail_statevars%boundary_table => ext_coef_boundary_table
  grid%tail_statevars%dname_table => ext_coef_dname_table
  grid%tail_statevars%desc_table => ext_coef_desc_table
  grid%tail_statevars%units_table => ext_coef_units_table
  grid%tail_statevars%streams_table => ext_coef_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%ext_coef(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1620,&
    'frame/module_domain.f: Failed to allocate grid%ext_coef(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'bscat_coef'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_bscat_coef)) * 4
  ALLOCATE(grid%bscat_coef(sm31:em31,sm32:em32,sm33:em33,num_bscat_coef),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1629,&
    'frame/module_domain.f: Failed to allocate grid%bscat_coef(sm31:em31,sm32:em32,sm33:em33,num_bscat_coef). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%bscat_coef=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'bscat_coef'
  grid%tail_statevars%DataName = 'BSCAT_COEF'
  grid%tail_statevars%Description = 'Scatter coefficients'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%bscat_coef
  grid%tail_statevars%num_table => bscat_coef_num_table
  grid%tail_statevars%index_table => bscat_coef_index_table
  grid%tail_statevars%boundary_table => bscat_coef_boundary_table
  grid%tail_statevars%dname_table => bscat_coef_dname_table
  grid%tail_statevars%desc_table => bscat_coef_desc_table
  grid%tail_statevars%units_table => bscat_coef_units_table
  grid%tail_statevars%streams_table => bscat_coef_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%bscat_coef(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1686,&
    'frame/module_domain.f: Failed to allocate grid%bscat_coef(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'asym_par'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_asym_par)) * 4
  ALLOCATE(grid%asym_par(sm31:em31,sm32:em32,sm33:em33,num_asym_par),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1695,&
    'frame/module_domain.f: Failed to allocate grid%asym_par(sm31:em31,sm32:em32,sm33:em33,num_asym_par). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%asym_par=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'asym_par'
  grid%tail_statevars%DataName = 'ASYM_PAR'
  grid%tail_statevars%Description = 'assymetry parameter'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%asym_par
  grid%tail_statevars%num_table => asym_par_num_table
  grid%tail_statevars%index_table => asym_par_index_table
  grid%tail_statevars%boundary_table => asym_par_boundary_table
  grid%tail_statevars%dname_table => asym_par_dname_table
  grid%tail_statevars%desc_table => asym_par_desc_table
  grid%tail_statevars%units_table => asym_par_units_table
  grid%tail_statevars%streams_table => asym_par_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%asym_par(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1752,&
    'frame/module_domain.f: Failed to allocate grid%asym_par(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'conv_ct'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_conv_ct)) * 4
  ALLOCATE(grid%conv_ct(sm31:em31,sm32:em32,sm33:em33,num_conv_ct),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1761,&
    'frame/module_domain.f: Failed to allocate grid%conv_ct(sm31:em31,sm32:em32,sm33:em33,num_conv_ct). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%conv_ct=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'conv_ct'
  grid%tail_statevars%DataName = 'CONV_CT'
  grid%tail_statevars%Description = '-'
  grid%tail_statevars%Units = '-'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%conv_ct
  grid%tail_statevars%num_table => conv_ct_num_table
  grid%tail_statevars%index_table => conv_ct_index_table
  grid%tail_statevars%boundary_table => conv_ct_boundary_table
  grid%tail_statevars%dname_table => conv_ct_dname_table
  grid%tail_statevars%desc_table => conv_ct_desc_table
  grid%tail_statevars%units_table => conv_ct_units_table
  grid%tail_statevars%streams_table => conv_ct_streams_table
  grid%tail_statevars%streams(1) = 0
  ...
  ENDIF
ELSE
  ALLOCATE(grid%conv_ct(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1818,&
    'frame/module_domain.f: Failed to allocate grid%conv_ct(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'chem_ct'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_chem_ct)) * 4
  ALLOCATE(grid%chem_ct(sm31:em31,sm32:em32,sm33:em33,num_chem_ct),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1827,&
    'frame/module_domain.f: Failed to allocate grid%chem_ct(sm31:em31,sm32:em32,sm33:em33,num_chem_ct). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_ct=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'chem_ct'
  grid%tail_statevars%DataName = 'CHEM_CT'
  grid%tail_statevars%Description = '-'
  grid%tail_statevars%Units = '-'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%chem_ct
  grid%tail_statevars%num_table => chem_ct_num_table
  grid%tail_statevars%index_table => chem_ct_index_table
  grid%tail_statevars%boundary_table => chem_ct_boundary_table
  grid%tail_statevars%dname_table => chem_ct_dname_table
  grid%tail_statevars%desc_table => chem_ct_desc_table
  grid%tail_statevars%units_table => chem_ct_units_table
  grid%tail_statevars%streams_table => chem_ct_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%chem_ct(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1884,&
    'frame/module_domain.f: Failed to allocate grid%chem_ct(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'vmix_ct'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_vmix_ct)) * 4
  ALLOCATE(grid%vmix_ct(sm31:em31,sm32:em32,sm33:em33,num_vmix_ct),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1893,&
    'frame/module_domain.f: Failed to allocate grid%vmix_ct(sm31:em31,sm32:em32,sm33:em33,num_vmix_ct). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%vmix_ct=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'vmix_ct'
  grid%tail_statevars%DataName = 'VMIX_CT'
  grid%tail_statevars%Description = '-'
  grid%tail_statevars%Units = '-'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%vmix_ct
  grid%tail_statevars%num_table => vmix_ct_num_table
  grid%tail_statevars%index_table => vmix_ct_index_table
  grid%tail_statevars%boundary_table => vmix_ct_boundary_table
  grid%tail_statevars%dname_table => vmix_ct_dname_table
  grid%tail_statevars%desc_table => vmix_ct_desc_table
  grid%tail_statevars%units_table => vmix_ct_units_table
  grid%tail_statevars%streams_table => vmix_ct_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%vmix_ct(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1950,&
    'frame/module_domain.f: Failed to allocate grid%vmix_ct(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'advh_ct'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_advh_ct)) * 4
  ALLOCATE(grid%advh_ct(sm31:em31,sm32:em32,sm33:em33,num_advh_ct),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",1959,&
    'frame/module_domain.f: Failed to allocate grid%advh_ct(sm31:em31,sm32:em32,sm33:em33,num_advh_ct). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%advh_ct=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'advh_ct'
  grid%tail_statevars%DataName = 'ADVH_CT'
  grid%tail_statevars%Description = '-'
  grid%tail_statevars%Units = '-'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%advh_ct
  grid%tail_statevars%num_table => advh_ct_num_table
  grid%tail_statevars%index_table => advh_ct_index_table
  grid%tail_statevars%boundary_table => advh_ct_boundary_table
  grid%tail_statevars%dname_table => advh_ct_dname_table
  grid%tail_statevars%desc_table => advh_ct_desc_table
  grid%tail_statevars%units_table => advh_ct_units_table
  grid%tail_statevars%streams_table => advh_ct_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%advh_ct(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2016,&
    'frame/module_domain.f: Failed to allocate grid%advh_ct(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'advz_ct'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_advz_ct)) * 4
  ALLOCATE(grid%advz_ct(sm31:em31,sm32:em32,sm33:em33,num_advz_ct),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2025,&
    'frame/module_domain.f: Failed to allocate grid%advz_ct(sm31:em31,sm32:em32,sm33:em33,num_advz_ct). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%advz_ct=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'advz_ct'
  grid%tail_statevars%DataName = 'ADVZ_CT'
  grid%tail_statevars%Description = '-'
  grid%tail_statevars%Units = '-'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%advz_ct
  grid%tail_statevars%num_table => advz_ct_num_table
  grid%tail_statevars%index_table => advz_ct_index_table
  grid%tail_statevars%boundary_table => advz_ct_boundary_table
  grid%tail_statevars%dname_table => advz_ct_dname_table
  grid%tail_statevars%desc_table => advz_ct_desc_table
  grid%tail_statevars%units_table => advz_ct_units_table
  grid%tail_statevars%streams_table => advz_ct_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%advz_ct(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2082,&
    'frame/module_domain.f: Failed to allocate grid%advz_ct(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'dvel'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((model_config_rec%kdvel)-(1)+1))*(((em33)-(sm33)+1)*num_dvel)) * 4
  ALLOCATE(grid%dvel(sm31:em31,1:model_config_rec%kdvel,sm33:em33,num_dvel),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2091,&
    'frame/module_domain.f: Failed to allocate grid%dvel(sm31:em31,1:model_config_rec%kdvel,sm33:em33,num_dvel). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%dvel=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'dvel'
  grid%tail_statevars%DataName = 'DVEL'
  grid%tail_statevars%Description = 'deposition velocities'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%dvel
  grid%tail_statevars%num_table => dvel_num_table
  grid%tail_statevars%index_table => dvel_index_table
  grid%tail_statevars%boundary_table => dvel_boundary_table
  grid%tail_statevars%dname_table => dvel_dname_table
  grid%tail_statevars%desc_table => dvel_desc_table
  grid%tail_statevars%units_table => dvel_units_table
  grid%tail_statevars%streams_table => dvel_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  grid%tail_statevars%sd2 = 1
  grid%tail_statevars%ed2 = config_flags%kdvel
  grid%tail_statevars%sd3 = jds
  grid%tail_statevars%ed3 = (jde-1)
  grid%tail_statevars%sm1 = ims
  grid%tail_statevars%em1 = ime
  grid%tail_statevars%sm2 = 1
  grid%tail_statevars%em2 = config_flags%kdvel
  grid%tail_statevars%sm3 = jms
  grid%tail_statevars%em3 = jme
  grid%tail_statevars%sp1 = ips
  grid%tail_statevars%ep1 = MIN( (ide-1), ipe )
  grid%tail_statevars%sp2 = 1
  grid%tail_statevars%ep2 = config_flags%kdvel
  grid%tail_statevars%sp3 = jps
  grid%tail_statevars%ep3 = MIN( (jde-1), jpe )
  grid%tail_statevars%subgrid_x = .FALSE.
  grid%tail_statevars%subgrid_y = .FALSE.
  grid%tail_statevars%dimname1 = 'west_east'
  grid%tail_statevars%dimname2 = 'klevs_for_dvel'
  grid%tail_statevars%dimname3 = 'south_north'
  ENDIF
ELSE
  ALLOCATE(grid%dvel(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2148,&
    'frame/module_domain.f: Failed to allocate grid%dvel(1,1,1,1).  ')
  endif
ENDIF
  IF (.NOT.grid%is_intermediate) THEN
   ALLOCATE( grid%tail_statevars%next )
   ...
   NULLIFY( grid%tail_statevars%next )
   grid%tail_statevars%ProcOrient = '  '
   grid%tail_statevars%VarName = 'num_vert_mix'
   grid%tail_statevars%DataName = 'NUM_VERT_MIX'
   grid%tail_statevars%Description = 'Number of chemical species used in deposition; num_gas or num_chem'
   grid%tail_statevars%Units = ''
   grid%tail_statevars%Type = 'i'
   grid%tail_statevars%Ntl = 0
   grid%tail_statevars%Restart = .TRUE.
   grid%tail_statevars%Ndim = 0
   grid%tail_statevars%scalar_array = .FALSE.
   grid%tail_statevars%ifield_0d => grid%num_vert_mix
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  ENDIF
IF ( setinitval .EQ. 3 ) grid%num_vert_mix=0
IF(okay_to_alloc.AND.in_use_for_config(id,'aerwrf').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%aerwrf(sm31:em31,sm32:em32,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2176,&
    'frame/module_domain.f: Failed to allocate grid%aerwrf(sm31:em31,sm32:em32,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%aerwrf=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'aerwrf'
  grid%tail_statevars%DataName = 'AERWRF'
  grid%tail_statevars%Description = 'STANDARD AEROSOL PROFILE'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_3d => grid%aerwrf
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%aerwrf(1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2226,&
    'frame/module_domain.f: Failed to allocate grid%aerwrf(1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'pm2_5_dry').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%pm2_5_dry(sm31:em31,sm32:em32,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2235,&
    'frame/module_domain.f: Failed to allocate grid%pm2_5_dry(sm31:em31,sm32:em32,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%pm2_5_dry=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'pm2_5_dry'
  grid%tail_statevars%DataName = 'PM2_5_DRY'
  grid%tail_statevars%Description = 'pm2.5 aerosol dry mass'
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_3d => grid%pm2_5_dry
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%pm2_5_dry(1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2285,&
    'frame/module_domain.f: Failed to allocate grid%pm2_5_dry(1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'pm2_5_dry_ec').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%pm2_5_dry_ec(sm31:em31,sm32:em32,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2294,&
    'frame/module_domain.f: Failed to allocate grid%pm2_5_dry_ec(sm31:em31,sm32:em32,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%pm2_5_dry_ec=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'pm2_5_dry_ec'
  grid%tail_statevars%DataName = 'PM2_5_EC_DRY'
  grid%tail_statevars%Description = 'dry ec aerosol mass'
  grid%tail_statevars%Units = 'ug m^-3'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_3d => grid%pm2_5_dry_ec
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%pm2_5_dry_ec(1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2344,&
    'frame/module_domain.f: Failed to allocate grid%pm2_5_dry_ec(1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'pm2_5_water').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%pm2_5_water(sm31:em31,sm32:em32,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2353,&
    'frame/module_domain.f: Failed to allocate grid%pm2_5_water(sm31:em31,sm32:em32,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%pm2_5_water=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'pm2_5_water'
  grid%tail_statevars%DataName = 'PM2_5_WATER'
  grid%tail_statevars%Description = 'pm2.5 aerosol liquid water content'
  grid%tail_statevars%Units = 'ug m^-3'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_3d => grid%pm2_5_water
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%pm2_5_water(1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2403,&
    'frame/module_domain.f: Failed to allocate grid%pm2_5_water(1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'pm10').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%pm10(sm31:em31,sm32:em32,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2412,&
    'frame/module_domain.f: Failed to allocate grid%pm10(sm31:em31,sm32:em32,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%pm10=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'pm10'
  grid%tail_statevars%DataName = 'PM10 '
  grid%tail_statevars%Description = 'pm10 dry mass'
  grid%tail_statevars%Units = 'ug m^-3'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_3d => grid%pm10
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%pm10(1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2462,&
    'frame/module_domain.f: Failed to allocate grid%pm10(1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'uvrad').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%uvrad(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2471,&
    'frame/module_domain.f: Failed to allocate grid%uvrad(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%uvrad=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'uvrad'
  grid%tail_statevars%DataName = 'UV_RAD '
  grid%tail_statevars%Description = 'uvb net-radiation'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%uvrad
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%uvrad(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2521,&
    'frame/module_domain.f: Failed to allocate grid%uvrad(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'tcosz').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%tcosz(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2530,&
    'frame/module_domain.f: Failed to allocate grid%tcosz(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%tcosz=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'tcosz'
  grid%tail_statevars%DataName = 'TCOSZ '
  grid%tail_statevars%Description = 'daily average cossza'
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%tcosz
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%tcosz(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2580,&
    'frame/module_domain.f: Failed to allocate grid%tcosz(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'ttday').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%ttday(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2589,&
    'frame/module_domain.f: Failed to allocate grid%ttday(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%ttday=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'ttday'
  grid%tail_statevars%DataName = 'TTDAY '
  grid%tail_statevars%Description = ''
  grid%tail_statevars%Units = '?'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%ttday
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%ttday(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2639,&
    'frame/module_domain.f: Failed to allocate grid%ttday(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'tsoa').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%tsoa(sm31:em31,sm32:em32,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2648,&
    'frame/module_domain.f: Failed to allocate grid%tsoa(sm31:em31,sm32:em32,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%tsoa=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'tsoa'
  grid%tail_statevars%DataName = 'TSOA'
  grid%tail_statevars%Description = 'soa dry mass'
  grid%tail_statevars%Units = 'ug m^-3'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_3d => grid%tsoa
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%tsoa(1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2698,&
    'frame/module_domain.f: Failed to allocate grid%tsoa(1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'bsoa').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%bsoa(sm31:em31,sm32:em32,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2707,&
    'frame/module_domain.f: Failed to allocate grid%bsoa(sm31:em31,sm32:em32,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%bsoa=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'bsoa'
  grid%tail_statevars%DataName = 'BSOA'
  grid%tail_statevars%Description = 'bsoa dry mass'
  grid%tail_statevars%Units = 'ug m^-3'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_3d => grid%bsoa
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%bsoa(1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2757,&
    'frame/module_domain.f: Failed to allocate grid%bsoa(1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'asoa').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%asoa(sm31:em31,sm32:em32,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2766,&
    'frame/module_domain.f: Failed to allocate grid%asoa(sm31:em31,sm32:em32,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%asoa=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'asoa'
  grid%tail_statevars%DataName = 'ASOA'
  grid%tail_statevars%Description = 'asoa dry mass'
  grid%tail_statevars%Units = 'ug m^-3'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_3d => grid%asoa
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  ENDIF
ELSE
  ALLOCATE(grid%asoa(1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2816,&
    'frame/module_domain.f: Failed to allocate grid%asoa(1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'dms_0').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%dms_0(sm31:em31,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2825,&
    'frame/module_domain.f: Failed to allocate grid%dms_0(sm31:em31,sm33:em33). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%dms_0=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'dms_0'
  grid%tail_statevars%DataName = 'DMS_0 '
  grid%tail_statevars%Description = 'dms oceanic concentrations'
  grid%tail_statevars%Units = 'nM/L'
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XY'
  grid%tail_statevars%Stagger = ''
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_2d => grid%dms_0
  grid%tail_statevars%streams(1) = 0
  ...
  ENDIF
ELSE
  ALLOCATE(grid%dms_0(1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2875,&
    'frame/module_domain.f: Failed to allocate grid%dms_0(1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'vprm_in'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((8)-(1)+1))*(((em33)-(sm33)+1)*num_vprm_in)) * 4
  ALLOCATE(grid%vprm_in(sm31:em31,1:8,sm33:em33,num_vprm_in),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2884,&
    'frame/module_domain.f: Failed to allocate grid%vprm_in(sm31:em31,1:8,sm33:em33,num_vprm_in). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%vprm_in=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'vprm_in'
  grid%tail_statevars%DataName = 'VPRM_IN'
  grid%tail_statevars%Description = 'VPRM input fields'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%vprm_in
  grid%tail_statevars%num_table => vprm_in_num_table
  grid%tail_statevars%index_table => vprm_in_index_table
  grid%tail_statevars%boundary_table => vprm_in_boundary_table
  grid%tail_statevars%dname_table => vprm_in_dname_table
  grid%tail_statevars%desc_table => vprm_in_desc_table
  grid%tail_statevars%units_table => vprm_in_units_table
  grid%tail_statevars%streams_table => vprm_in_streams_table
  grid%tail_statevars%streams(1) = 0
  ...
  grid%tail_statevars%subgrid_y = .FALSE.
  grid%tail_statevars%dimname1 = 'west_east'
  grid%tail_statevars%dimname2 = 'vprm_vgcls'
  grid%tail_statevars%dimname3 = 'south_north'
  ENDIF
ELSE
  ALLOCATE(grid%vprm_in(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2941,&
    'frame/module_domain.f: Failed to allocate grid%vprm_in(1,1,1,1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'rad_vprm').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((8)-(1)+1))) * 4
  ALLOCATE(grid%rad_vprm(1:8),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2950,&
    'frame/module_domain.f: Failed to allocate grid%rad_vprm(1:8). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%rad_vprm=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'rad_vprm'
  grid%tail_statevars%DataName = 'RAD_VPRM'
  grid%tail_statevars%Description = ' '
  grid%tail_statevars%Units = ' '
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_1d => grid%rad_vprm
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = 1
  grid%tail_statevars%ed1 = 8
  grid%tail_statevars%sd2 = 1
  grid%tail_statevars%ed2 = 1
  grid%tail_statevars%sd3 = 1
  grid%tail_statevars%ed3 = 1
  grid%tail_statevars%sm1 = 1
  grid%tail_statevars%em1 = 8
  grid%tail_statevars%sm2 = 1
  grid%tail_statevars%em2 = 1
  grid%tail_statevars%sm3 = 1
  grid%tail_statevars%em3 = 1
  grid%tail_statevars%sp1 = 1
  grid%tail_statevars%ep1 = 8
  grid%tail_statevars%sp2 = 1
  grid%tail_statevars%ep2 = 1
  grid%tail_statevars%sp3 = 1
  grid%tail_statevars%ep3 = 1
  grid%tail_statevars%dimname1 = 'vprm_vgcls'
  grid%tail_statevars%dimname2 = ''
  grid%tail_statevars%dimname3 = ''
  ENDIF
ELSE
  ALLOCATE(grid%rad_vprm(1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",2998,&
    'frame/module_domain.f: Failed to allocate grid%rad_vprm(1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'lambda_vprm').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((8)-(1)+1))) * 4
  ALLOCATE(grid%lambda_vprm(1:8),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3007,&
    'frame/module_domain.f: Failed to allocate grid%lambda_vprm(1:8). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%lambda_vprm=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'lambda_vprm'
  grid%tail_statevars%DataName = 'LAMBDA_VPRM'
  grid%tail_statevars%Description = ' '
  grid%tail_statevars%Units = ' '
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_1d => grid%lambda_vprm
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = 1
  grid%tail_statevars%ed1 = 8
  ...
  ENDIF
ELSE
  ALLOCATE(grid%lambda_vprm(1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3055,&
    'frame/module_domain.f: Failed to allocate grid%lambda_vprm(1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'alpha_vprm').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((8)-(1)+1))) * 4
  ALLOCATE(grid%alpha_vprm(1:8),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3064,&
    'frame/module_domain.f: Failed to allocate grid%alpha_vprm(1:8). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%alpha_vprm=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'alpha_vprm'
  grid%tail_statevars%DataName = 'ALPHA_VPRM'
  grid%tail_statevars%Description = ' '
  grid%tail_statevars%Units = ' '
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_1d => grid%alpha_vprm
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = 1
  grid%tail_statevars%ed1 = 8
  ...
  ENDIF
ELSE
  ALLOCATE(grid%alpha_vprm(1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3112,&
    'frame/module_domain.f: Failed to allocate grid%alpha_vprm(1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'resp_vprm').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((8)-(1)+1))) * 4
  ALLOCATE(grid%resp_vprm(1:8),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3121,&
    'frame/module_domain.f: Failed to allocate grid%resp_vprm(1:8). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%resp_vprm=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'resp_vprm'
  grid%tail_statevars%DataName = 'RESP_VPRM'
  grid%tail_statevars%Description = ' '
  grid%tail_statevars%Units = ' '
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  ...
  grid%tail_statevars%Restart = .FALSE.
  grid%tail_statevars%scalar_array = .FALSE.
  grid%tail_statevars%rfield_1d => grid%resp_vprm
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = 1
  grid%tail_statevars%ed1 = 8
  ...
  ENDIF
ELSE
  ALLOCATE(grid%resp_vprm(1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3169,&
    'frame/module_domain.f: Failed to allocate grid%resp_vprm(1).  ')
  endif
ENDIF
IF(okay_to_alloc.AND.in_use_for_config(id,'wet_in'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((1)-(1)+1))*(((em33)-(sm33)+1)*num_wet_in)) * 4
  ALLOCATE(grid%wet_in(sm31:em31,1:1,sm33:em33,num_wet_in),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3178,&
    'frame/module_domain.f: Failed to allocate grid%wet_in(sm31:em31,1:1,sm33:em33,num_wet_in). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%wet_in=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
  ALLOCATE( grid%tail_statevars%next )
  grid%tail_statevars => grid%tail_statevars%next
  NULLIFY( grid%tail_statevars%next )
  grid%tail_statevars%VarName = 'wet_in'
  grid%tail_statevars%DataName = 'WET_IN'
  grid%tail_statevars%Description = 'Wetland input fields'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'r'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'XZY'
  ...
  grid%tail_statevars%Restart = .TRUE.
  grid%tail_statevars%scalar_array = .TRUE.
  grid%tail_statevars%rfield_4d => grid%wet_in
  grid%tail_statevars%num_table => wet_in_num_table
  grid%tail_statevars%index_table => wet_in_index_table
  grid%tail_statevars%boundary_table => wet_in_boundary_table
  grid%tail_statevars%dname_table => wet_in_dname_table
  grid%tail_statevars%desc_table => wet_in_desc_table
  grid%tail_statevars%units_table => wet_in_units_table
  grid%tail_statevars%streams_table => wet_in_streams_table
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097664
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  ...
  grid%tail_statevars%subgrid_y = .FALSE.
  grid%tail_statevars%dimname1 = 'west_east'
  grid%tail_statevars%dimname2 = 'one'
  grid%tail_statevars%dimname3 = 'south_north'
  ENDIF
ELSE
  ALLOCATE(grid%wet_in(1,1,1,1),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3235,&
    'frame/module_domain.f: Failed to allocate grid%wet_in(1,1,1,1).  ')
  endif
ENDIF
...
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1)*num_chem)) * 4
  ...
  grid%tail_statevars%streams(1) = 33554433
  grid%tail_statevars%streams(2) = 2097216
  grid%tail_statevars%sd1 = ids
  grid%tail_statevars%ed1 = (ide-1)
  grid%tail_statevars%sd2 = kds
  grid%tail_statevars%ed2 = (kde-1)
...
IF(.TRUE..AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
(((em33-sm33+1)*(em32-sm32+1)*(spec_bdy_width)*num_chem)) * 4
  ALLOCATE(grid%chem_bxs(sm33:em33,sm32:em32,spec_bdy_width,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3310,&
    'frame/module_domain.f: Failed to allocate grid%chem_bxs(sm33:em33,sm32:em32,spec_bdy_width,num_chem). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_bxs=initial_data_value
  num_bytes_allocated = num_bytes_allocated + &
(((em33-sm33+1)*(em32-sm32+1)*(spec_bdy_width)*num_chem)) * 4
  ALLOCATE(grid%chem_bxe(sm33:em33,sm32:em32,spec_bdy_width,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3318,&
    'frame/module_domain.f: Failed to allocate grid%chem_bxe(sm33:em33,sm32:em32,spec_bdy_width,num_chem). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_bxe=initial_data_value
  num_bytes_allocated = num_bytes_allocated + &
(((em31-sm31+1)*(em32-sm32+1)*(spec_bdy_width)*num_chem)) * 4
  ALLOCATE(grid%chem_bys(sm31:em31,sm32:em32,spec_bdy_width,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3326,&
    'frame/module_domain.f: Failed to allocate grid%chem_bys(sm31:em31,sm32:em32,spec_bdy_width,num_chem). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_bys=initial_data_value
  num_bytes_allocated = num_bytes_allocated + &
(((em31-sm31+1)*(em32-sm32+1)*(spec_bdy_width)*num_chem)) * 4
  ALLOCATE(grid%chem_bye(sm31:em31,sm32:em32,spec_bdy_width,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3334,&
    'frame/module_domain.f: Failed to allocate grid%chem_bye(sm31:em31,sm32:em32,spec_bdy_width,num_chem). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_bye=initial_data_value
ELSE
  ALLOCATE(grid%chem_bxs(1,1,1,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3341,&
    'frame/module_domain.f: Failed to allocate grid%chem_bxs(1,1,1,num_chem).  ')
  endif
  ALLOCATE(grid%chem_bxe(1,1,1,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3346,&
    'frame/module_domain.f: Failed to allocate grid%chem_bxe(1,1,1,num_chem).  ')
  endif
  ALLOCATE(grid%chem_bys(1,1,1,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3351,&
    'frame/module_domain.f: Failed to allocate grid%chem_bys(1,1,1,num_chem).  ')
  endif
  ALLOCATE(grid%chem_bye(1,1,1,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3356,&
    'frame/module_domain.f: Failed to allocate grid%chem_bye(1,1,1,num_chem).  ')
  endif
ENDIF
IF(.TRUE..AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
(((em33-sm33+1)*(em32-sm32+1)*(spec_bdy_width)*num_chem)) * 4
  ALLOCATE(grid%chem_btxs(sm33:em33,sm32:em32,spec_bdy_width,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3365,&
    'frame/module_domain.f: Failed to allocate grid%chem_btxs(sm33:em33,sm32:em32,spec_bdy_width,num_chem). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_btxs=initial_data_value
  num_bytes_allocated = num_bytes_allocated + &
(((em33-sm33+1)*(em32-sm32+1)*(spec_bdy_width)*num_chem)) * 4
  ALLOCATE(grid%chem_btxe(sm33:em33,sm32:em32,spec_bdy_width,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3373,&
    'frame/module_domain.f: Failed to allocate grid%chem_btxe(sm33:em33,sm32:em32,spec_bdy_width,num_chem). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_btxe=initial_data_value
  num_bytes_allocated = num_bytes_allocated + &
(((em31-sm31+1)*(em32-sm32+1)*(spec_bdy_width)*num_chem)) * 4
  ALLOCATE(grid%chem_btys(sm31:em31,sm32:em32,spec_bdy_width,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3381,&
    'frame/module_domain.f: Failed to allocate grid%chem_btys(sm31:em31,sm32:em32,spec_bdy_width,num_chem). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_btys=initial_data_value
  num_bytes_allocated = num_bytes_allocated + &
(((em31-sm31+1)*(em32-sm32+1)*(spec_bdy_width)*num_chem)) * 4
  ALLOCATE(grid%chem_btye(sm31:em31,sm32:em32,spec_bdy_width,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3389,&
    'frame/module_domain.f: Failed to allocate grid%chem_btye(sm31:em31,sm32:em32,spec_bdy_width,num_chem). ')
  endif
  IF ( setinitval .EQ. 1 .OR. setinitval .EQ. 3 ) grid%chem_btye=initial_data_value
ELSE
  ALLOCATE(grid%chem_btxs(1,1,1,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3396,&
    'frame/module_domain.f: Failed to allocate grid%chem_btxs(1,1,1,num_chem).  ')
  endif
  ALLOCATE(grid%chem_btxe(1,1,1,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3401,&
    'frame/module_domain.f: Failed to allocate grid%chem_btxe(1,1,1,num_chem).  ')
  endif
  ALLOCATE(grid%chem_btys(1,1,1,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3406,&
    'frame/module_domain.f: Failed to allocate grid%chem_btys(1,1,1,num_chem).  ')
  endif
  ALLOCATE(grid%chem_btye(1,1,1,num_chem),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",3411,&
    'frame/module_domain.f: Failed to allocate grid%chem_btye(1,1,1,num_chem).  ')
  endif
ENDIF
...
ENDIF
  IF (.NOT.grid%is_intermediate) THEN
   ...
   NULLIFY( grid%tail_statevars%next )
   grid%tail_statevars%ProcOrient = '  '
   grid%tail_statevars%VarName = 'stepbioe'
   grid%tail_statevars%DataName = 'STEPBIOE'
   grid%tail_statevars%Description = 'NUMBER OF FUNDAMENTAL TIMESTEPS BETWEEN BIOGENIC EMIS CALLS'
   ...
   grid%tail_statevars%Ndim = 0
   grid%tail_statevars%scalar_array = .FALSE.
   grid%tail_statevars%ifield_0d => grid%stepbioe
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  ENDIF
IF ( setinitval .EQ. 3 ) grid%stepbioe=0
  IF (.NOT.grid%is_intermediate) THEN
   ALLOCATE( grid%tail_statevars%next )
   grid%tail_statevars => grid%tail_statevars%next
   NULLIFY( grid%tail_statevars%next )
   grid%tail_statevars%ProcOrient = '  '
   grid%tail_statevars%VarName = 'stepphot'
   grid%tail_statevars%DataName = 'STEPPHOT'
   grid%tail_statevars%Description = 'NUMBER OF FUNDAMENTAL TIMESTEPS BETWEEN PHOTOLYSIS CALLS'
   grid%tail_statevars%Units = 'NA'
   grid%tail_statevars%Type = 'i'
   grid%tail_statevars%Ntl = 0
   grid%tail_statevars%Restart = .TRUE.
   grid%tail_statevars%Ndim = 0
   grid%tail_statevars%scalar_array = .FALSE.
   grid%tail_statevars%ifield_0d => grid%stepphot
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  ENDIF
IF ( setinitval .EQ. 3 ) grid%stepphot=0
  IF (.NOT.grid%is_intermediate) THEN
   ALLOCATE( grid%tail_statevars%next )
   grid%tail_statevars => grid%tail_statevars%next
   NULLIFY( grid%tail_statevars%next )
   grid%tail_statevars%ProcOrient = '  '
   grid%tail_statevars%VarName = 'stepchem'
   grid%tail_statevars%DataName = 'STEPCHEM'
   grid%tail_statevars%Description = 'NUMBER OF FUNDAMENTAL TIMESTEPS BETWEEN CHEM MECH CALLS'
   grid%tail_statevars%Units = 'NA'
   grid%tail_statevars%Type = 'i'
   grid%tail_statevars%Ntl = 0
   grid%tail_statevars%Restart = .TRUE.
   grid%tail_statevars%Ndim = 0
   grid%tail_statevars%scalar_array = .FALSE.
   grid%tail_statevars%ifield_0d => grid%stepchem
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  ENDIF
IF ( setinitval .EQ. 3 ) grid%stepchem=0
  IF (.NOT.grid%is_intermediate) THEN
   ALLOCATE( grid%tail_statevars%next )
   grid%tail_statevars => grid%tail_statevars%next
   NULLIFY( grid%tail_statevars%next )
   grid%tail_statevars%ProcOrient = '  '
   grid%tail_statevars%VarName = 'stepfirepl'
   grid%tail_statevars%DataName = 'STEPFIREPL'
   grid%tail_statevars%Description = 'NUMBER OF FUNDAMENTAL TIMESTEPS BETWEEN CALLSTO PLUMERISE FIRE ROUTINE'
   grid%tail_statevars%Units = 'NA'
   grid%tail_statevars%Type = 'i'
   grid%tail_statevars%Ntl = 0
   grid%tail_statevars%Restart = .TRUE.
   grid%tail_statevars%Ndim = 0
   grid%tail_statevars%scalar_array = .FALSE.
   grid%tail_statevars%ifield_0d => grid%stepfirepl
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 2097152
  ENDIF
IF ( setinitval .EQ. 3 ) grid%stepfirepl=0
IF ( setinitval .EQ. 3 ) grid%enh_vermix=.FALSE.
IF ( setinitval .EQ. 3 ) grid%bb_dcycle=.FALSE.
IF ( setinitval .EQ. 3 ) grid%flam_part=initial_data_value
IF ( setinitval .EQ. 3 ) grid%io_style_emissions=0
IF ( setinitval .EQ. 3 ) grid%bioemdt=initial_data_value
IF ( setinitval .EQ. 3 ) grid%photdt=initial_data_value
IF ( setinitval .EQ. 3 ) grid%chemdt=initial_data_value
IF ( setinitval .EQ. 3 ) grid%ne_area=0
IF ( setinitval .EQ. 3 ) grid%kemit=0
IF ( setinitval .EQ. 3 ) grid%nmegan=0
IF ( setinitval .EQ. 3 ) grid%kfuture=0
IF ( setinitval .EQ. 3 ) grid%kfire=0
IF ( setinitval .EQ. 3 ) grid%kemit_aircraft=0
IF ( setinitval .EQ. 3 ) grid%kdvel=0
IF ( setinitval .EQ. 3 ) grid%ndepvel=0
IF ( setinitval .EQ. 3 ) grid%kdepvel=0
IF ( setinitval .EQ. 3 ) grid%erosion_dim=0
IF ( setinitval .EQ. 3 ) grid%biomass_emiss_opt=0
IF ( setinitval .EQ. 3 ) grid%cam_mam_mode=0
IF ( setinitval .EQ. 3 ) grid%cam_mam_nspec=0
IF ( setinitval .EQ. 3 ) grid%cam_mp_mam_cpled=.FALSE.
IF ( setinitval .EQ. 3 ) grid%lightning_opt=0
IF ( setinitval .EQ. 3 ) grid%lightning_time_step=0
IF ( setinitval .EQ. 3 ) grid%temp_upper=initial_data_value
IF ( setinitval .EQ. 3 ) grid%temp_lower=initial_data_value
IF ( setinitval .EQ. 3 ) grid%n_ic=initial_data_value
IF ( setinitval .EQ. 3 ) grid%n_cg=initial_data_value
IF ( setinitval .EQ. 3 ) grid%passive_ltng=0
IF ( setinitval .EQ. 3 ) grid%lflash_data=0
IF ( setinitval .EQ. 3 ) grid%flashrate_method=0
IF ( setinitval .EQ. 3 ) grid%wpeat=initial_data_value
IF ( setinitval .EQ. 3 ) grid%wflood=initial_data_value
IF ( setinitval .EQ. 3 ) grid%chem_conv_tr=0
IF ( setinitval .EQ. 3 ) grid%conv_tr_wetscav=0
IF ( setinitval .EQ. 3 ) grid%conv_tr_aqchem=0
IF ( setinitval .EQ. 3 ) grid%chem_opt=0
IF ( setinitval .EQ. 3 ) grid%gaschem_onoff=0
IF ( setinitval .EQ. 3 ) grid%aerchem_onoff=0
IF ( setinitval .EQ. 3 ) grid%wetscav_onoff=0
IF ( setinitval .EQ. 3 ) grid%dustwd_onoff=0
IF ( setinitval .EQ. 3 ) grid%cldchem_onoff=0
IF ( setinitval .EQ. 3 ) grid%is_full_tuv=.FALSE.
IF ( setinitval .EQ. 3 ) grid%lambda_cutoff=initial_data_value
IF ( setinitval .EQ. 3 ) grid%cld_od_opt=0
IF ( setinitval .EQ. 3 ) grid%pht_cldfrc_opt=0
IF ( setinitval .EQ. 3 ) grid%vertmix_onoff=0
IF ( setinitval .EQ. 3 ) grid%chem_in_opt=0
IF ( setinitval .EQ. 3 ) grid%phot_opt=0
IF ( setinitval .EQ. 3 ) grid%gas_drydep_opt=0
IF ( setinitval .EQ. 3 ) grid%aer_drydep_opt=0
IF ( setinitval .EQ. 3 ) grid%aero_diag_opt=0
IF ( setinitval .EQ. 3 ) grid%aero_cw_diag_opt=0
IF ( setinitval .EQ. 3 ) grid%kfcup_diag=0
IF ( setinitval .EQ. 3 ) grid%diagnostic_chem=0
IF ( setinitval .EQ. 3 ) grid%aer_aerodynres_opt=0
IF ( setinitval .EQ. 3 ) grid%emiss_opt=0
IF ( setinitval .EQ. 3 ) grid%emiss_opt_vol=0
IF ( setinitval .EQ. 3 ) grid%dust_opt=0
IF ( setinitval .EQ. 3 ) grid%dust_schme=0
IF ( setinitval .EQ. 3 ) grid%dmsemis_opt=0
IF ( setinitval .EQ. 3 ) grid%seas_opt=0
IF ( setinitval .EQ. 3 ) grid%bio_emiss_opt=0
IF ( setinitval .EQ. 3 ) grid%biomass_burn_opt=0
IF ( setinitval .EQ. 3 ) grid%plumerisefire_frq=0
IF ( setinitval .EQ. 3 ) grid%emiss_inpt_opt=0
IF ( setinitval .EQ. 3 ) grid%gas_bc_opt=0
IF ( setinitval .EQ. 3 ) grid%gas_ic_opt=0
IF ( setinitval .EQ. 3 ) grid%aer_bc_opt=0
IF ( setinitval .EQ. 3 ) grid%aer_ic_opt=0
IF ( setinitval .EQ. 3 ) grid%have_bcs_chem=.FALSE.
IF ( setinitval .EQ. 3 ) grid%have_bcs_tracer=.FALSE.
IF ( setinitval .EQ. 3 ) grid%scale_fire_emiss=.FALSE.
IF ( setinitval .EQ. 3 ) grid%aer_ra_feedback=0
IF ( setinitval .EQ. 3 ) grid%aer_op_opt=0
IF ( setinitval .EQ. 3 ) grid%opt_pars_out=0
IF ( setinitval .EQ. 3 ) grid%diagnostic_dep=0
IF ( setinitval .EQ. 3 ) grid%aircraft_emiss_opt=0
IF ( setinitval .EQ. 3 ) grid%have_bcs_upper=.FALSE.
IF ( setinitval .EQ. 3 ) grid%fixed_ubc_press=initial_data_value
IF ( setinitval .EQ. 3 ) grid%chemdiag=0
IF ( setinitval .EQ. 3 ) grid%dust_alpha=initial_data_value
IF ( setinitval .EQ. 3 ) grid%dust_gamma=initial_data_value
IF ( setinitval .EQ. 3 ) grid%dust_smtune=initial_data_value
IF ( setinitval .EQ. 3 ) grid%dust_ustune=initial_data_value
IF ( setinitval .EQ. 3 ) grid%dust_dsr=0
IF ( setinitval .EQ. 3 ) grid%dust_veg=0
IF ( setinitval .EQ. 3 ) grid%dust_soils=0
IF ( setinitval .EQ. 3 ) grid%dust_smois=0
IF ( setinitval .EQ. 3 ) grid%emiss_ash_hgt=initial_data_value
IF ( setinitval .EQ. 3 ) grid%depo_fact=initial_data_value
IF ( setinitval .EQ. 3 ) grid%track_chem_num=0
IF ( setinitval .EQ. 3 ) grid%track_rad_num=0
IF ( setinitval .EQ. 3 ) grid%track_tuv_num=0
IF ( setinitval .EQ. 3 ) grid%track_tuv_lev=0
IF ( setinitval .EQ. 3 ) grid%n2o5_hetchem=0
IF ( setinitval .EQ. 3 ) grid%af_lambda_start=initial_data_value
IF ( setinitval .EQ. 3 ) grid%af_lambda_end=initial_data_value
IF ( setinitval .EQ. 3 ) grid%plumerise_flag=0
IF ( setinitval .EQ. 3 ) grid%debug_chem=.FALSE.
IF ( setinitval .EQ. 3 ) grid%simple_dir_fdb=0
IF ( setinitval .EQ. 3 ) grid%simple_ind_fdb=0
IF ( setinitval .EQ. 3 ) grid%lnox_opt=0
IF ( setinitval .EQ. 3 ) grid%lnox_passive=.FALSE.
IF ( setinitval .EQ. 3 ) grid%ltng_temp_upper=initial_data_value
IF ( setinitval .EQ. 3 ) grid%ltng_temp_lower=initial_data_value
IF ( setinitval .EQ. 3 ) grid%has_o3_exo_coldens=.FALSE.
IF ( setinitval .EQ. 3 ) grid%du_at_grnd=initial_data_value
IF ( setinitval .EQ. 3 ) grid%scale_o3_to_grnd_exo_coldens=.FALSE.
IF ( setinitval .EQ. 3 ) grid%scale_o3_to_du_at_grnd=.FALSE.
...
((((em31)-(sm31)+1))*(((em32)-(sm32)+1))*(((em33)-(sm33)+1))) * 4
  ...
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",6338,&
...
IF(okay_to_alloc.AND.in_use_for_config(id,'pattern_spp_mp'))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((em31)-(sm31)+1))*(((model_config_rec%num_stoch_levels)-(1)+1))*(((em33)-(sm33)+1))) * 4
  ALLOCATE(grid%pattern_spp_mp(sm31:em31,1:model_config_rec%num_stoch_levels,sm33:em33),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",7520,&
    'frame/module_domain.f: Failed to allocate grid%pattern_spp_mp(sm31:em31,1:model_config_rec%num_stoch_levels,sm33:em33). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spstreamforcc').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spstreamforcc(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",7697,&
    'frame/module_domain.f: Failed to allocate grid%spstreamforcc(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
  ...
  grid%tail_statevars%streams(2) = 0
  grid%tail_statevars%sd1 = 1
  grid%tail_statevars%ed1 = config_flags%lmax_ideal
  grid%tail_statevars%sd2 = 1
  grid%tail_statevars%ed2 = config_flags%kmax_ideal
  grid%tail_statevars%sd3 = 1
  grid%tail_statevars%ed3 = 1
  grid%tail_statevars%sm1 = 1
  grid%tail_statevars%em1 = config_flags%lmax_ideal
  grid%tail_statevars%sm2 = 1
  grid%tail_statevars%em2 = config_flags%kmax_ideal
  grid%tail_statevars%sm3 = 1
  grid%tail_statevars%em3 = 1
  grid%tail_statevars%sp1 = 1
  grid%tail_statevars%ep1 = config_flags%lmax_ideal
  grid%tail_statevars%sp2 = 1
  grid%tail_statevars%ep2 = config_flags%kmax_ideal
  grid%tail_statevars%sp3 = 1
  grid%tail_statevars%ep3 = 1
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spstreamforcs').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spstreamforcs(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",7754,&
    'frame/module_domain.f: Failed to allocate grid%spstreamforcs(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spstream_amp').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spstream_amp(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",7811,&
    'frame/module_domain.f: Failed to allocate grid%spstream_amp(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'sptforcc').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%sptforcc(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",7868,&
    'frame/module_domain.f: Failed to allocate grid%sptforcc(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'sptforcs').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%sptforcs(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",7925,&
    'frame/module_domain.f: Failed to allocate grid%sptforcs(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spt_amp').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spt_amp(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",7982,&
    'frame/module_domain.f: Failed to allocate grid%spt_amp(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcc').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcc(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8039,&
    'frame/module_domain.f: Failed to allocate grid%spforcc(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcs').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcs(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8096,&
    'frame/module_domain.f: Failed to allocate grid%spforcs(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'sp_amp').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%sp_amp(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8153,&
    'frame/module_domain.f: Failed to allocate grid%sp_amp(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcc2').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcc2(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8210,&
    'frame/module_domain.f: Failed to allocate grid%spforcc2(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcs2').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcs2(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8267,&
    'frame/module_domain.f: Failed to allocate grid%spforcs2(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'sp_amp2').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%sp_amp2(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8324,&
    'frame/module_domain.f: Failed to allocate grid%sp_amp2(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcc3').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcc3(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8381,&
    'frame/module_domain.f: Failed to allocate grid%spforcc3(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcs3').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcs3(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8438,&
    'frame/module_domain.f: Failed to allocate grid%spforcs3(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'sp_amp3').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%sp_amp3(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8495,&
    'frame/module_domain.f: Failed to allocate grid%sp_amp3(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcc4').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcc4(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8552,&
    'frame/module_domain.f: Failed to allocate grid%spforcc4(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcs4').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcs4(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8609,&
    'frame/module_domain.f: Failed to allocate grid%spforcs4(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'sp_amp4').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%sp_amp4(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8666,&
    'frame/module_domain.f: Failed to allocate grid%sp_amp4(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcc5').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcc5(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8723,&
    'frame/module_domain.f: Failed to allocate grid%spforcc5(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spforcs5').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spforcs5(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8780,&
    'frame/module_domain.f: Failed to allocate grid%spforcs5(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'sp_amp5').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%sp_amp5(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8837,&
    'frame/module_domain.f: Failed to allocate grid%sp_amp5(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spptforcc').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spptforcc(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8894,&
    'frame/module_domain.f: Failed to allocate grid%spptforcc(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'spptforcs').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%spptforcs(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",8951,&
    'frame/module_domain.f: Failed to allocate grid%spptforcs(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'sppt_amp').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%lmax_ideal)-(1)+1))*(((model_config_rec%kmax_ideal)-(1)+1))) * 4
  ALLOCATE(grid%sppt_amp(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",9008,&
    'frame/module_domain.f: Failed to allocate grid%sppt_amp(1:model_config_rec%lmax_ideal,1:model_config_rec%kmax_ideal). ')
  endif
...
IF(okay_to_alloc.AND.in_use_for_config(id,'iseedarr_spp_mp').AND.(.NOT.grid%is_intermediate))THEN
  num_bytes_allocated = num_bytes_allocated + &
((((model_config_rec%seed_dim)-(1)+1))) * 4
  ALLOCATE(grid%iseedarr_spp_mp(1:model_config_rec%seed_dim),STAT=ierr)
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",9521,&
    'frame/module_domain.f: Failed to allocate grid%iseedarr_spp_mp(1:model_config_rec%seed_dim). ')
  endif
  ...
  grid%tail_statevars%DataName = 'ISEEDARRAY_SPP_MP'
  grid%tail_statevars%Description = 'Array to hold seed for restart, RAND_PERT5'
  grid%tail_statevars%Units = ''
  grid%tail_statevars%Type = 'i'
  grid%tail_statevars%ProcOrient = ' '
  grid%tail_statevars%MemoryOrder = 'Z'
...
IF ( setinitval .EQ. 3 ) grid%alph_rand4=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
   ...
   NULLIFY( grid%tail_statevars%next )
   grid%tail_statevars%ProcOrient = '  '
   grid%tail_statevars%VarName = 'alph_rand5'
   grid%tail_statevars%DataName = 'ALPH_RAND5'
   grid%tail_statevars%Description = 'autoregressive coeff. for generic rand. pert.'
   grid%tail_statevars%Units = ''
   grid%tail_statevars%Type = 'r'
   grid%tail_statevars%Ntl = 0
   ...
   grid%tail_statevars%Ndim = 0
   grid%tail_statevars%scalar_array = .FALSE.
   grid%tail_statevars%rfield_0d => grid%alph_rand5
  grid%tail_statevars%streams(1) = 0
  grid%tail_statevars%streams(2) = 0
  ENDIF
IF ( setinitval .EQ. 3 ) grid%alph_rand5=initial_data_value
  IF (.NOT.grid%is_intermediate) THEN
   ALLOCATE( grid%tail_statevars%next )
   grid%tail_statevars => grid%tail_statevars%next
   NULLIFY( grid%tail_statevars%next )
...
IF ( setinitval .EQ. 3 ) grid%lmax_ideal=0
IF ( setinitval .EQ. 3 ) grid%kmax_ideal=0
IF ( setinitval .EQ. 3 ) grid%stepstoch=0
...
ELSE
  ...
  if (ierr.ne.0) then
    CALL wrf_error_fatal3("<stdin>",15263,&
