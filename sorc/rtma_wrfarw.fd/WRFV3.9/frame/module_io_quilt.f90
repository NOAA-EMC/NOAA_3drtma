...
mess)
        ...
        IF ( obufsize .GT. 0 ) THEN
          ALLOCATE( obuf( (obufsize+1)/itypesize ) )
          CALL collect_on_comm_debug("module_io_quilt_old.F",1758, mpi_comm_io_groups(1), &
                                onebyte, &
                                dummy, 0, &
                                obuf, obufsize )
        ELSE
...
logical :: ncd_nofill
logical , DIMENSION(max_domains) :: enh_vermix
logical , DIMENSION(max_domains) :: bb_dcycle
real , DIMENSION(max_domains) :: flam_part
...
integer , DIMENSION(max_domains) :: lightning_opt
integer , DIMENSION(max_domains) :: lightning_time_step
real , DIMENSION(max_domains) :: temp_upper
real , DIMENSION(max_domains) :: temp_lower
real , DIMENSION(max_domains) :: n_ic
real , DIMENSION(max_domains) :: n_cg
integer , DIMENSION(max_domains) :: passive_ltng
integer :: lflash_data
integer , DIMENSION(max_domains) :: flashrate_method
...
integer , DIMENSION(max_domains) :: plumerise_flag
logical , DIMENSION(max_domains) :: debug_chem
integer , DIMENSION(max_domains) :: simple_dir_fdb
integer , DIMENSION(max_domains) :: simple_ind_fdb
...
NAMELIST /chem/ enh_vermix
NAMELIST /chem/ bb_dcycle
NAMELIST /chem/ flam_part
...
NAMELIST /chem/ erosion_dim
NAMELIST /chem/ biomass_emiss_opt
NAMELIST /chem/ cam_mam_mode
NAMELIST /chem/ cam_mam_nspec
NAMELIST /chem/ cam_mp_mam_cpled
NAMELIST /chem/ lightning_opt
NAMELIST /chem/ lightning_time_step
NAMELIST /chem/ temp_upper
NAMELIST /chem/ temp_lower
NAMELIST /chem/ n_ic
NAMELIST /chem/ n_cg
NAMELIST /chem/ passive_ltng
NAMELIST /chem/ lflash_data
NAMELIST /chem/ flashrate_method
...
NAMELIST /chem/ plumerise_flag
NAMELIST /chem/ debug_chem
NAMELIST /chem/ simple_dir_fdb
NAMELIST /chem/ simple_ind_fdb
...
SUBROUTINE wrf_quilt_open_for_write_begin( FileName , gridid, Comm_compute, Comm_io, SysDepInfo, &
  ...
  CALL collect_on_comm_debug("module_io_quilt_old.F",3096, comm_io_group, &
                        onebyte, &
                        hdrbuf, hdrbufsize , &
                        dummy, 0 )
  Status = 0
...
SUBROUTINE wrf_quilt_open_for_write_commit( DataHandle , Status )
  ...
  CALL MPI_Reduce(reduced,reduced_dummy,2,MPI_INTEGER,MPI_SUM,tasks_in_group-1,comm_io_group,ierr)
  CALL collect_on_comm_debug("module_io_quilt_old.F",3172, comm_io_group, &
                        onebyte, &
                        hdrbuf, hdrbufsize , &
                        dummy, 0 )
  Status = 0
...
SUBROUTINE wrf_quilt_iosync ( DataHandle, Status )
  ...
  IF ( associated ( int_local_output_buffer ) ) THEN
    ...
    IF ( compute_group_master(1) ) reduced(2) = DataHandle
    CALL MPI_Reduce(reduced,reduced_dummy,2,MPI_INTEGER,MPI_SUM,tasks_in_group-1,comm_io_group,ierr)
    CALL collect_on_comm_debug("module_io_quilt_old.F",3339, comm_io_group, &
                          onebyte, &
...
SUBROUTINE wrf_quilt_set_time ( DataHandle, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( compute_group_master(1) ) reduced(2) = DataHandle
      CALL MPI_Reduce(reduced,reduced_dummy,2,MPI_INTEGER,MPI_SUM,tasks_in_group-1,comm_io_group,ierr)
      CALL collect_on_comm_debug("module_io_quilt_old.F",3610, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_real ( DataHandle,Element, Data, Count, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF( compute_group_master(1) ) reduced(2) = DataHandle
      CALL MPI_Reduce(reduced,reduced_dummy,2,MPI_INTEGER,MPI_SUM,tasks_in_group-1,comm_io_group,ierr)
      CALL collect_on_comm_debug("module_io_quilt_old.F",3728, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_integer ( DataHandle,Element, Data, Count, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( compute_group_master(1) ) reduced(2) = DataHandle
      CALL MPI_Reduce(reduced,reduced_dummy,2,MPI_INTEGER,MPI_SUM,tasks_in_group-1,comm_io_group,ierr)
      CALL collect_on_comm_debug("module_io_quilt_old.F",3888, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_char ( DataHandle, Element, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF(compute_group_master(1)) reduced(2) = DataHandle
      CALL MPI_Reduce(reduced,reduced_dummy,2,MPI_INTEGER,MPI_SUM,tasks_in_group-1,comm_io_group,ierr)
      CALL collect_on_comm_debug("module_io_quilt_old.F",4059, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_var_ti_char ( DataHandle,Element, Varname, Data, Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      IF ( compute_group_master(1) ) reduced(2) = DataHandle
      CALL MPI_Reduce(reduced,reduced_dummy,2,MPI_INTEGER,MPI_SUM,tasks_in_group-1,comm_io_group,ierr)
      CALL collect_on_comm_debug("module_io_quilt_old.F",4591, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
"frame/module_io_quilt.F: wrf_quilt_write_field: DataHandle not opened" )
  ...
  IF ( .NOT. okay_to_write( DataHandle ) ) THEN
      ...
      IF ( compute_group_master(1) ) reduced(2) = DataHandle
      CALL MPI_Reduce(reduced,reduced_dummy,2,MPI_INTEGER,MPI_SUM,tasks_in_group-1,comm_io_group,ierr)
      CALL collect_on_comm_debug("module_io_quilt_old.F",5013, comm_io_group, &
                            onebyte, &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
  ELSE
...
subroutine wrf_quilt_find_server(iserver)
  ...
  real(kind=c_double) :: start,finish
  call hires_timer(start)
...
'mpi_bcast failed in wrf_quilt_find_server')
  ...
  call hires_timer(finish)
  write(message,'("I/O server ",I0," is ready for operations after ",F10.5," seconds")') &
       iserver,finish-start
