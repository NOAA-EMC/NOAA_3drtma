...
character *24 :: temp24 , temp24b
   ...
   REAL , DIMENSION(:,:,:) , ALLOCATABLE , SAVE :: qcbdy3dtemp1, qrbdy3dtemp1, qsbdy3dtemp1, qgbdy3dtemp1, qnrbdy3dtemp1, &
                                                   qnibdy3dtemp1, qibdy3dtemp1, qncbdy3dtemp1
   REAL , DIMENSION(:,:,:) , ALLOCATABLE , SAVE :: qcbdy3dtemp2, qrbdy3dtemp2, qsbdy3dtemp2, qgbdy3dtemp2, qnrbdy3dtemp2, &
                                                   qnibdy3dtemp2, qibdy3dtemp2, qncbdy3dtemp2
real::t1,t2
        REAL , DIMENSION(:,:,:) , ALLOCATABLE , SAVE :: smkbdy3dtemp1, smkbdy3dtemp2
   ids = grid%sd31
   ide = grid%ed31
   kds = grid%sd32
   kde = grid%ed32
...
'Submit the real program again with fewer processors ')
   ...
   IF ( loop .EQ. 1 ) THEN
      ...
      ELSE
         ...
         IF ( ALLOCATED ( qcbdy3dtemp1 ) ) DEALLOCATE ( qcbdy3dtemp1 )
         IF ( ALLOCATED ( qibdy3dtemp1 ) ) DEALLOCATE ( qibdy3dtemp1 )
         IF ( ALLOCATED ( qrbdy3dtemp1 ) ) DEALLOCATE ( qrbdy3dtemp1 )
         IF ( ALLOCATED ( qsbdy3dtemp1 ) ) DEALLOCATE ( qsbdy3dtemp1 )
         IF ( ALLOCATED ( qgbdy3dtemp1 ) ) DEALLOCATE ( qgbdy3dtemp1 )
         IF ( ALLOCATED ( qnrbdy3dtemp1 ) ) DEALLOCATE ( qnrbdy3dtemp1 )
         IF ( ALLOCATED ( qnibdy3dtemp1 ) ) DEALLOCATE ( qnibdy3dtemp1 )
         IF ( ALLOCATED ( qncbdy3dtemp1 ) ) DEALLOCATE ( qncbdy3dtemp1 )
         IF ( ALLOCATED ( qcbdy3dtemp2 ) ) DEALLOCATE ( qcbdy3dtemp2 )
         IF ( ALLOCATED ( qibdy3dtemp2 ) ) DEALLOCATE ( qibdy3dtemp2 )
         IF ( ALLOCATED ( qrbdy3dtemp2 ) ) DEALLOCATE ( qrbdy3dtemp2 )
         IF ( ALLOCATED ( qsbdy3dtemp2 ) ) DEALLOCATE ( qsbdy3dtemp2 )
         IF ( ALLOCATED ( qgbdy3dtemp2 ) ) DEALLOCATE ( qgbdy3dtemp2 )
         IF ( ALLOCATED ( qnrbdy3dtemp2 ) ) DEALLOCATE ( qnrbdy3dtemp2 )
         IF ( ALLOCATED ( qnibdy3dtemp2 ) ) DEALLOCATE ( qnibdy3dtemp2 )
         IF ( ALLOCATED ( qncbdy3dtemp2 ) ) DEALLOCATE ( qncbdy3dtemp2 )
         ...
         ALLOCATE ( qcbdy3dtemp1(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qibdy3dtemp1(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qrbdy3dtemp1(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qsbdy3dtemp1(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qgbdy3dtemp1(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qnrbdy3dtemp1(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qnibdy3dtemp1(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qncbdy3dtemp1(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qcbdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qibdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qrbdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qsbdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qgbdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qnrbdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qnibdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         ALLOCATE ( qncbdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         ...
         END IF
         IF (config_flags%chem_opt .eq. 18) THEN
            IF ( ALLOCATED ( smkbdy3dtemp1 ) ) DEALLOCATE ( smkbdy3dtemp1 )
            IF ( ALLOCATED ( smkbdy3dtemp2 ) ) DEALLOCATE ( smkbdy3dtemp2 )
            ALLOCATE ( smkbdy3dtemp1(ims:ime,kms:kme,jms:jme) )
            ALLOCATE ( smkbdy3dtemp2(ims:ime,kms:kme,jms:jme) )
         END IF
      END IF
...
'real: error opening wrflowinp for writing' )
      ...
      ELSE
         ...
         CALL couple ( grid%mu_2 , grid%mub , qbdy3dtemp1 , grid%moist(:,:,:,P_QV) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1h, grid%c2h, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qcbdy3dtemp1 , grid%moist(:,:,:,P_QC) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qibdy3dtemp1 , grid%moist(:,:,:,P_QI) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qrbdy3dtemp1 , grid%moist(:,:,:,P_QR) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qsbdy3dtemp1 , grid%moist(:,:,:,P_QS) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qgbdy3dtemp1 , grid%moist(:,:,:,P_QG) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qnrbdy3dtemp1 , grid%scalar(:,:,:,P_QNR) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qnibdy3dtemp1 , grid%scalar(:,:,:,P_QNI) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qncbdy3dtemp1 , grid%scalar(:,:,:,P_QNC) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         ...
         IF (config_flags%chem_opt==18) THEN
            CALL couple ( grid%mu_2 , grid%mub , smkbdy3dtemp1 , grid%chem(:,:,:,P_SMOKE) , 'c' , grid%msfty , &
                          grid%c1h, grid%c2h, grid%c1h, grid%c2h, &
                          ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         ENDIF
         smkbdy3dtemp1=1.e+9*smkbdy3dtemp1
      END IF
      ...
      ELSE
         ...
         CALL stuff_bdy ( mbdy2dtemp1 , grid%mu_bxs, grid%mu_bxe, grid%mu_bys, grid%mu_bye, &
                                                              'M' , spec_bdy_width , &
                                                                    ...
                                                                    ims , ime , jms , jme , 1 , 1 , &
                                                                    ips , ipe , jps , jpe , 1 , 1 )
         CALL stuff_bdy ( qcbdy3dtemp1 , grid%moist_bxs(:,:,:,P_QC), grid%moist_bxe(:,:,:,P_QC), &
                                             grid%moist_bys(:,:,:,P_QC), grid%moist_bye(:,:,:,P_QC), &
                                                              'T' , spec_bdy_width , &
                                                                    ids , ide , jds , jde , kds , kde , &
                                                                    ims , ime , jms , jme , kms , kme , &
                                                                    ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdy ( qibdy3dtemp1 , grid%moist_bxs(:,:,:,P_QI), grid%moist_bxe(:,:,:,P_QI), &
                                             grid%moist_bys(:,:,:,P_QI), grid%moist_bye(:,:,:,P_QI), &
                                                              'T' , spec_bdy_width , &
                                                                    ids , ide , jds , jde , kds , kde , &
                                                                    ims , ime , jms , jme , kms , kme , &
                                                                    ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdy ( qrbdy3dtemp1 , grid%moist_bxs(:,:,:,P_QR), grid%moist_bxe(:,:,:,P_QR), &
                                             grid%moist_bys(:,:,:,P_QR), grid%moist_bye(:,:,:,P_QR), &
                                                              'T' , spec_bdy_width , &
                                                                    ids , ide , jds , jde , kds , kde , &
                                                                    ims , ime , jms , jme , kms , kme , &
                                                                    ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdy ( qsbdy3dtemp1 , grid%moist_bxs(:,:,:,P_QS), grid%moist_bxe(:,:,:,P_QS), &
                                             grid%moist_bys(:,:,:,P_QS), grid%moist_bye(:,:,:,P_QS), &
                                                              'T' , spec_bdy_width , &
                                                                    ids , ide , jds , jde , kds , kde , &
                                                                    ims , ime , jms , jme , kms , kme , &
                                                                    ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdy ( qgbdy3dtemp1 , grid%moist_bxs(:,:,:,P_QG), grid%moist_bxe(:,:,:,P_QG), &
                                             grid%moist_bys(:,:,:,P_QG), grid%moist_bye(:,:,:,P_QG), &
                                                              'T' , spec_bdy_width , &
                                                                    ids , ide , jds , jde , kds , kde , &
                                                                    ims , ime , jms , jme , kms , kme , &
                                                                    ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdy ( qnrbdy3dtemp1 , grid%scalar_bxs(:,:,:,P_QNR), grid%scalar_bxe(:,:,:,P_QNR), &
                                            grid%scalar_bys(:,:,:,P_QNR), grid%scalar_bye(:,:,:,P_QNR), &
                                                              'T' , spec_bdy_width , &
                                                                    ids , ide , jds , jde , kds , kde , &
                                                                    ims , ime , jms , jme , kms , kme , &
                                                                    ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdy ( qnibdy3dtemp1 , grid%scalar_bxs(:,:,:,P_QNI), grid%scalar_bxe(:,:,:,P_QNI), &
                                            grid%scalar_bys(:,:,:,P_QNI), grid%scalar_bye(:,:,:,P_QNI), &
                                                              'T' , spec_bdy_width , &
                                                                    ids , ide , jds , jde , kds , kde , &
                                                                    ims , ime , jms , jme , kms , kme , &
                                                                    ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdy ( qncbdy3dtemp1 , grid%scalar_bxs(:,:,:,P_QNC), grid%scalar_bxe(:,:,:,P_QNC), &
                                            grid%scalar_bys(:,:,:,P_QNC), grid%scalar_bye(:,:,:,P_QNC), &
                                                              'T' , spec_bdy_width , &
                                                                    ids , ide , jds , jde , kds , kde , &
                                                                    ims , ime , jms , jme , kms , kme , &
                                                                    ips , ipe , jps , jpe , kps , kpe )
         ...
         END IF
        IF (config_flags%chem_opt==18) THEN
        CALL stuff_bdy ( smkbdy3dtemp1, grid%chem_bxs(:,:,:,P_SMOKE), grid%chem_bxe(:,:,:,P_SMOKE), &
                         grid%chem_bys(:,:,:,P_SMOKE), grid%chem_bye(:,:,:,P_SMOKE), &
                         'T' , spec_bdy_width , &
                         ids , ide , jds , jde , kds , kde , &
                         ims , ime , jms , jme , kms , kme , &
                         ips , ipe , jps , jpe , kps , kpe )
...
'real: error opening wrffdda for writing' )
      ...
      ELSE
         ...
         CALL couple ( grid%mu_2 , grid%mub , qbdy3dtemp2 , grid%moist(:,:,:,P_QV) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1h, grid%c2h, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qcbdy3dtemp2 , grid%moist(:,:,:,P_QC) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qibdy3dtemp2 , grid%moist(:,:,:,P_QI) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qrbdy3dtemp2 , grid%moist(:,:,:,P_QR) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qsbdy3dtemp2 , grid%moist(:,:,:,P_QS) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qgbdy3dtemp2 , grid%moist(:,:,:,P_QG) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qnrbdy3dtemp2 , grid%scalar(:,:,:,P_QNR) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qnibdy3dtemp2 , grid%scalar(:,:,:,P_QNI) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         CALL couple ( grid%mu_2 , grid%mub , qncbdy3dtemp2 , grid%scalar(:,:,:,P_QNC) , 't' , grid%msfty , &
                       grid%c1h, grid%c2h, grid%c1f, grid%c2f, &
                       ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         DO j = jps , jpe
            DO i = ips , ipe
         ...
         END IF
         IF (config_flags%chem_opt==18) THEN
            CALL couple ( grid%mu_2 , grid%mub , smkbdy3dtemp2 , grid%chem(:,:,:,P_SMOKE) , 'c' , grid%msfty , &
                          grid%c1h, grid%c2h, grid%c1h, grid%c2h, &
                          ids, ide, jds, jde, kds, kde, ims, ime, jms, jme, kms, kme, ips, ipe, jps, jpe, kps, kpe )
         END IF
         smkbdy3dtemp2=1.e+9* smkbdy3dtemp2
      END IF
      IF(grid_fdda .GE. 1)THEN
         DO j = jps , jpe
            DO k = kps , kpe
      ...
      ELSE
         ...
         CALL stuff_bdytend ( qbdy3dtemp2 , qbdy3dtemp1 , REAL(interval_seconds) , &
                                                               ...
                                                               ims , ime , jms , jme , kms , kme , &
                                                               ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdytend ( qcbdy3dtemp2 , qcbdy3dtemp1 , REAL(interval_seconds) , &
                                                               grid%moist_btxs(:,:,:,P_QC), grid%moist_btxe(:,:,:,P_QC), &
                                                               grid%moist_btys(:,:,:,P_QC), grid%moist_btye(:,:,:,P_QC), &
                                                               'T' , &
                                                               spec_bdy_width , &
                                                               ids , ide , jds , jde , kds , kde , &
                                                               ims , ime , jms , jme , kms , kme , &
                                                               ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdytend ( qibdy3dtemp2 , qibdy3dtemp1 , REAL(interval_seconds) , &
                                                               grid%moist_btxs(:,:,:,P_QI), grid%moist_btxe(:,:,:,P_QI), &
                                                               grid%moist_btys(:,:,:,P_QI), grid%moist_btye(:,:,:,P_QI), &
                                                               'T' , &
                                                               spec_bdy_width , &
                                                               ids , ide , jds , jde , kds , kde , &
                                                               ims , ime , jms , jme , kms , kme , &
                                                               ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdytend ( qrbdy3dtemp2 , qrbdy3dtemp1 , REAL(interval_seconds) , &
                                                               grid%moist_btxs(:,:,:,P_QR), grid%moist_btxe(:,:,:,P_QR), &
                                                               grid%moist_btys(:,:,:,P_QR), grid%moist_btye(:,:,:,P_QR), &
                                                               'T' , &
                                                               spec_bdy_width , &
                                                               ids , ide , jds , jde , kds , kde , &
                                                               ims , ime , jms , jme , kms , kme , &
                                                               ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdytend ( qsbdy3dtemp2 , qsbdy3dtemp1 , REAL(interval_seconds) , &
                                                               grid%moist_btxs(:,:,:,P_QS), grid%moist_btxe(:,:,:,P_QS), &
                                                               grid%moist_btys(:,:,:,P_QS), grid%moist_btye(:,:,:,P_QS), &
                                                               'T' , &
                                                               spec_bdy_width , &
                                                               ids , ide , jds , jde , kds , kde , &
                                                               ims , ime , jms , jme , kms , kme , &
                                                               ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdytend ( qgbdy3dtemp2 , qgbdy3dtemp1 , REAL(interval_seconds) , &
                                                               grid%moist_btxs(:,:,:,P_QG), grid%moist_btxe(:,:,:,P_QG), &
                                                               grid%moist_btys(:,:,:,P_QG), grid%moist_btye(:,:,:,P_QG), &
                                                               'T' , &
                                                               spec_bdy_width , &
                                                               ids , ide , jds , jde , kds , kde , &
                                                               ims , ime , jms , jme , kms , kme , &
                                                               ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdytend ( qnrbdy3dtemp2 , qnrbdy3dtemp1 , REAL(interval_seconds) , &
                                                               grid%scalar_btxs(:,:,:,P_QNR), grid%scalar_btxe(:,:,:,P_QNR), &
                                                               grid%scalar_btys(:,:,:,P_QNR), grid%scalar_btye(:,:,:,P_QNR), &
                                                               'T' , &
                                                               spec_bdy_width , &
                                                               ids , ide , jds , jde , kds , kde , &
                                                               ims , ime , jms , jme , kms , kme , &
                                                               ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdytend ( qnibdy3dtemp2 , qnibdy3dtemp1 , REAL(interval_seconds) , &
                                                               grid%scalar_btxs(:,:,:,P_QNI), grid%scalar_btxe(:,:,:,P_QNI), &
                                                               grid%scalar_btys(:,:,:,P_QNI), grid%scalar_btye(:,:,:,P_QNI), &
                                                               'T' , &
                                                               spec_bdy_width , &
                                                               ids , ide , jds , jde , kds , kde , &
                                                               ims , ime , jms , jme , kms , kme , &
                                                               ips , ipe , jps , jpe , kps , kpe )
         CALL stuff_bdytend ( qncbdy3dtemp2 , qncbdy3dtemp1 , REAL(interval_seconds) , &
                                                               grid%scalar_btxs(:,:,:,P_QNC), grid%scalar_btxe(:,:,:,P_QNC), &
                                                               grid%scalar_btys(:,:,:,P_QNC), grid%scalar_btye(:,:,:,P_QNC), &
                                                               'T' , &
                                                               spec_bdy_width , &
                                                               ids , ide , jds , jde , kds , kde , &
                                                               ims , ime , jms , jme , kms , kme , &
         ...
         END IF
        IF (config_flags%chem_opt==18) THEN
        CALL stuff_bdytend (smkbdy3dtemp2 , smkbdy3dtemp1 , REAL(interval_seconds) , &
                            grid%chem_btxs(:,:,:,P_SMOKE), grid%chem_btxe(:,:,:,P_SMOKE), &
                            grid%chem_btys(:,:,:,P_SMOKE), grid%chem_btye(:,:,:,P_SMOKE), &
                            'T' , &
                            spec_bdy_width , &
                            ids , ide , jds , jde , kds , kde , &
                            ims , ime , jms , jme , kms , kme , &
         ...
         ELSE
            ...
            IF (config_flags%chem_opt==18) THEN
               DO j = jps , jpe
                  DO k = kps , kpe
                     DO i = ips , ipe
                        smkbdy3dtemp1(i,k,j) = smkbdy3dtemp2(i,k,j)
                     END DO
                  END DO
               END DO
            END IF
