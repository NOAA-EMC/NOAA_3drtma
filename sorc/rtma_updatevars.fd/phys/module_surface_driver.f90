...
CONTAINS
   SUBROUTINE surface_driver( &
     ...
     & ,hfx_sea,qfx_sea,lh_sea,qsfc_sea,tsk_sea,znt_sea &
     & ,chs_sea,chs2_sea,cqs2_sea,cpm_sea,flhc_sea,flqc_sea &
     & ,qgh_sea,psih_sea,rmol_sea,ust_sea &
     & ,psim,p_phy,q10,q2,qfx,qsfc,qshltr,qz0 &
     & ,T2_alt,TH2_alt,Q2_alt &
   ...
   USE module_sf_noah_seaice_drv
   USE module_sf_mynn
   USE module_sf_mynn_v36
   USE module_sf_mynn_v34
   ...
   REAL, DIMENSION( ims:ime, jms:jme ) , INTENT(INOUT):: ALBSOL
   REAL, DIMENSION( ims:ime, jms:jme ) , INTENT(INOUT):: ALBBCKSOL
 ...
 GRAINXY, GDDXY, &
  XSAIXY, TAUSSXY, T2MVXY, T2MBXY, Q2MVXY, Q2MBXY, TRADXY, NEEXY, GPPXY, &
   ...
   INTEGER, OPTIONAL, INTENT(IN) :: bl_mynn_cloudpdf, &
                                                                   icloud_bl
   REAL, DIMENSION( ims:ime, jms:jme ) , INTENT(INOUT):: TH2_alt, T2_alt, Q2_alt
  ...
  real, optional, dimension(ims:ime,1:maxpatch,jms:jme ),intent(inout) :: &
   ...
   REAL, DIMENSION( ims:ime , jms:jme ) :: snowh_save
   REAL :: DTMIN,DTBL
  ...
  REAL, DIMENSION( ims:ime, 1:num_soil_layers*mosaic_cat, jms:jme ), OPTIONAL, INTENT(INOUT):: &
   ...
   REAL, OPTIONAL, DIMENSION( ims:ime, 1:num_soil_layers*mosaic_cat, jms:jme ), INTENT(INOUT) :: TGL_URB3D_mosaic
     ...
     REAL, DIMENSION( ims:ime, jms:jme ), OPTIONAL, INTENT(OUT) :: &
                     HFX_SEA, QFX_SEA, LH_SEA, QSFC_SEA, TSK_SEA, &
                     ZNT_SEA, CHS_SEA, CHS2_SEA, CQS2_SEA, CPM_SEA, &
                     FLHC_SEA, FLQC_SEA, QGH_SEA
     REAL, DIMENSION( ims:ime, jms:jme ), OPTIONAL :: PSIH_SEA
     REAL, DIMENSION( ims:ime, jms:jme ), OPTIONAL :: RMOL_SEA
     REAL, DIMENSION( ims:ime, jms:jme ), OPTIONAL :: UST_SEA
...
"Field ALBSI not found in input.  Field ALBSI is required if SEAICE_ALBEDO_OPT=2")
  ...
  isisfc = ( FRACTIONAL_SEAICE .EQ. 1 .AND. ( &
            ...
            (sf_sfclay_physics .EQ. MYNNSFCSCHEMEV36 ) .OR. &
            (sf_sfclay_physics .EQ. MYNNSFCSCHEMEV34 ) .OR. &
...
'Lacking arguments for SFCLAY_mynn in surface driver')
       ENDIF
    CASE(MYNNSFCSCHEMEV36)
       IF (PRESENT(qv_curr) .AND. PRESENT(qc_curr) &
            & .AND. PRESENT(qcg) ) THEN
          CALL wrf_debug(100,'in MYNNSFC V36')
          CALL SFCLAY_mynn_V36(u_phytmp,v_phytmp,t_phy,qv_curr, &
               p_phy,dz8w,cp,g,rcp,r_d,xlv,psfc,chs,chs2,cqs2,cpm, &
               znt,ust,pblh,mavail,zol,mol,regime,psim,psih, &
               xland,hfx,qfx,lh,tsk,flhc,flqc,qgh,qsfc,rmol, &
               u10,v10,th2,t2,q2,SNOWH, &
...
'Lacking arguments for SFCLAY_mynnV36 in surface driver')
       ENDIF
    CASE(MYNNSFCSCHEMEV34)
       IF (PRESENT(qv_curr) .AND. PRESENT(qc_curr) &
            & .AND. PRESENT(qcg) ) THEN
          CALL wrf_debug(100,'in MYNNSFCv3.4')
          CALL SFCLAY_mynn_v34(u_phytmp,v_phytmp,t_phy,qv_curr, &
               p_phy,dz8w,cp,g,rcp,r_d,xlv,psfc,chs,chs2,cqs2,cpm, &
               znt,ust,pblh,mavail,zol,mol,regime,psim,psih, &
               xland,hfx,qfx,lh,tsk,flhc,flqc,qgh,qsfc,rmol, &
               u10,v10,th2,t2,q2, &
...
'Lacking args for SFCLAY_mynnV34 in surf driver')
       ENDIF
     CASE (TEMFSFCSCHEME)
     ...
     CASE (RUCLSMSCHEME)
       IF (PRESENT(qv_curr) .AND. PRESENT(qc_curr) .AND. &
           ...
           DO j = j_start(ij) , j_end(ij)
              DO i = i_start(ij) , i_end(ij)
                 IF ( ( XICE(I,J) .GE. XICE_THRESHOLD ) .AND. ( XICE(i,j) .LE. 1. ) ) THEN
                    ALBBCKSOL(I,J) = SEAICE_ALBEDO_DEFAULT
                 ENDIF
              ENDDO
           ENDDO
           IF ( FRACTIONAL_SEAICE == 1 ) THEN
              DO j = j_start(ij) , j_end(ij)
                 DO i = i_start(ij) , i_end(ij)
                    IF ( ( XICE(I,J) .GE. XICE_THRESHOLD ) .AND. ( XICE(i,j) .LE. 1 ) ) THEN
                       ALBSOL(I,J) = (ALBSOL(I,J) - (1.-XICE(I,J))*0.08) / XICE(I,J)
                       EMISS(I,J) = (EMISS(I,J) - (1.-XICE(I,J))*0.98) / XICE(I,J)
           ...
           CALL LSMRUC( spp_lsm_loc, &
                ...
                chs,flqc,flhc,mavail,canwat,vegfra,albsol,znt, &
                z0,snoalb, albbcksol, lai, &
           ...
           IF ( FRACTIONAL_SEAICE == 1 ) THEN
              DO j=j_start(ij),j_end(ij)
                 DO i=i_start(ij),i_end(ij)
                    IF ( ( XICE(I,J) .GE. XICE_THRESHOLD) .AND. ( XICE(i,j) .LE. 1.0 ) ) THEN
                       albsol(i,j) = ( albsol(i,j) * XICE(i,j) ) + ( (1.0-XICE(i,j)) * 0.08 )
                       emiss(i,j) = ( emiss(i,j) * XICE(i,j) ) + ( (1.0-XICE(i,j)) * 0.98 )
                    ENDIF
                 ENDDO
              ENDDO
           ...
           ENDIF
            DO j=j_start(ij),j_end(ij)
               ...
               ENDDO
            ENDDO
          CALL SFCDIAGS_RUCLSM(HFX,QFX,TSK,QSFC,CQS,CQS2,CHS,CHS2, &
...
'Lacking arguments for P-X LSM in surface driver')
     ...
     IF (MYJ) call wrf_error_fatal3("<stdin>",3097,&
...
'Lacking arguments for SSIB in surface driver')
 ...
 430 CONTINUE
   ...
   IF ( LakeModel == 1 ) THEN
     ...
     if(lakemask(i,j) == 1.) then
            SNOW (i,j) = h2osno2d(i,j)
            SNOWH(i,j) = snowdp2d(i,j)
            snowfallac(i,j) = snowfallac(i,j) + max(0.,snowh(i,j) - snowh_save(i,j))
     else
            h2osno2d(i,j) = 0.
            snowdp2d(i,j) = 0.
     endif
     if(lakemask(i,j) == 1.) then
          if(lake_icefrac3d(i,1,j) > xice_threshold) then
             if ( fractional_seaice == 0 ) then
                  xice(i,j)=1.
             else if ( fractional_seaice == 1 ) then
 ...
 subroutine qnsesfc_seaice_wrapper(ITIMESTEP,HT,DZ, &
   ...
   SUBROUTINE mynn_seaice_wrapper(U3D,V3D,T3D,QV3D, &
     ...
     INTEGER, INTENT(IN ), optional ::bl_mynn_cloudpdf,icloud_bl,&
                                      ISFTCFLX, IZ0TLND
     ...
     REAL, DIMENSION( ims:ime, kms:kme, jms:jme ) , &
               INTENT(IN ) :: dz8w, &
                                                            ...
                                                            QC3D, &
                                                             U3D, &
                                                             V3D, &
                           rho,th3d,pi3d,tsq,qsq,cov,Sh3d,el_pbl, &
                                                qc_bl, cldfra_bl
     REAL, DIMENSION( ims:ime, jms:jme ) , &
               INTENT(IN ) :: MAVAIL, &
                                                            PBLH, &
