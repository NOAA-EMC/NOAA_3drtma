MODULE module_sf_mynn_v36
  USE module_model_constants, only: &
       &p1000mb, cp, xlv, ep_2
  USE module_sf_sfclay, ONLY: sfclayinit
  USE module_bl_mynn_v36, only: tv0, mym_condensation
...
CONTAINS
  ...
  END SUBROUTINE mynn_sf_init_driver
   SUBROUTINE SFCLAY_mynn_v36(U3D,V3D,T3D,QV3D,P3D,dz8w, &
      ...
      REAL, DIMENSION( ims:ime, kms:kme, jms:jme ) :: qc_bl,cldfra_bl
      ...
      REAL, DIMENSION( its:ite ) :: U1D, &
                                                        ...
                                                        U1D2,V1D2, &
                                                         ...
                                                         T1D,QC1D, &
                                                           ...
                                                           dz8w1d, &
                                                           dz2w1d
      REAL, DIMENSION( its:ite,kts:kts+1 ) :: qc_bl2D,cldfra_bl2D
      ...
      DO J=jts,jte
        DO i=its,ite
           ...
           if(bl_sgcloud > 0) then
             qc_bl2D(i,kts)=qc_bl(i,kts,j)
             qc_bl2D(i,kts+1)=qc_bl(i,kts+1,j)
             cldfra_bl2D(i,kts)=cldfra_bl(i,kts,j)
             cldfra_bl2D(i,kts+1)=cldfra_bl(i,kts+1,j)
           else
             qc_bl2D(i,kts)=0.
             qc_bl2D(i,kts+1)=0.
             cldfra_bl2D(i,kts)=0.
             cldfra_bl2D(i,kts+1)=0.
           endif
        ENDDO
        IF (itimestep==1) THEN
           DO i=its,ite
      ...
      ENDDO
    END SUBROUTINE SFCLAY_MYNN_v36
      ...
      IF((XLAND(I)-1.5).GE.0)THEN
       ...
       ELSE
          ...
          IF ( SNOWH(i) .GE. 0.1) THEN
             CALL Andreas_2002(ZNT(i),restar,z_t(i),z_q(i),BR(i))
          ELSE
...
END SUBROUTINE SFCLAY1D_mynn
   ...
   SUBROUTINE garratt_1992(Zt,Zq,Z_0,Ren,landsea)
    ...
    SUBROUTINE Andreas_2002(Z_0,Ren,Zt,Zq,BulkRi)
       IMPLICIT NONE
       REAL, INTENT(IN) :: Z_0, Ren, BulkRi
       REAL, INTENT(OUT) :: Zt, Zq
       REAL :: Ren2, zntsno
       ...
       IF (Z_0 >= SNOWZ0) THEN
          zntsno = SNOWZ0*EXP(-3.*MAX(BulkRi,0.0))
       ELSE
          zntsno = Z_0
       ENDIF
       Ren2 = Ren
