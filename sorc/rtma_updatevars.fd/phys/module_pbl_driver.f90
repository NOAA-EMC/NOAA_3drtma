...
CONTAINS
   ...
   USE module_state_description, ONLY : &
                   ...
                   MYNNPBLSCHEMEV36,MYNNPBLSCHEMEV34,&
        ...
        CASE (MYNNPBLSCHEME2, MYNNPBLSCHEME3)
           IF ( PRESENT( qv_curr ) .AND. PRESENT( qc_curr ) .AND. &
              ...
              CALL mynn_bl_driver(&
                   ...
                   &,kbot_shallow=kbot_shallow,mf_at_base=mf_at_base &
...
'Lack arguments to call MYNN pbl')
        ...
        CASE (MYNNPBLSCHEMEV36)
           IF ( PRESENT( qv_curr ) .AND. PRESENT( qc_curr ) .AND. &
                ...
                PRESENT(qke_adv) .AND. PRESENT(bl_mynn_tkeadvect) .AND.&
                PRESENT(vdfg) ) THEN
              CALL wrf_debug(100,'in MYNNPBLv36')
              IF (itimestep==1) THEN
                 initflag=1
              ELSE
                 initflag=0
              ENDIF
              CALL mynn_bl_driver_v36( &
                   &initflag=initflag, &
                   &grav_settling=grav_settling, &
                   &delt=dtbl,dz=dz8w, &
...
'Lack arguments to call MYNNv36 pbl')
           ENDIF
        CASE (MYNNPBLSCHEMEV34)
           IF ( PRESENT( qv_curr ) .AND. PRESENT( qc_curr ) .AND. &
                ...
                PRESENT(qke_adv) .AND. PRESENT(bl_mynn_tkeadvect) .AND.&
                PRESENT(vdfg) ) THEN
              CALL wrf_debug(100,'in MYNNPBLv34')
              IF (itimestep==1) THEN
                 initflag=1
              ELSE
                 initflag=0
              ENDIF
              CALL mynn_bl_driver_v34( &
                   &initflag=initflag, &
                   &grav_settling=grav_settling, &
                   &delt=dtbl, &
                   &dz=dz8w, &
...
'Lack arguments to call MYNNv34 pbl')
           ENDIF
        CASE (BOULACSCHEME)
...
'Lack arguments to call turbine_drag')
   ...
   END SELECT windfarm_select
       IF(gwd_opt .EQ. 1 .OR. gwd_opt .EQ. 2)THEN
          IF (PRESENT(dtaux3d)) THEN
             CALL gwdo( &
              ...
              ,ITS=its,ITE=ite,JTS=jts,JTE=jte,KTS=kts,KTE=kte )
          ENDIF
       ELSEIF(gwd_opt .EQ. 3 .OR. gwd_opt .EQ. 33)THEN
             CALL gwdo_gsd( &
               U3D=u_phytmp,V3D=v_phytmp,T3D=t_phy &
              ,QV3D=qv_curr &
              ,P3D=p_phy,P3DI=p8w,PI3D=pi_phy,Z=z &
              ...
              ,OA2D1ss=oa1ss,OA2D2ss=oa2ss,OA2D3ss=oa3ss,OA2D4ss=oa4ss &
              ,OL2D1ss=ol1ss,OL2D2ss=ol2ss,OL2D3ss=ol3ss,OL2D4ss=ol4ss &
              ,ZNU=znu,ZNW=znw,P_TOP=p_top &
              ,DZ=dz8w,PBLH=PBLH &
              ,CP=cp,G=g,RD=r_d &
              ,RV=r_v,EP1=ep_1,PI=3.141592653 &
