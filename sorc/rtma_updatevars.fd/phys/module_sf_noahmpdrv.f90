...
CONTAINS
  ...
  BGAPXY, WGAPXY, TGVXY, TGBXY, CHVXY, CHBXY, &
   ...
   GHBXY, IRGXY, IRCXY, IRBXY, TRXY, EVCXY, &
    ...
    USE NOAHMP_TABLES, ONLY: ISICE_TABLE, CO2_TABLE, O2_TABLE, DEFAULT_CROP_TABLE, ISURBAN_TABLE, NATURAL_TABLE, &
  ...
  REAL :: PAH
   ...
   ILOOP : DO I = its, ite
    ...
    ELSE
       ...
       CROPTYPE = 0
       IF (IDVEG == 10) CROPTYPE = DEFAULT_CROP_TABLE
...
SUBROUTINE TRANSFER_MP_PARAMETERS(VEGTYPE,SOILTYPE,SLOPETYPE,SOILCOLOR,CROPTYPE,parameters)
 ...
 END SUBROUTINE TRANSFER_MP_PARAMETERS
  ...
  IMPLICIT NONE
    ...
    REAL, DIMENSION(ims:ime,jms:jme), INTENT(IN) , OPTIONAL :: rivermask
    REAL, DIMENSION(ims:ime,jms:jme), INTENT(IN) , OPTIONAL :: nonriverxy
    ...
    END IF
       ...
       if(iopt_run.eq.5) then
          IF ( PRESENT(smoiseq) .AND. &
            ...
            PRESENT(pexpxy) .AND. &
            PRESENT(rechclim) .AND. &
            PRESENT(rivermask) .AND. &
            PRESENT(nonriverxy) ) THEN
...
"Problem with the logic assigning snow layers.")
  ...
  IMPLICIT NONE
    ...
    REAL, INTENT(IN), DIMENSION(ims:ime, jms:jme) :: rechclim ,rivermask ,nonriverxy
