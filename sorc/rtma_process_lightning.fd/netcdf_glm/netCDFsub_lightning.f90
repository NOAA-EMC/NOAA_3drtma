...
!     ylat
  ...
  STATUS = NF_INQ_VARID (NCID, 'flash_lat', VARID)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
  STATUS = NF_GET_VARA_REAL (NCID, VARID, START, COUNT, ylat)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
!
  STATUS = NF_INQ_VARID (NCID, 'flash_lon', VARID)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
  STATUS = NF_GET_VARA_REAL (NCID, VARID, START, COUNT, xlon)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
!
  STATUS = NF_INQ_VARID (NCID, 'flash_time_offset_of_first_event', VARID)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
  STATUS = NF_GET_VARA_REAL (NCID, VARID, START, COUNT, time)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
!
  STATUS = NF_INQ_VARID (NCID, 'flash_quality_flag', VARID)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
  STATUS = NF_GET_VARA_REAL (NCID, VARID, START, COUNT, strike)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
...
!
  ...
  STATUS = NF_INQ_DIMID(NCID, 'number_of_flashes', ATT_ID)
  IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR_lightning(STATUS)
