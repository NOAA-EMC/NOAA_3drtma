...
CONTAINS
   ...
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ), &
         INTENT(INOUT) :: &
                                                          W0AVG
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ), &
         OPTIONAL, INTENT(INOUT) :: cldfra_dp &
                                                    , cldfra_sh
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ), &
         INTENT(INOUT) :: udr_kf &
                                                    , ddr_kf &
...
wrf_err_message )
     ...
     CASE (GFSCHEME)
         ...
         CALL GFDRV(spp_conv,pattern_spp_conv,field_conv, &
               ...
               ,do_capsuppress=do_capsuppress &
               ,cap_suppress_loc=cap_suppress_loc &
               ,bl_mynn_edmf=grid%bl_mynn_edmf &
               ,chem=grid%chem,chem_opt=grid%chem_opt &
               ,gf_convmix=grid%chem_conv_tr &
               ,gf_wetrmv=grid%conv_tr_wetscav &
               ,num_chem=num_chem )
     CASE (CAMZMSCHEME)
