...
errstring)
             ...
             call compute_vdiff( lchnk, pcols, pver, pcnst, ncol, pmiddry8, pintdry8, rpdeldry8, t8, &
                  ...
                  tautmsy, dtk, topflx, errstring, tauresx, tauresy, 1, do_molec_diff, &
                  compute_molec_diff, vd_lu_qdecomp)
             if( errstring .ne. '' ) call wrf_error_fatal3("<stdin>",497,&
errstring)
          end if
...
'convect_shallow_init: shallow_scheme and eddy_scheme are incompatible' )
    ...
    if( errstring .ne. '' ) call wrf_error_fatal3("<stdin>",797,&
errstring )
    if( vdiff_select( fieldlist_wet, 'u' ) .ne. '' ) call wrf_error_fatal3("<stdin>",803,&
vdiff_select( fieldlist_wet, 'u' ) )
    if( vdiff_select( fieldlist_wet, 'v' ) .ne. '' ) call wrf_error_fatal3("<stdin>",805,&
vdiff_select( fieldlist_wet, 'v' ) )
    if( vdiff_select( fieldlist_wet, 's' ) .ne. '' ) call wrf_error_fatal3("<stdin>",807,&
vdiff_select( fieldlist_wet, 's' ) )
    ...
    do k = 1, pcnst
       ...
       if( cnst_get_type_byind(k) .eq. 'wet' ) then
          if( vdiff_select( fieldlist_wet, 'q', k ) .ne. '' ) call wrf_error_fatal3("<stdin>",833,&
