...
CONTAINS
   SUBROUTINE radiation_driver ( &
     ...
     & ,ALBBCK,ALBSOL,ALBBCKSOL &
              ...
              ,progn &
              ,smkfdb_opt, chem_opt, AOD3D_SMOKE &
   ...
   INTEGER, OPTIONAL, INTENT(IN) :: smkfdb_opt, chem_opt
   ...
   INTEGER, INTENT(IN), OPTIONAL :: spp_rad
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ),INTENT(IN),OPTIONAL ::pattern_spp_rad
   REAL, DIMENSION( ims:ime, kms:kme, jms:jme ), OPTIONAL , &
   ...
   REAL, DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) :: Gx,Bx,gg,bb, &
    ...
    REAL, DIMENSION(ims:ime,kms:kme,jms:jme), OPTIONAL, INTENT(IN) :: AOD3D_SMOKE
   ...
   real, dimension(ims:ime,jms:jme), INTENT(OUT) :: obscur
   integer, dimension(ims:ime,jms:jme), INTENT(OUT) :: mask
   real, INTENT(OUT) :: elat_track, elon_track
     ...
     INTEGER, PARAMETER:: taer_asy_opt = 3
      integer :: ilst
      real :: dm
      real, dimension (2), parameter :: d = (/0.1,0.25/)
      integer, dimension (1:21) :: istwe
      DATA istwe /5*1,2,2,1,1,5*2,1,2,2,1,2,2,2/
   ...
   IF (PRESENT(f_qnwfa) .AND. PRESENT(f_qnifa) .AND. PRESENT(taod5503d) .AND. PRESENT(taod5502d)) THEN
      IF (F_QNWFA .AND. aer_opt.eq.3 .AND. &
        ...
        IF (chem_opt==18 .AND. smkfdb_opt==1) THEN
            do j=jts,jte
             do i=its,ite
              do k=kts,kte
                 taod5503d(i,k,j)= taod5503d(i,k,j) + MIN(3.0,AOD3D_SMOKE(i,k,j))
              enddo
             enddo
            enddo
        ENDIF
   ...
   DO ij = 1 , num_tiles
     ...
     do i=its,ite
       if(itimestep == 1 ) then
         albsol(i,j) = albedo(i,j)
         albbcksol(i,j) = albbck(i,j)
       endif
     if(alb_sol == 1) then
      if(coszen(i,j) > 0.) then
       if(xland(i,j) < 1.5 .and. snow(i,j)==0. .and. xice(i,j)==0.) then
         ilst=istwe(ivgtyp(i,j))
         dm = (1.+2.*d(ilst))/(1.+2.*d(ilst)*coszen(i,j))
         albsol(i,j) = albbck(i,j)*dm
         albbcksol(i,j) = albbck(i,j)*dm
       endif
      endif
     endif
     enddo
     ...
     IF ( F_QC .AND. F_QI ) THEN
        ...
        IF ( PRESENT ( CLDFRA_DP ) ) THEN
          IF ( icloud_cu .EQ. 2 ) THEN
             ...
             DO k = kts,kte
             DO i = its,ite
                CLDFRA(i,k,j)=(1.-(cldfra_dp(i,k,j)+cldfra_sh(i,k,j)))*CLDFRA(i,k,j)
                CLDFRA(i,k,j)=CLDFRA(i,k,j)+(cldfra_dp(i,k,j)+cldfra_sh(i,k,j))
                CLDFRA(i,k,j)=AMIN1(1.0,CLDFRA(i,k,j))
                qc(i,k,j) = qc(i,k,j)+qc_cu(i,k,j)*(cldfra_dp(i,k,j)+cldfra_sh(i,k,j))
                qi(i,k,j) = qi(i,k,j)+qi_cu(i,k,j)*(cldfra_dp(i,k,j)+cldfra_sh(i,k,j))
             ENDDO
             ENDDO
             ENDDO
          ENDIF
        ...
        IF ( PRESENT ( CLDFRA_BL ) .AND. PRESENT ( QC_BL ) ) THEN
         ...
         ENDIF
           ...
           DO i = its,ite
           DO k = kts,kte
              IF (qc(i,k,j) < 1.E-6 .AND. qi(i,k,j) < 1.E-8 .AND. CLDFRA_BL(i,k,j)>0.001) THEN
...
'arguments not present for calling cam radiation' )
        ...
        CASE (RRTMG_SWSCHEME)
             ...
             CALL RRTMG_SWRAD( &
                     ...
                     ALBEDO=ALBSOL,t3d=t,t8w=t8w,TSK=TSK, &
                     ...
                     asyaer3d_sw=asyaer_sw, &
                     swddir=swddir,swddni=swddni,swddif=swddif, &
                     swddnic=swddnic,swddifc=swddifc, &
                     obscur=obscur, &
                     xcoszen=coszen,julian=julian,mp_physics=mp_physics,&
                     spp_rad=spp_rad,pattern_spp_rad=pattern_spp_rad )
             DO j=jts,jte
             DO k=kts,kte
             DO i=its,ite
                RTHRATEN(I,K,J)=RTHRATEN(I,K,J)+RTHRATENSW(I,K,J)
...
wrf_err_message )
     ...
     IF (sw_physics .gt. 0 .and. .not.gfdl_sw .and. .not.flg_sw) THEN
        ...
        DO j=jts,jte
        DO i=its,ite
           SWDOWN(I,J)=GSW(I,J)/(1.-ALBSOL(I,J))
        ENDDO
        ENDDO
     ENDIF
