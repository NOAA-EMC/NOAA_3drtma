...
CONTAINS
   ...
   LOGICAL :: print_flag
   print_flag=wrf_at_debug_level(3000)
         NZS=NSL
         NDDZS=2*(nzs-2)
...
print *,'INFMAX,INFMAX1,HYDRO(1)*SOILIQW(1),-TOTLIQ', &
  ...
  10 I1=I
   ...
   1 PRINT *,'     AVOST IN VILKA     Table index= ',I
       ...
       CALL wrf_error_fatal3("module_sf_ruclsm.b",6681,&
'  Crash in surface energy budget  ' )
  ...
  LOGICAL, intent(in) :: rdlai2d
   ...
   REAL, DIMENSION( 1:NLCAT ) :: ZNTtoday, LAItoday, deltalai
    ...
    ENDIF
      ...
      enddo
    IF ( wrf_at_debug_level(3000) ) THEN
      if(i.eq.539.and.j.eq.334)then
        print *,'ivgtyp,factor,vegfrac,shdmin,shdmax,deltalai(ivgtyp),laitoday(ivgtyp),znttoday(ivgtyp)', &
 ...
 IF(.not.restart)THEN
   ...
   IF ( errflag .EQ. 1 ) THEN
      CALL wrf_error_fatal3("module_sf_ruclsm.b",7260,&
  ...
  END SUBROUTINE ruclsminit
       ...
       IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF(ierr .NE. OPEN_OK ) THEN
          ...
          CALL wrf_error_fatal3("module_sf_ruclsm.b",7395,&
message )
        END IF
 ...
 2000 FORMAT (A8)
        ...
        IF (LUMATCH == 1) then
           ...
           if ( ierr /= 0 ) call wrf_error_fatal3("module_sf_ruclsm.b",7462,&
"Problem read URBAN from VEGPARM.TBL")
 ...
 2002 CONTINUE
    ...
    ENDIF
        IF (LUMATCH == 0) then
           CALL wrf_error_fatal3("module_sf_ruclsm.b",7476,&
"Land Use Dataset '"//MMINLURUC//"' not found in VEGPARM.TBL.")
      ...
      IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF(ierr .NE. OPEN_OK ) THEN
          ...
          CALL wrf_error_fatal3("module_sf_ruclsm.b",7515,&
message )
        END IF
 ...
 2003 CONTINUE
      ...
      IF(LUMATCH.EQ.0)THEN
          ...
          CALL wrf_error_fatal3("module_sf_ruclsm.b",7576,&
'INCONSISTENT OR MISSING SOILPARM FILE' )
      ...
      IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF(ierr .NE. OPEN_OK ) THEN
          ...
          CALL wrf_error_fatal3("module_sf_ruclsm.b",7588,&
message )
        END IF
        READ (19,*)
        READ (19,*)
