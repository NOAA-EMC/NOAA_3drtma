   ...
   ENDIF
    ...
    print *,'zero out buckets'
       ...
       DO i = its, min(ite,ide-1)
            ...
            grid%prec_acc_nc1(i,j) = 0.
            grid%snow_acc_nc(i,j) = 0.
            grid%snow_acc_nc1(i,j) = 0.
            grid%graup_acc_nc(i,j) = 0.
            grid%graup_acc_nc1(i,j) = 0.
...
'---- Error : Cannot use old input and try to use hybrid vertical coordinate option' )
   ...
   IF(.not.config_flags%restart)THEN
    ...
    ELSE
     ...
     END IF
    END IF
  IF ( ( grid%dfi_opt .EQ. DFI_NODFI ) .and. config_flags%rebalance == 1) THEN
  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   DO ij = 1, grid%num_tiles
     CALL phy_init ( grid%id , config_flags, grid%DT, grid%RESTART, grid%znw, grid%znu, &
                      ...
                      grid%rechclim ,grid%rivermask ,grid%nonriverxy, &
   ...
   ENDIF
    IF ( config_flags%chem_opt>0 ) THEN
      CALL smoke_init (grid%id,chem, &
        ...
        do j=jts,min(jte,jde-1)
           do i=its,min(ite,ide-1)
              grid%coef_bb_dc(i,j)= 0.
              grid%fire_hist(i,j)= 1.
              grid%min_fplume(i,j)= 0
              grid%max_fplume(i,j)= 0
              grid%ddmass_smoke(i,j)= 0.
              do k=kts,min(kte,kde-1)
                 grid%aod3d_smoke(i,k,j)= 0.
              enddo
          enddo
        enddo
   ENDIF
...
grid%emis_ant,grid%emis_dust,grid%emis_seas,grid%emis_seas2,grid%emis_vol,grid%ebu,grid%ebu_in,grid%emis_aircraft,grid%ext_coef, &
grid%bscat_coef,grid%asym_par,grid%conv_ct,grid%chem_ct,grid%vmix_ct,grid%advh_ct,grid%advz_ct,grid%dvel,grid%vprm_in, &
grid%wet_in,grid%chem,grid%chem_bxs,grid%chem_bxe,grid%chem_bys,grid%chem_bye,grid%chem_btxs,grid%chem_btxe,grid%chem_btys, &
grid%chem_btye,grid%tracer,grid%tracer_bxs,grid%tracer_bxe,grid%tracer_bys,grid%tracer_bye,grid%tracer_btxs,grid%tracer_btxe, &
grid%tracer_btys,grid%tracer_btye,grid%nba_mij,grid%nba_rij &
      )
