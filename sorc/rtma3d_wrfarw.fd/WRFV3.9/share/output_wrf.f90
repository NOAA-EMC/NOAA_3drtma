...
! For 1 only
  ...
  integer, parameter :: WRF_HDF5_ERR_ATTRIBUTE_OTHERS = -321
    ...
    INTEGER km_opt, diff_opt, damp_opt, &
            ...
            use_theta_m, use_maxw_level, use_trop_level,hybrid_opt
    ...
    call end_timing('barrier at top of output_wrf')
    ...
    IF ( ierr /= 0 ) THEN
      ...
      CALL wrf_error_fatal3("output_wrf.b",131,&
wrf_err_message )
    ENDIF
    ...
    IF ( .NOT. dryrun ) THEN
       ...
       call wrf_get_dm_communicator(comm)
       call MPI_Barrier(comm,ierr)
       call end_timing('barrier before iosync')
    ...
    CALL wrf_debug( 300 , wrf_err_message )
    call end_timing('output_wrf '//trim(fname))
    RETURN
