...
NASALARC=${GSI_ROOT}/process_NASALaRC_cloud.exe
fixdir=${FIX_ROOT}
...
if [ -x "process_NASALaRC_cloud.exe" ]; then
  ${RM} process_NASALaRC_cloud.exe
fi
...
${LN} -s ${NASALARC_DATA}/goes-west/visst/rr/netcdf/G15V3.0.RR.${YYYYJJJ12}.${HH12}30.PX.08K.CDF ./nasaLaRC_gesoWest.nc
${LN} -s ${NASALARC_DATA}/goes-east/visst/rr/netcdf/G16V3.0.RR.${YYYYJJJ12}.${HH12}45.PX.06K.CDF ./nasaLaRC_gesoEast.nc

echo ${YYYYMMDDHH} > nasaLaRC_cycle_date

#  Run obs pre-processor
${MPIRUN} -envall -np ${np} ${NASALARC} > stdout_satellite 2>&1
error=$?
if [ ${error} -ne 0 ]; then
