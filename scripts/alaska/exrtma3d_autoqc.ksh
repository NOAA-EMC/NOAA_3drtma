...
while [ ${i} -lt ${max_cycs} ]; do
  ...
  probe_HH=`echo $probe | cut -c 9-10`
  probe_dir=${COMOUTautoqc_base}/${RUN}.${probe_YYYYMMDD}/${dom}/autoqcprd.t${probe_HH}z
  if [ $probe_HH -eq "23" ] && [ -s ${probe_dir}/${RUN}ak.t${probe_HH}z.accept_merged.txt ]; then
...
while [ ${i} -lt ${max_cycs} ]; do
  ...
  probe_HH=`echo $probe | cut -c 9-10`
  probe_dir=${COMOUTautoqc_base}/${RUN}.${probe_YYYYMMDD}/${dom}/autoqcprd.t${probe_HH}z
  if [ $probe_HH -eq "23" ] && [ -s ${probe_dir}/${RUN}ak.t${probe_HH}z.aircraft_rjs_merged.txt ]; then
    echo $probe
...
if [ ${HH} -eq 23 ]; then
  ...
  python ${NWROOT}/ush/aircraft_reject_lists.py ${NET} ${RUN}ak ${startcyc_aircraft} ${YYYYMMDDHH} ${DATA} ${COMOUTautoqc_rtma3d} ${PDYprev_aircraft_dir} ${PDYprev_aircraft} ${EXECrtma3d}/${NET}_read_diag ${dom}
...
tar -cvf ${RUN}ak.t${HH}z.autoqc_output.tar ${RUN}ak.t${HH}z.*.csv
gzip ${RUN}ak.t${HH}z.autoqc_output.tar
${CP} -p ${RUN}ak.t${HH}z.autoqc_output.tar.gz ${COMOUTautoqc_rtma3d}

${CP} -p ${RUN}ak.t${HH}z.*.txt ${COMOUTautoqc_rtma3d}
${CP} -p ${RUN}ak.t${HH}z.*database*.db ${COMOUTautoqc_rtma3d}
${CP} -p ${RUN}ak.t${HH}z.accept_daily_*.csv ${COMOUTautoqc_rtma3d}
