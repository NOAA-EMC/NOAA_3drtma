...
#  loop of searching for firstguess in HRRR forecast file
   while [ $ic -le ${ic_max} ] ; do
     ...
     probe_hrrr_guess_nc=hrrr_${hrrrCYCLE}f0${hrrrFHH}
     size_match=no
     found_hrrrges=no
...
#        loop of checking the filesize of hrrr forecast
         ...
         if [[ ${size_match} =~ [yYtT] ]] ; then
             cpreq $GESINhrrr/${probe_hrrr_guess_nc} ${RUN}.t${cyc}z.firstguess.nc
             echo "HRRR ${ic}-hour forecast file ${probe_hrrr_guess_nc} is used as the firstguess for analysis cycle at ${PDY} ${cyc}Z"
             echo "export hrrrCYCLE=$hrrrCYCLE" >> $COMOUT/${RUN}.t${cyc}z.envir.sh
             echo "export hrrrFHH=$hrrrFHH" >> $COMOUT/${RUN}.t${cyc}z.envir.sh
...
# Saving this problematic file for investigation later 
             cpreq -p $GESINhrrr/${probe_hrrr_guess_nc} ${COMOUT}/${RUN}.t${cyc}z.${probe_hrrr_guess_nc}
# Store information needed for retrospective runs in rerun_info.txt
             ...
             echo "WARNING: HRRR ${ic}-hour forecast file ${probe_hrrr_guess_nc} exists for analysis cycle at ${PDY} ${cyc}Z, but its filesize (${filesize}) does not match the standard size (${targetsize_hrrr}) even after waiting for ${ics_max} minutes. Try to search in the earlier HRRR forecast files ..."
         fi
     else
         echo "HRRR ${ic}-hour forecat file ${probe_hrrr_guess_nc} is not available for analysis cycle at ${PDY} ${cyc}Z. Try to search in the earlier HRRR foreast files ... "
     fi
   ...
   if [[ ${found_hrrrges} =~ [NnFf] ]] ; then
       err_exit "No HRRR guess available. The missing files in the above while-do loop are of the form $GESINhrrr/${dom}/hrrr_${hrrrCYCLE}f0${hrrrFHH}. The script must be able to find at least one file out of the 9 files that it queries"
   fi
...
# grid_specs: for hrrr-based 3D RTMA on CONUS domain
...
#  Sea-Land Mask for the correct interpolation of the howv Background.
  cpreq -p $FIXrtma3d/${RUN}_hrrr_conus_3km_slmask_nolakes.grib2    ./slmask.grib2
...
#
   echo "COMINww3GL is $COMINww3GL (Wave background from Great Lakes model)"
   ...
   while [ $ic -le 23 ] ; do
      ...
      ww3CYCLE_GL=`$NDATE -$ww3FHH_GL $CDATE`
      ...
      if [ -s $probe_ww3_GL_guess_grb2 ]; then
         ...
         echo "found wave background for Great Lakes: $probe_ww3_GL_guess_grb2"
         ...
         echo "export ww3_fguess_gl=glwu.${ww3PDY_GL}_glwu.grlr_500m.t${ww3CC_GL}z.grib2" >> rerun_info.txt
  ...
  echo "COMINww3 is $COMINww3 (Wave background from WW3 Ocean Wave model)"
   ...
   while [ $ic -le 24 ] ; do
      ...
      if [ -s "${probe_ww3_guess_grb2[0]}" ] && \
         ...
         ${HOMErtma3d}/scripts/ex${RUN}_GribMerge.sh ${GL_InputGribmerge} -i ww3.guess0.grib2 -i ww3.guess1.grib2 \
         ...
         echo "export ww3_fguess_arctic=gfs.${ww3PDY}_gfswave.t${ww3CC}z.arctic.9km.f${ww3FHH}.grib2" >> rerun_info.txt
         echo "export ww3_fguess_global=gfs.${ww3PDY}_gfswave.t${ww3CC}z.global.0p16.f${ww3FHH}.grib2" >> rerun_info.txt
...
# 3. Appending Wave height (2-D) field to 3D-RTMA firstguess file (netcdf format)
    ...
    FGS_FILE="${RUN}.t${cyc}z.firstguess.nc"           
...
#    and dumping out to grib2 file
   ...
   ic=1
   while [ $ic -le 3 ] ; do
      PRE_YYYYMMDDHH=`$NDATE -${ic} $CDATE`
      PRE_YYYYMMDD=$(echo ${PRE_YYYYMMDDHH} | cut -c1-8)
      PRE_HH=$(echo ${PRE_YYYYMMDDHH} | cut -c9-10)
      ic2=$(printf %02d ${ic})

      hrrr_guess_grb2=${COMINhrrr}/hrrr.${PRE_YYYYMMDD}/${dom}/hrrr.t${PRE_HH}z.wrfprsf${ic2}.grib2

      if [[ -f ${hrrr_guess_grb2} ]] ; then 
         echo "found HRRR ${ic} hour forecast grib2 file ${hrrr_guess_grb2} and retrieve 10-m Wind Gust from it: "
         ...
         found_gustges=yes
         echo "export gusthrrrCYCLE=$PRE_YYYYMMDDHH" >> $COMOUT/${RUN}.t${cyc}z.envir.sh
         echo "export gusthrrrFHH=$PRE_HH" >> $COMOUT/${RUN}.t${cyc}z.envir.sh
         echo "export gusthrrrfgs=hrrr.t${PRE_HH}z.wrfprsf${ic2}.grib2" >> rerun_info.txt
         break
      else
         let "ic=ic+1"
      fi
...
#    and dumping out to grib2 file
   ...
   while [ $ic -le 3 ] ; do
      ...
      if [[ -f ${hrrr_guess_grb2} ]] ; then 
         echo "found HRRR ${ic} hour forecast grib2 file ${hrrr_guess_grb2} and retrieve Surface Visibility from it: "
         rm -f ./hrrr_guess.grib2
         cpreq ${hrrr_guess_grb2}   ./hrrr_guess.grib2
         FHH_string=":VIS:surface:$ic hour fcst:"
# Change max from 90000 m to 16000 m
         wgrib2 ./hrrr_guess.grib2 -match "${FHH_string}" -rpn "16000:min" -grib_out ./vis.guess.grib2
         export err=$?; err_chk

         found_visges=yes
         echo "export vishrrrCYCLE=$PRE_YYYYMMDDHH" >> $COMOUT/${RUN}.t${cyc}z.envir.sh
         echo "export vishrrrFHH=$PRE_HH" >> $COMOUT/${RUN}.t${cyc}z.envir.sh
         echo "export vishrrrfgs=hrrr.t${PRE_HH}z.wrfprsf${ic2}.grib2" >> rerun_info.txt
   ...
   if [[ "${found_visges}" == "no" ]] ; then
      err_exit "Could NOT find any HRRR 0~3 hours forecast grib2 file to \
                provide firstguess for surface visibility. "
   fi
...
if [ $SENDCOM = YES ]; then
  ...
  cat $COMOUT/${RUN}.t${cyc}z.envir.sh rerun_info.txt > $COMOUT/${RUN}.t${cyc}z.rerun_info.txt
  cpreq -p ${RUN}.t${cyc}z.firstguess.nc    $COMOUT 
  cpreq $GESINhrrr/${probe_hrrr_guess_nc} $COMOUT/${RUN}.t${cyc}z.hrrr_${hrrrCYCLE}f0${hrrrFHH}
  cpreq $probe_ww3_GL_guess_grb2 $COMOUT/${RUN}.t${cyc}z.glwu.${ww3PDY_GL}_glwu.grlr_500m.t${ww3CC_GL}z.grib2
  wgrib2 $COMINww3/gfs.${ww3PDY}/${ww3CC}/wave/gridded/gfswave.t${ww3CC}z.arctic.9km.f${ww3FHH}.grib2 \
         -match "HTSGW" -grib $COMOUT/${RUN}.t${cyc}z.gfs.${ww3PDY}_gfswave.t${ww3CC}z.arctic.9km.f${ww3FHH}.grib2_htsgw
  wgrib2 $COMINww3/gfs.${ww3PDY}/${ww3CC}/wave/gridded/gfswave.t${ww3CC}z.global.0p16.f${ww3FHH}.grib2 \
         -match "HTSGW" -grib $COMOUT/${RUN}.t${cyc}z.gfs.${ww3PDY}_gfswave.t${ww3CC}z.global.0p16.f${ww3FHH}.grib2_htsgw
  cpreq ww3.guess.grib2 $COMOUT/${RUN}.t${cyc}z.fgs.howv.grib2
  cpreq gust.guess.grib2 $COMOUT/${RUN}.t${cyc}z.fgs.gust.grib2
  cpreq vis.guess.grib2 $COMOUT/${RUN}.t${cyc}z.fgs.vis.grib2
fi
