...
if [ -r ${COMIN}/${RUN}.t${cyc}z.firstguess.nc ]; then
  cpfs ${COMIN}/${RUN}.t${cyc}z.firstguess.nc ./wrf_inout
  echo " Cycle ${CDATE}: GSI background=${COMIN}/${RUN}.t${cyc}z.firstguess.nc"
else
...
if [ -s ${COMINobsproc}/${NET}.t${cyc}z.prepbufr.tm00 ]; then
  ...
  cpreq -p ${COMINobsproc}/${NET}.t${cyc}z.prepbufr.tm00 ${COMOUT}
else
  err_exit "prepbufr file $COMINobsproc/${NET}.t${cyc}z.prepbufr.tm00 not found"
fi

if [ -s ${COMOUT}/${RUN}.t${cyc}z.NSSLRefInGSI.bufr ]; then
  cpreq -p ${COMOUT}/${RUN}.t${cyc}z.NSSLRefInGSI.bufr ./refInGSI
else
  echo "WARNING: ${RUN}.t${cyc}z.NSSLRefInGSI.bufr is not available ..."
fi

if [ -s ${COMOUT}/${RUN}.t${cyc}z.LightningInGSI_bufr.bufr ]; then
  cpreq -p ${COMOUT}/${RUN}.t${cyc}z.LightningInGSI_bufr.bufr ./lghtInGSI
else
  echo "WARNING: ${RUN}.t${cyc}z.LightningInGSI_bufr.bufr is not available ..."
fi

if [ -s ${COMOUT}/${RUN}.t${cyc}z.NASALaRCCloudInGSI.bufr ]; then
  cpreq -p ${COMOUT}/${RUN}.t${cyc}z.NASALaRCCloudInGSI.bufr ./larcInGSI
else
  echo "WARNING: ${RUN}.t${cyc}z.NASALaRCCloudInGSI.bufr is not available ..."
fi
...
if [ ${HRRRDAS_BEC} -eq 1 ]; then
  ...
  while [[ $c -le 36 ]]; do
   cc=$(printf "%02d" $c)
   hrrrdas_file=${GESINhrrrdas}/hrrrdas_small_d02_${time_1hour_ago}00f01_mem00${cc}
   ls ${hrrrdas_file} >> filelist.hrrrdas
   ((c = c + 1))
  done
  ...
  while [[ $c -le 36 ]]; do
   cc=$(printf "%02d" $c)
   hrrrdas_file=${GESINhrrrdas}/hrrrdas.t${cyc}z/hrrrdas_small_d02_${time_1hour_ago}00f01_mem00${cc}_thinned
   ls ${hrrrdas_file} >> filelist.hrrrdas
   ((c = c + 1))
  done
else
...
if [[ ${hrrrmem} -gt 30 ]] && [[ ${HRRRDAS_BEC} -ge 1  ]]; then #if HRRRDAS BEC is available, use it as first choice
  ...
  ens_h=20 #40 #110
  ens_v=1 #3
  echo " Cycle ${YYYYMMDDHH}: GSI hybrid uses HRRRDAS BEC with n_ens=${nummem}"
elif [[ ${nummem} -eq 80 ]]; then
# need to test this option to make sure it works as backup to missing HRRRDAS
...
if [[ "${readin_localization}" == ".true." ]] ; then
   cpreq ${PARMrtma3d}/${RUN}_hybens_info_hrrrdas hybens_info
...
anavinfo=${FIXrtma3d}/${RUN}_anavinfo_arw_netcdf
...
BERROR=${FIXrtma3d}/${RUN}_berror_stats_hz01_DLS_TUNED_to_8th

SATANGL=${FIXrtma3d}/${RUN}_global_satangbias.txt
SATINFO=${FIXrtma3d}/${RUN}_global_satinfo.txt
CONVINFO=${FIXrtma3d}/${RUN}_convinfo_v1.0.txt
OZINFO=${FIXrtma3d}/${RUN}_global_ozinfo.txt
PCPINFO=${FIXrtma3d}/${RUN}_global_pcpinfo.txt
OBERROR=${FIXrtma3d}/${RUN}_errtable_smallSFCerr_ascat

#==========================================================================#
# If doing the analysis of wave height (HOWV) in 3DRTMA
if [[ ${RUN_HOWV} =~ [TtYy] ]] ; then
   anavinfo=${FIXrtma3d}/${RUN}_anavinfo_arw_howv_netcdf
   CONVINFO=${FIXrtma3d}/${RUN}_convinfo_v1.0_howv.txt
fi
#==========================================================================#
...
#   Note: in current setup, this option "i_gsdsfc_uselist" is also
#         used to control the selection of AIRCRAFT reject list.
#         "i_gsdsfc_uselist" can be pre-set in xml file under workflow directory.
# 1: using GSD Surface Obs uselist
#      and GSD AIRCRAFT report ject list also;
#      wind bias correction file (generated by AutoQC) is not used; 
# 2: using surface obs uselist generated by EMC Automated QC package (<== default)
#      and AutoQC generated AIRCRAFT report reject list as well;
#      wind bias correction file (generated by AutoQC) is used; 
i_gsdsfc_uselist=${i_gsdsfc_uselist:-2}

# Get mesonet accept list derived from automated QC package
found_aclist=False
...
# Matt please revise with your backup accept list changes.
# i_gsdsfc_uselist = 1 ==> using GSD mesonet obs uselist and aircraft report rejectlist
if [ "${i_gsdsfc_uselist}" -eq 1 ] ; then
   echo "Using GSD Surface Obs Uselist and GSD Aircraft Report Reject-list if i_gsdsfc_uselist=1"
...
#  AIRCRAFT Reject list
   if [[ -f "${FIXrtma3d}/obsuselist/amdar_reject_lists/${DATEm24}_rejects.txt" ]] ; then
       ...
       cp ${FIXrtma3d}/${RUN}_aircraft_reject_list.txt current_bad_aircraft
   fi
...
#  MESONET Use list
   if [[ -f "${FIXrtma3d}/obsuselist/mesonet_uselists/${YYm24}-${MMm24}-${DDm24}_meso_uselist.txt" ]] ; then
       ...
       cp ${FIXrtma3d}/${RUN}_mesonet_uselist.txt gsd_sfcobs_uselist.txt
   fi

#  Do not use the wind bias correction file generated by AutoQC if i_gsdsfc_uselist = 1
#   (remove it for now, may take it back in the future)
   if [[ -f stnwindbiascor ]] ; then
       echo "NOT using wind bias correction at observation stations (stnwindbiascor) if i_gsdsfc_uselist=1"
       rm -f stnwindbiascor
   fi

fi
cp ${FIXrtma3d}/${RUN}_gsd_sfcobs_provider.txt gsd_sfcobs_provider.txt

# Only need this file for single obs test
bufrtable=${PARMrtma3d}/${RUN}_prepobs_prep.bufrtable
...
estvisoe=2.61            # Obs Err of visibility (in transformed g-space, not in physical space)
vis_thres=16000.0        # upper-bound set for visibility (16 km, ~10 miles)
scale_cv=1.0             # scaling factor used in nonlinear transform
corp_vis0=3.0            # static BE of vis (in transformed g-space, not in physical space)
...
if [[ ${RUN_HOWV} =~ [TtYy] ]] ; then
  ${CP} ${PARMrtma3d}/${RUN}_gsiparm.anl_howv.sh gsiparm.anl.sh      # with setup for howv (wave height)
else
  ${CP} ${PARMrtma3d}/${RUN}_gsiparm.anl.sh gsiparm.anl.sh
fi
...
cpreq -p ${FIXrtma3d}/${RUN}_conus_terrain.dat          ./rtma_terrain.dat
cpreq -p ${FIXrtma3d}/${RUN}_conus_anl_slmask.dat       ./rtma_slmask.dat
cpreq -p ${FIXrtma3d}/${RUN}_valley_map_hrrr_conus_ieee.dat  ./valley_map.dat

# Copy MESONET wind observation sensor height list (same as used in 2DRTMA)
cpreq -p ${FIXrtma3d}/${RUN}_conus_provider_windheight  ./provider_windheight

## satellite bias correction
cpreq ${FIXrtma3d}/${RUN}_rap_satbias_starting_file.txt ./satbias_in
cpreq ${FIXrtma3d}/${RUN}_rap_satbias_pc_starting_file.txt ./satbias_pc
...
#  binary format obs diag
   ...
   for type in $listall_cnv_bin; do
      ...
      if [[ $count -gt 0 ]]; then
         ...
         cpreq diag_${type}_${string}.gz $COMOUT/${RUN}.t${cyc}z.diag_${type}_${string}.gz
         chgrp rstprod $COMOUT/${RUN}.t${cyc}z.diag_${type}_${string}.gz
      fi
   done
done
...
if [[ ${RUN_HOWV} =~ [TtYy] ]] ; then
  ...
  cat fit_p1 fit_w1 fit_t1 fit_q1 fit_pw1 fit_rad1 fit_rw1 fit_sst fit_gust fit_vis fit_howv > ${COMOUT}/${RUN}.t${cyc}z.fits
else
  cat fit_p1 fit_w1 fit_t1 fit_q1 fit_pw1 fit_rad1 fit_rw1 fit_sst fit_gust fit_vis > ${COMOUT}/${RUN}.t${cyc}z.fits
fi

# Saving ANALYSIS, DIAG, Obs-Fitting files TO COM2 DIRECTORY AS PRODUCT for archive
cpreq -p gsiparm.anl ${COMOUT}/${RUN}.t${cyc}z.gsiparm.anl
...
cpreq -p misc_info_${CDATE}.tar          ${COMOUT}/${RUN}.t${cyc}z.misc_info.tar

cpreq -p fort.220   ${COMOUT}/${RUN}.t${cyc}z.minimization_info
cpreq -p filelist03                          ${COMOUT}/${RUN}.t${cyc}z.filelist03
