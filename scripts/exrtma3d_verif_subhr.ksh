...
pb2nc ${urma_dir_ops}/${mod}.t${HH}${subcyc}z.prepbufr.tm00 \
      ${urma_dir}/${mod}.t${HH}${subcyc}z.prepbufr.tm00.nr.nc ${config_dir}/PB2NCConfig_DNG_new -v 2
      export err=$? ; err_chk
      if [ ${err} -ne 0 ]; then
...
for fld in $flds; do
  for exp in $exps; do
    mkdir -p $stat_dir/${dom_out}/${exp}/${fld}
        point_stat ${COMOUTpost_rtma3d}/${PROD_HEAD}.wrfprs_subhrconus_00.grib2 \
